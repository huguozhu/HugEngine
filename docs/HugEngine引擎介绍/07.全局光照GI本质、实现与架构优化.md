...
- [第二部分 · 实现原理（逐技术：算法与着色器）](#第二部分--实现原理逐技术算法与着色器)
  ...
  - [2.8 屏幕空间环境光遮蔽（SSAO / GTAO）](#28-屏幕空间环境光遮蔽ssao--gtao)
  - [2.9 RTReflectionPass —— 硬件光线追踪反射](#29-rtreflectionpass--硬件光线追踪反射)
  - [2.10 RTAOPass —— 硬件光线追踪环境光遮蔽](#210-rtaopass--硬件光线追踪环境光遮蔽)
  - [2.11 Lumen —— 虚拟化几何 GI（世界空间源）](#211-lumen--虚拟化几何-gi世界空间源)
  - [2.12 间接光在延迟光照中的集成（顺序总览）](#212-间接光在延迟光照中的集成顺序总览)
  - [2.13 技术对比总结](#213-技术对比总结)
  - [2.14 待实现技术](#214-待实现技术)
...
> ——**共 11 种**：IBL / SSGI / SSR / SSAO / GTAO / RSM / DDGI / RTGI / RT 反射 / RTAO / Lumen
> （完整索引与实现位置见 §2.1.1）——作为对照，
> 回答「所有 GI 技术背后共通的东西」。
...
HugEngine 的 GI 分**三套并行的体系**：**光栅化 GI 子系统**（`Engine/Render/GI/` 的 `GI_*` 类）、**硬件光追路径**（`Engine/Render/RT/` 的 `RTEffectPass`）、**虚拟化几何 GI（Lumen）**（`GI/LumenProvider.h` + `Engine/Render/Lumen/`）。三者在**帧图层**统一：每个源都包成 `IGIProvider`，由**注册表**（`GIRegistry`）按能力位与质量档位裁剪后形成通道层栈，合成端按 `GISourceId` 分派（§3.3、§3.5）。

### 2.1.1 本部分覆盖的全部 GI 源（实现原理索引）

以 `GISourceId`（`Engine/Render/GI/GITypes.h`）为权威口径，**已实现 11 个源**，另有 1 项已取消：

| `GISourceId` | 实现 | 本部分章节 |
|---|---|---|
| `IBL` = 1 | `GI/GI_IBL.cpp` | §2.2 |
| `Lightmap` = 2 | **未实现**（原任务 31 已取消，枚举内保留占位并注明） | — |
| `DDGI` = 3 | `GI/GI_DDGI.cpp` + `DDGITracePass` + `GIProbeGrid` + `GIRadianceHistory` | §2.6 |
| `SSGI` = 4 | `GI/GI_SSGI.cpp` | §2.3 |
| `SSR` = 5 | `GI/GI_SSR.cpp` | §2.4 |
| `SSAO` = 6 | `GI/AOProvider.h`（`ScreenAOProvider`）+ `PostProcess/SSAO.cpp` | §2.8 |
| `RSM` = 7 | `GI/GI_RSM.cpp` + `RSMIndirect` + `RSMFrustum` | §2.5 |
| `RTGI` = 8 | `RT/RTGIPass.cpp` | §2.7 |
| `RTReflection` = 9 | `RT/RTReflectionPass.cpp` | §2.9 |
| `RTAO` = 10 | `RT/RTAOPass.cpp` | §2.10 |
| `GTAO` = 11 | 与 SSAO 同一个 pass 的另一种模式（`useGTAO`） | §2.8 |
| `Lumen` = 12 | `GI/LumenProvider.h` + `Engine/Render/Lumen/` | §2.11（完整版见《08.Lumen实现分析》） |

> **RT 阴影**（`RT/RTShadowPass.cpp`）不在上表：它是**阴影技术**而非 GI 源（走独立的 `ShadowChannel` 枚举、不进 GI 层栈），实现见《12.阴影系统实现分析》。

### 2.1.2 历史接口：`GIMode` 与 `IGlobalIllumination`（仍在用，但已不是"源清单"）

光栅化的 `GI_*` 类仍继承统一接口 `IGlobalIllumination`（`Engine/Render/GI/GlobalIllumination.h`，该接口继承 `IRenderSubsystem`），其能力是：
...
> ⚠ **口径提醒（2026-09-22 复核）**：`GIMode` 是**先于**注册表架构的枚举，它**没有** SSAO / GTAO / RT 反射 / RTAO / Lumen 的位置（这些源在后来的 `GISourceId` 里才有编号），也没有独立的 SSR 项（这正是 `GI_SSR::GetMode()` 复用 `GIMode::SSGI` 的原因）。因此：
> - **列举"引擎有哪些 GI 源"必须用 `GISourceId`（§2.1.1 的表）**，不要用 `GIMode`；
> - §2.14 的"待实现技术"同样以 `GISourceId` 为准。

### 2.1.3 硬件光线追踪路径

独立于上述接口，位于 `Engine/Render/RT/`，统一继承 `RTEffectPass` 并由 `RTEffectProvider` 参数化装配：

- `RTGIPass` —— RT 间接漫反射（§2.7）；
- `RTReflectionPass` —— RT 反射（§2.9）；
- `RTAOPass` —— RT 环境光遮蔽（§2.10）；
- `RTShadowPass` —— RT 阴影（非 GI 源，见《12.阴影系统实现分析》）。

四者共用 `RT_HitCommon.slang::EvaluateHitRadiance` 与 `RTRayEffectPushConstant`（量纲约定见 §3.7.6）。

> 历史遗留的 `GIMode::VXGI`（体素锥追踪）与 `GIMode::ReSTIR` 仍无实现类：`Engine/Render/RT/ReSTIRPass.h:34` 实现的是 ReSTIR **直接光照（DI）** 时空重采样，不是 GI（详见 §2.14）。
...
## 2.8 屏幕空间环境光遮蔽（SSAO / GTAO）

**实现文件**：`Engine/Render/PostProcess/SSAO.{h,cpp}`（两种模式共用一套资源与 UBO）
**着色器**：`Engine/Shader/Shaders/GI/SSAO.frag.slang`、`GTAO.frag.slang`、`SSAO_Blur.frag.slang`（顶点着色器 `SSAO.vert.slang` 被本 pass 与 SSGI / SSR / SMAA 共用）
**Provider**：`Engine/Render/GI/AOProvider.h`（`ScreenAOProvider`，一个 Provider 同时声明 `Handles(SSAO)` 与 `Handles(GTAO)`）

### 2.8.1 原理：AO 是"间接项的可见性"，不是独立辐照度

环境光遮蔽估计半球可见性

$$
\mathrm{Vis}(N) \;=\; \frac{1}{\pi}\int_{\Omega} V(\omega)\,\cos\theta\,\mathrm{d}\omega
$$

关键性质：**它在合成端只乘间接项**（直接光与自发光不乘，见 §3.6.2 的通道合成），所以 AO 不是独立的辐照度估计量，而是**对其它漫反射源的调制**——这也解释了为什么它可以与 IBL / DDGI / Lumen 任意共存而不构成"重复估计"。

两种模式**互相替代**：SSAO 在半球内撒点、判断"是否被遮挡"（0/1 后平均）；GTAO 搜索地平线仰角后解析积分 ⇒ 同采样数下更准、噪点更少。层栈里同时启用会被 REDUNDANCY 判为**严格冗余**并可去重（§3.3.5）。

### 2.8.2 参数唯一出处

| 参数 | 默认值 | 位置 |
|---|---|---|
| 采样半径 `radius` | 1.0（世界尺度，着色器内再按视距缩放） | `PostProcess/SSAO.h:25` |
| 深度偏移 `bias` | 0.025 | `SSAO.h:26` |
| 强度 `intensity` | 1.0（**幂指数**，不是线性系数） | `SSAO.h:27` |
| 每像素采样数 `sampleCount`（SSAO 模式） | 16 | `SSAO.h:28` |
| 半分辨率 `halfRes` | false | `SSAO.h:29` |
| GTAO 开关 `useGTAO` | false | `SSAO.h:35` |
| GTAO 方位角切片数 `sliceCount` | 4 | `SSAO.h:36` |
| 采样核大小 `kKernelSize` | 64（对应 UBO 的 `u_Samples[64]`，1024 B） | `SSAO.h:121`、`SSAO.frag.slang:14` |
| 噪声纹理 | 4×4；SSAO 用 `uv * float2(320,180)` 放大采样以避免相邻像素同向 | `SSAO.cpp:98`、`SSAO.frag.slang:33-34` |
| GTAO 每侧步数 `kSteps` | 6（步长按步数递增：近密远疏） | `GTAO.frag.slang:89`、`:106-108` |

> **UBO 布局是两者共用的**：`u_Samples[64]` + `u_Params`（x=radius / y=bias / z=intensity / w=采样数或切片数）+ `u_InvProj` + `u_Proj`（`SSAO.frag.slang:13-18`、`GTAO.frag.slang:31-36`）——只有 `w` 的语义随模式切换，因此 C++ 侧只需一处填写。

### 2.8.3 SSAO 着色器算法

```hlsl
// SSAO.frag.slang:20-79 —— 半球撒点 + view-space 深度比较 + 距离范围检查
float depth = u_Depth.Sample(u_PointSampler, uv).r;
if (depth >= 1.0) return float4(1,1,1,1);                        // 天空：不遮蔽
float3 N = normalize(u_Normal.Sample(u_PointSampler, uv).rgb * 2.0 - 1.0);

// 随机旋转 TBN（避免所有像素用同一组采样方向而出现规则图案）
float3 randVec   = normalize(u_Noise.Sample(u_PointSampler, uv * float2(320.0,180.0)).rgb * 2.0 - 1.0);
float3 tangent   = normalize(randVec - N * dot(randVec, N));
float3x3 TBN     = float3x3(tangent, cross(N, tangent), N);

float scaledRadius = radius * max(abs(viewPos.z), 0.1) * 0.5;    // 按视距缩放
for (int i = 0; i < samples; i++) {
    float3 sPos = viewPos + mul(TBN, u_Samples[i].xyz) * scaledRadius;
    // 投影回屏幕 → 用该处深度重建 view 位置 geoPos
    if (abs(geoPos.z) < abs(sPos.z) - bias) {                    // 几何更近 ⇒ 采样点被遮挡
        occlusion += smoothstep(scaledRadius, 0.0, length(geoPos - viewPos));  // 距离范围检查
    }
}
occlusion /= float(samples);
float ao = pow(1.0 - occlusion, intensity);
```

三个要点：
1. **视距缩放**（`radius * |viewZ| * 0.5`）：固定世界半径在远处会覆盖过多屏幕像素，缩放后 AO 的视觉尺度才与画面一致；
2. **距离范围检查**（`smoothstep(scaledRadius, 0, dist)`）：只让半径内的几何贡献遮蔽，抑制远处大遮挡物造成的"黑边"；
3. **强度是幂指数**（`pow(ao, intensity)`），不是线性缩放。

### 2.8.4 GTAO 着色器算法（地平线切片 + 解析积分）

按 Jimenez 2016 *Practical Realtime Strategies for Accurate IBL*，三步（`GTAO.frag.slang:1-14` 的头注释就是算法说明）：

```hlsl
// (1) 每像素取 slices 个方位角方向，用噪声抖动抑制条带
const float phi = (float(s) + rand) * GTAO_PI / float(slices);
float3 T = normalize(sliceDir - N * dot(sliceDir, N));       // 切片平面内的切向

// (2) 沿 ±t 两侧步进：屏幕空间采样深度 → 重建几何点 → 取最大仰角余弦作为地平线
for (int step = 1; step <= kSteps; step++) {
    float3 sp = P + T * (sgn * (float(step) / float(kSteps)) * scaledRadius);   // 近密远疏
    if (!ProjectToUV(sp, suv)) break;
    float3 geoPos = ReconstructViewPos(suv, u_Depth.Sample(u_PointSampler, suv).r);
    cosHorizon[side] = max(cosHorizon[side], clamp(dot(geoPos - P, N) / length(geoPos - P), -1.0, 1.0));
}

// (3) 地平线角解析积分（含余弦权重与视线夹角 n）
...
float visibility = pow(saturate(1.0 - occlusion / GTAO_PI), max(intensity, 0.001));
```

- `IntegrateArc` 即 Jimenez 2016 的解析积分闭式解（`GTAO.frag.slang:60-64`）；
- `n = dot(normalize(-P), N)` 是视线与法线夹角的余弦（`:84`）；
- **归一化用 π 而非切片数**：每切片最多贡献 $\pi$ 的遮蔽量（`:130-132`）；
- 采样点落在天空或法线几乎平行切片方向时分别按"开阔"处理与跳过（`:99` 的 `tlen < 1e-5`、`:115` 的 `gDepth >= 1.0`）。

### 2.8.5 模糊与两趟离屏通道

AO 结果经 `SSAO_Blur` 模糊一次。**注意历史缺陷**：早期版本两趟共处同一通道、Blur 采样自己正在写的附件（attachment feedback loop，属未定义行为），实测同一构建两次运行 `hdr` 相差 5.8 万像素；现已改为"Pass 1 写原始 AO → 显式 RT→SRV 屏障 → Pass 2 读原始 AO 写最终 AO"（`PostProcess/SSAO.cpp:332-357`）。详见 §3.7.7 与《11.后处理链与抗锯齿实现分析》§7.3。

---

## 2.9 RTReflectionPass —— 硬件光线追踪反射

**实现文件**：`Engine/Render/RT/RTReflectionPass.{h,cpp}`
**着色器**：`Engine/Shader/Shaders/RayTracing/RT_Reflection.rgen.slang`（复用共享的 ClosestHit / Miss 与 `RT_HitCommon.slang`）

### 2.9.1 原理与回退边界

对 GBuffer 有效像素从法线出发、以 **GGX 微表面法线重要性采样**得到反射方向，发射**一次**反射光线：

- 命中 → 由 ClosestHit 评估命中点出射辐射度（与 RTGI 共用 `EvaluateHitRadiance`，量纲约定见 §3.7.6）；
- 未命中 → miss 着色器回退程序化天空色，并把 payload 的 `a` 置 `-1` 标记"未命中"；
- **粗糙度超过 `maxRoughness`（默认 0.6）时根本不发射射线**，直接回退 IBL prefilter 反射（`RT_Reflection.rgen.slang:88-91`）——因为高粗糙度下的单条射线毫无意义，而 IBL 的预滤波图正是为这一档准备的。

### 2.9.2 资源、分辨率与参数

| 项 | 值 | 位置 |
|---|---|---|
| 输出格式 | `RGBA16_FLOAT`（rgb = 反射颜色，a = 命中距离；-1 = 未命中） | `RTReflectionPass.cpp:112`、`RTReflectionPass.h:15` |
| 分辨率 | 默认**半分辨率**（可用 `HE_RT_FULLRES=1` 切全分辨率） | `RTReflectionPass.cpp:33-37`、`DeferredPipeline.cpp:242-247` |
| set0 绑定 | b0=TLAS、b1=输出、b2=GBuffer 深度、b3=GBufferB（法线+粗糙度） | `RT_Reflection.rgen.slang:13-16` |
| push constant | `RTRayEffectPushConstant`（C++/Slang 共享布局） | `RT_Reflection.rgen.slang:19` |
| `r.RT.Reflection.SPP` | 1（C++ 侧 clamp 到 1..16） | `RTQualityCVars.cpp:18`、`RTReflectionPass.cpp:164` |
| `r.RT.Reflection.MaxDistance` | 500.0 m | `RTQualityCVars.cpp:19` |
| `r.RT.Reflection.MaxRoughness` | 0.6 | `RTQualityCVars.cpp:26` |

### 2.9.3 Shader 算法

```hlsl
// RT_Reflection.rgen.slang:74-117
float depth = g_Depth.Load(int3(idx, 0)).r;
if (depth >= 1.0 || depth <= 0.0) { g_Output[idx] = float4(0,0,0,-1); return; }   // 天空像素无反射
float3 N = normalize(gbB.xyz * 2.0 - 1.0);
float roughness = gbB.w;

if (roughness > g_PC.maxRoughness) { g_Output[idx] = float4(0,0,0,-1); return; }  // 高粗糙 → 回退 IBL

float3 V = normalize(g_PC.cameraPos.xyz - worldPos);
float3 R = reflect(-V, N);                        // 镜面方向
if (roughness > 0.02) {                           // 粗糙时改为 GGX 重要性采样
    float3 H = SampleGGX(N, roughness, float2(Rand(idx, frame, 0), Rand(idx, frame, 1)));
    R = reflect(-V, H);
    if (dot(R, N) < 0.0) R = reflect(-V, N);      // 采样射入表面内 → 回退镜面，避免自交
}
RayDesc ray; ray.Origin = worldPos + N * 0.01; ray.Direction = R; ray.TMin = 0.01; ray.TMax = g_PC.maxDistance;
ReflectionPayload payload; payload.colorT = float4(0,0,0,-1);
TraceRay(g_TLAS, RAY_FLAG_NONE, 0xFF, 0, 0, 0, ray, payload);
g_Output[idx] = payload.colorT;
```

- `SampleGGX`（`:47-58`）与 IBL 预滤波的 `ImportanceSampleGGX`（`IBL_Prefilter.frag.slang:40`）同源实现（半向量重要性采样 + 切线空间变换）；
- 随机数用 PCG 风格整数哈希 + 像素/帧/采样号三元组播种（`:31-44`），**帧号参与播种** ⇒ 多帧累积时样本逐帧变化；
- ⚠ **`r.RT.Reflection.SPP` 当前是空开关（已完成复核）**：C++ 侧虽然设置 `pc.sampleCount = clamp(cvRTReflectionSPP, 1, 16)`（`RTReflectionPass.cpp:164`），但本 rgen **完全不读 `sampleCount`**——全文件 0 次命中，而 `RT_GI.rgen.slang` 与 `RT_AO.rgen.slang` 各有 1 次并在 rgen 内循环 ⇒ 本 pass 每像素恒定只发 **1 条**射线，改 SPP 不改变结果；
- **粗糙反射的噪声来源**：采样方向随 `frameIndex` 变化（`:41-44` 的播种含帧号），而本 pass **没有时域累积缓冲** ⇒ 粗糙表面的反射表现为**逐帧噪声**；若要压制，可参照 RTGI 的 `Temporal → Denoise → Upscale` 链（`RTProvider.h:67-73`）。

### 2.9.4 与 SSR 的关系

两者在**镜面层栈**里是竞争源：`GITypes.h` 的 `RTCounterpartOf` 把 SSR→RTReflection 登记为"同一物理量的两份估计"（供 REDUNDANCY 诊断），可用性由层栈与硬件光追能力共同决定（无光追设备经 `GIRegistry::Degrade` 裁剪后回退 SSR/IBL）。

---

## 2.10 RTAOPass —— 硬件光线追踪环境光遮蔽

**实现文件**：`Engine/Render/RT/RTAOPass.{h,cpp}`
**着色器**：`Engine/Shader/Shaders/RayTracing/RT_AO.rgen.slang`（复用 `RT_Shadow.rahit` 与 `RT_Common.rmiss`）

### 2.10.1 原理：只统计遮蔽半径内的几何

对 GBuffer 有效像素在法线半球做**余弦权重采样**，每个样本发一条射线；**命中即视为遮蔽**（AnyHit 把 payload 置 `blocked`）。关键约束是 `TMax` 取遮蔽半径（默认 2 m）⇒ **只有近处几何产生遮蔽**，等价于"局部 AO"，而不是整条视线的可见性。

### 2.10.2 资源与参数

| 项 | 值 | 位置 |
|---|---|---|
| 输出格式 | `R8_UNORM`（1 = 无遮蔽，0 = 全遮蔽） | `RTAOPass.cpp:102`、`RT_AO.rgen.slang:15` |
| 分辨率 | 默认半分辨率（同样受 `HE_RT_FULLRES` 控制） | `RTAOPass.cpp:34-35` |
| set0 绑定 | b0=TLAS、b1=输出、b2=深度、b3=GBufferB（法线） | `RT_AO.rgen.slang:14-17` |
| `r.RT.AO.SPP` | 2（C++ clamp 1..16；**shader 内再夹到 1..8**） | `RTQualityCVars.cpp:16`、`RTAOPass.cpp:142`、`RT_AO.rgen.slang:85` |
| `r.RT.AO.MaxDistance` | 2.0 m（遮蔽半径） | `RTQualityCVars.cpp:17` |

### 2.10.3 Shader 算法

```hlsl
// RT_AO.rgen.slang:72-108
if (depth >= 1.0 || depth <= 0.0) { g_Output[idx] = 1.0; return; }   // 天空：无遮蔽
uint n = max(1u, min(g_PC.sampleCount, 8u));                          // shader 内二次夹紧
uint occluded = 0;
for (uint s = 0; s < n; s++) {
    float3 dir = CosineSampleHemisphere(N, float2(Rand(idx, frame, s*2), Rand(idx, frame, s*2+1)));
    RayDesc ray; ray.Origin = worldPos + N * 0.02; ray.Direction = dir;
    ray.TMin = 0.02; ray.TMax = g_PC.maxDistance;                     // 仅遮蔽半径内命中
    ShadowPayload payload; payload.blocked = false;
    TraceRay(g_TLAS,
        RAY_FLAG_ACCEPT_FIRST_HIT_AND_END_SEARCH | RAY_FLAG_SKIP_CLOSEST_HIT_SHADER,  // 只要"是否命中"
        0xFF, 0, 0, 0, ray, payload);
    if (payload.blocked) occluded++;
}
g_Output[idx] = 1.0 - float(occluded) / float(n);                     // 遮蔽率 → AO
```

- 两个射线 flag 是关键：`ACCEPT_FIRST_HIT_AND_END_SEARCH` + `SKIP_CLOSEST_HIT_SHADER` ⇒ **AnyHit 即终止、不跑 ClosestHit**，把"是否遮挡"这一位信息做到最省；
- 法线偏移 0.02（`:93`）与 `TMin = 0.02`（`:95`）双重抑制自交；
- **SPP 在 rgen 内循环生效**（`n = min(g_PC.sampleCount, 8u)`，`:85`），与 RTGI 同口径；注意 **RT 反射不读 SPP**（空开关，§2.9.3）——三者并不一致。

### 2.10.4 与 SSAO / GTAO 的关系

同为 AO 源、互为替代：RTAO 不受屏幕空间信息缺失影响（屏幕外几何也能遮蔽），代价是需硬件光追与 TLAS 更新。`RTCounterpartOf` 把 SSAO/GTAO→RTAO 登记为 counterpart 对，REDUNDANCY 据此判断"是否同一物理量的重复估计"。

---

## 2.11 Lumen —— 虚拟化几何 GI（世界空间源）

**实现位置**：`Engine/Render/GI/LumenProvider.h` + `Engine/Render/Lumen/`（14 个文件：`LumenSDF` 123 KB、`LumenScene_SurfaceCache` 83 KB、`LumenScene_Irradiance` 32 KB 等）
**着色器**：`Engine/Shader/Shaders/Lumen/`（27 个 `.slang`，其中 23 个是编译入口）
**完整实现原理**：见姊妹文档《[08.Lumen实现分析.md](08.Lumen实现分析.md)》——本文不重复展开，只交代它在 GI 体系中的定位与四个构成件。

### 2.11.1 定位：单一 Provider 同时喂漫反射与镜面

Lumen 是 `IGIProvider` 实例，注册在 DDGI 之后、RT Provider 之前（`DeferredPipeline.cpp:210-223`），`GIPassKind::Offscreen`。它的**漫反射与镜面共用一张输出**，因此帧图必须按 Provider 遍历（不能按通道分别遍历）。

与屏幕空间源的关键差别：它是**世界空间源**——
- 产物与命中都在世界空间，**屏外仍可命中**；
- 因此**刻意不声明相机屏幕覆盖判据**（不做 CameraCoverage 置信度掩码），屏幕边缘不退化（依据见《Lumen设计与实现》§15.2）。

### 2.11.2 四个构成件（各一句）

| 构成件 | 作用 | 细节出处 |
|---|---|---|
| **Mesh SDF / Global SDF** | 每 mesh 128³ R16F 距离场（scatter + JFA 构建），再按相机合成两层全局 clipmap，供探针做 sphere tracing | 08 §3 |
| **Surface Cache** | 卡片软件光栅化的世界空间辐照度缓存：逻辑页 ≤1024、物理页 64（3 张 512² RGBA16F atlas、每页 64×64）、三段预算（捕获 8 / 分配 8 / 反馈 256） | 08 §4 |
| **Screen Probe gather** | 16×16 像素一个单元、2×2 单元平坦合并（阈值 `cos ≥ 0.995`），每探针沿 SDF 追踪 8 条光线（合法 8..16） | 08 §5 |
| **SH 投影与探针滤波** | 二阶 4 系数 SH；空间 YCoCg AABB 滤波 + 时域重投影（α 夹 [0, 0.98]）；逐像素 $L_o = \mathrm{albedo}/\pi \cdot E(n)$ | 08 §6 |
| **远场硬件光追** | 与 SDF 追踪**同一条光线**做逐光线对照，按距离阈值 50 m（重叠带 [40, 60]）用 `smoothstep` 混合 | 08 §7 |

### 2.11.3 与 DDGI 的关系：Radiance Cache 就是 DDGI

Lumen 的 Radiance Cache **由 DDGI 承担**：每帧把 DDGI 的输入换成 Screen Probe 的输出（`SetScreenProbeInput`，`GI_DDGI.h:47-58`；帧图接线 `DeferredPipeline_FrameGraph.cpp:740-751`），单侧系数按 $k = 2.0$ 换算到整球，距离门限为 1 个格距。因为 DDGI 段注册在 Lumen 段**之前**，所以探针读到的辐射度是**上一帧**的 ⇒ 这是 Lumen 在这里的主要降级来源之一（08 §8）。

> 与 §2.6 DDGI 的关系可一句话概括：**同一套探针网格与 SH 表示，输入从"自己的追踪"换成了 Lumen 的 Screen Probe 结果**。

---

## 2.12 间接光在延迟光照中的集成（顺序总览）
...
| SSAO / GTAO | 屏幕空间光栅化 | —（只调制间接项） | 屏幕内 | 每帧实时 | 屏外几何不参与遮蔽；SSAO 噪点较多 |
| RT 反射 | 硬件光追 | 1 次 | 场景全局 | 每帧实时 | 需硬件光追；粗糙度 > 0.6 回退 IBL |
| RTAO | 硬件光追 | —（只调制间接项） | 场景全局（限遮蔽半径内） | 每帧实时（SPP 靠时域累积） | 需硬件光追；半径外几何不计入 |
| Lumen | SDF 追踪 + 远场光追 | 多帧累积 | 场景全局（**世界空间**） | 每帧实时 | 无 screen trace；Surface Cache 单次捕获；SDF 分辨率限制 |

> 两点读表注意：
> - **SSAO / GTAO / RTAO 三行不是独立的辐照度估计量**，它们只对间接项做可见性调制（§2.8.1），因此可以与表中的任意漫反射源共存而不构成重复估计；
> - **Lumen 是唯一的世界空间源**（屏外仍可命中）⇒ 不参与屏幕覆盖置信度掩码，与 SSGI/SSR/光追三源的退化行为不同（§2.11.1）。

---

## 2.14 待实现技术

**口径声明**：本节按 **`GISourceId` + `GIRegistry`**（当前架构）判定，不再按遗留的 `GIMode`（见 §2.1.2 的口径提醒）。

**（1）枚举内已声明、但明确未实现**

- **`Lightmap`（烘焙光照）**：`GISourceId::Lightmap = 2` 保留占位，**原任务 31 已取消**（`GITypes.h` 的枚举注释即写明）；`ToPipelineCap` 有对应说明，且单元测试锁定"即使把它放进层栈，`GIRegistry::Degrade` 也会把它裁掉"。当前 Nanite 侧只落了 MRT7 的 `lightmapKey` 通道（`Nanite/NaniteRaster.h:167`），**没有任何着色器采样光图**。

**（2）路线图上有、但尚无实现类的**

- **VXGI（体素锥追踪）**：`GIMode::VXGI` 仅占位，无实现类。注意：全仓出现 "Voxel" 的地方**全部是 Lumen 的 128³ SDF 体素**，不是独立的体素 GI；
- **ReSTIR GI（间接光重采样）**：`GIMode::ReSTIR` 仅占位；已实现的 `ReSTIRPass` 只做**直接光照（DI）**的时空重采样（`RT/ReSTIRPass.h:34`），且默认关闭。

**（3）已实现、但离参考级还有距离的（不是"未实现"，但列在此处便于排期）**

| 技术 | 现状缺口 | 依据 |
|---|---|---|
| RT 反射 | **`SPP` 是空开关 + 无时域累积**：rgen 不读 `sampleCount`（恒定 1 条射线），采样方向随帧号变化而**没有累积缓冲** ⇒ 粗糙反射呈逐帧噪点；未装配 RTGI 那样的 `Temporal → Denoise → Upscale` 链 | §2.9.3 |
| RTAO | **没有时域降噪链**：rgen 内循环采样（SPP 生效，但被 shader 夹到 ≤8），出图后直接交给重建，未装配 RTGI 式降噪链 | §2.10.3 |
| RTAO | **SPP 双夹不一致**：C++ clamp 到 1..16（`RTAOPass.cpp:142`），rgen 内再夹到 1..8（`RT_AO.rgen.slang:85`）⇒ 设 12 与设 8 等价 | §2.10.2 |
| IBL 辐照度 | 仍是**均匀网格黎曼和**：对含太阳盘的天空会漏掉亮斑（太阳盘仅占半球约 0.14% 立体角 ⇒ 每 texel 期望仅约 5 个样本命中） | §2.2.3 |
| Lumen | 两处已记档降级：**无 screen trace**、**Surface Cache 单次捕获**（无多弹射反馈） | §2.11.2、《08.Lumen实现分析》§10 |
...
| 7 | **RT 反射** | `RT/RTReflectionPass.cpp` | GGX 重要性采样发射反射光线；粗糙度 > 0.6 回退 IBL 预滤波 | 1 次 | 每帧 | ✅ |
| 8 | **RTAO** | `RT/RTAOPass.cpp` | 余弦半球采样 N 条射线，AnyHit 置 `blocked`；`TMax` = 遮蔽半径 | —（调制间接项） | 每帧（SPP 靠时域累积） | ✅ |
| 9 | **SSAO / GTAO** | `PostProcess/SSAO.cpp` | 半球撒点环境光遮蔽（GTAO 为地平线切片 + 解析积分，同 pass 双模式）| —（调制间接项） | 每帧 | ✅ |
| 10 | **Lumen** | `GI/LumenProvider.h` + `Lumen/` | Mesh SDF + Surface Cache + Screen Probe + Radiance Cache（= DDGI） | 多帧累积 | 每帧 | ✅ 已接入层栈（两处已记档降级，见 §2.11）|
