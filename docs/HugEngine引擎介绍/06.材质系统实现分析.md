...
> [07.全局光照GI本质、实现与架构优化.md](07.全局光照GI本质、实现与架构优化.md)（IBL 烘焙与 split-sum）、
> [16.Entity-Component 架构与组件功能.md](16.Entity-Component%20架构与组件功能.md)（组件字段）、
> [02.架构UML与可扩展性分析.md](02.架构UML与可扩展性分析.md)（类关系）。
>
...
- 镜面：$(F_0 \cdot A + B) \cdot \mathrm{prefiltered}(R,\ \mathrm{roughness}\cdot(\mathrm{mips}-1))$（`:235-237`）；A/B 的来历是 Schlick 拆成
  ...
  （`IBL_BRDF_LUT.frag.slang:61-86`，512² RG16F；Smith 几何项在 `:48-59`）。
...
### 6.13 IBL BRDF LUT 的 `k` 多平方一次 —— 已修复

`IBL_BRDF_LUT.frag.slang` 原为 $a = \mathrm{roughness}^2;\ k = a^2 \cdot 0.5$ ⇒ $k = \mathrm{roughness}^4/2$，而 Karis/UE4 的
IBL 惯例是 $k = a/2 = \mathrm{roughness}^2/2$ ⇒ 高粗糙下 Smith 遮蔽偏小 ⇒ **间接高光偏亮**。
（直接光路径的 $k=(r+1)^2/8$，`pbr_common.slang:53`，是正确的，两者不可混用。）

**2026-09 画质阶段 0 已按后者修正**（`IBL_BRDF_LUT.frag.slang` 的 `G_Smith`：`float a = roughness*roughness; float k = a * 0.5;`，
并留了推导注释）。验证口径与实测：
- **解析复算**：按 LUT 的积分式独立用 Python 重算，$k$ 修正前后 —— $\mathrm{roughness}=1$ 时**完全不变**（$a=1$，两次平方无差别）；
  $\mathrm{NdotV}=0.5,\ \mathrm{roughness}=0.5$ 时 A 项 +16.6%、B 项 +27.0%，方向与"高粗糙间接高光偏亮"一致（修正后更暗）。
- **引擎实测**（同一构建、同一配置，仅换这一处）：`hdr` 有 4,922,176 个元素变化（占 59.3%），**94.8% 变暗**，
  带符号均值 $-1.97\times10^{-3}$（仅统计变化像素 $-3.32\times10^{-3}$），约为噪声底的 1700 倍；
  亮度 >1.0 的像素不变（高光峰值无回归）；其余 17 个转储逐位不变 —— 与"该 LUT 只被 `DeferredLighting.frag.slang:279`
  与 `PBR.frag.slang:324` 在间接高光路径上消费"的接线事实一致。
- LUT 为**运行时烘焙、无磁盘缓存**（`GI_IBL.cpp:88-98`），所以修改不需要清理任何缓存文件。

该疑点此前已记录于 [GI 本质、实现与架构优化](07.全局光照GI本质、实现与架构优化.md) §2.2.3（IBL 的 Shader 算法细节），并在 §4.2 各技术现状要点里复核。
...
| `05.渲染管线实现分析.md` 的"文档/代码漂移"表 | 该表记录的是 **`DeferredPipeline.h` 头部注释**过时（"GBuffer 5×MRT" 实际 8 MRT），不是姊妹文档自身写错 |
...
| 18 | P3 | ~~IBL LUT $k = \mathrm{roughness}^4/2$~~ **已修复** | `IBL_BRDF_LUT.frag.slang` 的 `G_Smith` | 高粗糙间接高光偏亮（§6.13） |
...
| # | 项 | 改法 |
|---|---|---|
| P3-18 | IBL LUT 的 $k$ ✅ **已做（2026-09 画质阶段 0）** | `IBL_BRDF_LUT.frag.slang` 的 `G_Smith` 改为 `k = a * 0.5`（$a = \mathrm{roughness}^2$）并补推导注释。**验证**：解析复算（Python 独立重算积分）+ 引擎差分实测（`hdr` 4.92M 元素变化、94.8% 变暗、>1.0 的像素不变），见 §6.13 |
