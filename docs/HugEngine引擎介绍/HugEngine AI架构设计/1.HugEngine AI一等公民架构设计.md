...
> 详细设计见两份分册：`2.HugEngine AI统一基座设计.md`（方案 1）、`4.HugEngine AIGC创作平台设计.md`（方案 4）。
...
> 📌 现状更新（2026-09）：本文撰写时"代码为零"；AI 一等公民已按本文落地为 `Engine/AI/` 模块（推理运行时 / WorldModel / AIGC / Agent / Neural / GPU 后端），详见 `AI与反射功能配合分析.md` 的现状评估。
...
| 修改 | `Engine/Core/Core/Engine.h`（持有 `IAIDevice`——**实际未采纳**：改由 `Engine/AI/Runtime/AIModule.h` 进程级单例承载，避免 L0 Core 反向依赖 AI）、`Engine/Core/Threading/JobSystem.h`（流式/优先级车道——**实际未采纳**：由 `he::ai::InferenceScheduler` 自建线程实现）、`Engine/Reflect/Reflect/Attribute.h` + `ReflectionMacros.h`（`HE_ATTR_AI_*`）、`Engine/Scene/Scene/SceneReflect.cpp`（注册 AI 组件——**实际改在** `Engine/AI/Agent/AgentReflect.cpp`）、`Engine/Render/**`（神经渲染改为走 `IAIDevice`，当前仅 `NeuralUpscaler` 一例）、`Engine/Editor/**`（AI 面板——**未落地**，演示窗口在 `Samples/07.AISamples`） |
...
- 方案 1 详细设计与实现原理：`2.HugEngine AI统一基座设计.md`
- 方案 4 详细设计与实现原理：`4.HugEngine AIGC创作平台设计.md`
- 完整规格：`3.HugEngine AI统一基座设计规格.md`、`5.HugEngine AIGC创作平台设计规格.md`
