...
当前 HugEngine 中，AI 以两种"事后集成"的形态存在：
...
> 📌 现状更新（2026-09）：本文撰写时"代码层面为零"（`grep -r "Neural|Inference|Agent|LLM|AI" Engine/` 无结果）；此后已按本 spec 落地 `Engine/AI/` 模块（推理运行时 / WorldModel / AIGC / Agent / Neural），详见 `AI与反射功能配合分析.md` 的现状评估。下文未标注"规划"的接口段落已按实际代码校正。
...
| `rhi::DeviceCaps` 已声明 `supportsCooperativeVectors` / `supportsLinearAlgebra` | `Engine/RHI/RHI/Types.h:288` | 设计上用于 GPU 推理能力探测；**当前 `he::ai` 未读取 `DeviceCaps`**（`GPUBackend` 用普通 compute 核），能力位仅"已预埋" |
...
│   ├── AIModule.h/.cpp          # 进程级单例入口
│   ├── InferenceScheduler.h/.cpp# 优先级车道 + 主线程流式回调投递
│   └── Backend/
│       ├── IAIBackend.h         # 后端接口 + 共享基础类型
│       ├── GPUBackend.h/.cpp    # RHI compute（内置核 + 运行时 SPIR-V 核）
│       └── RemoteBackend.h/.cpp # LLM HTTP（经 DeepSeekClient）
│   # ※ CPUBackend.h/.cpp（ONNX Runtime / GGUF）为规划项，未落地
├── WorldModel/
...
│   └── AgentSystem.h/.cpp       # 节律驱动
├── Neural/
│   └── NeuralUpscaler.h/.cpp    # 首个神经子系统（IRenderSubsystem）
│   # ※ NeuralRadianceCache / NeuralMaterial / RTXNeuralShader 为规划项，未落地
└── AIGC/                        # 应用层（见 5 号 spec）
```
...
// 推理请求（基础类型定义在 IAIBackend.h，AIDevice.h 仅转包含）
struct InferenceRequest {
    IAIModel*               model = nullptr;    // 编译后的模型句柄（预置核可空）
    // 计算核选择（二选一）
    String                  kernel;             // 内置核名，如 "tensor_scale"/"texture_sample"/"texture_gen"
    Span<const u32>         customSpirv = {};   // 运行时自定义 SPIR-V（优先于 kernel）
...
public:
    ...
    virtual Ref<IAITensor> WrapRHITexture(rhi::IRHITexture* tex) = 0;   // 读渲染纹理（已实现）
    virtual Ref<IAITensor> WrapRHIBuffer(rhi::IRHIBuffer* buf)  = 0;    // 读 bindless 缓冲（当前为占位）
    virtual rhi::IRHIBuffer* ExportBuffer(IAITensor* t)          = 0;   // 写回渲染缓冲（已实现）
...
**后端**：`IAIBackend` 是 `IAIDevice` 的实现单元，`IAIDevice` 作为门面按模型格式 + 能力选择后端。工厂 `CreateAIDevice(InferenceScheduler*, rhi::IRHIDevice*)` 当前只注册 **RemoteBackend**（需环境变量 `DEEPSEEK_API_KEY`）与 **GPUBackend**（需 RHI 设备）。
...
**流式调度器**（`InferenceScheduler`，自建工作线程 + 优先级队列，不依赖 `JobSystem`）：
- **优先级车道**：`InferencePriority::Frame`（渲染耦合推理，如超分）> `Normal` > `Background`（LLM 智能体），互不阻塞；
- **流式回调**：经 `PostToMain()` 把 token 按序投递到主线程队列，主线程每帧 `DrainMainThread()` 执行。
...
struct ObservationFilter {
    float        radius = -1.0f;          // 以 center 为球的空间范围（-1 = 全场景；当前暂未启用）
    float3       center = {0,0,0};        // 空间过滤中心（当前暂未启用）
...
public:
    ...
    // 依据「语义快照 JSON」+ 记忆，产出动作计划
    // 注意：观察以 JSON 文本传入（非 Observation 对象），记忆以组件指针传入（可空）
...
// 在 Engine/AI/Agent/AgentReflect.cpp 注册（不放 Scene 的 SceneReflect.cpp，避免 Scene → AI 反向依赖）
...
// 记忆组件（当前仅短期 KV，上限 64 条；长期/情景记忆为后续扩展）
class MemoryComponent : public he::Component { /* AddShortTerm / Query / GetShortTermCount / ClearShortTerm */ };
// 目标组件（目标列表按优先级降序 + 达成标记）
class GoalComponent : public he::Component { /* AddGoal / MarkAchieved / GetCurrentGoal / GetGoals */ };
}
```
...
**落地现状**：目前只有 `NeuralUpscaler` 一个子系统（`Engine/AI/Neural/`），且目标是"验证架构形态"（静态渐变输入纹理 + 每帧采样推理统计），真实超分需待 Streamline SDK 接入；`NeuralRadianceCache`/`NeuralMaterial`/`RTXNeuralShader` 均未落地。零拷贝侧 `WrapRHITexture`/`ExportBuffer` 已实现，`WrapRHIBuffer` 仍为占位。
...
| `Engine/Editor/Editor/Command.h` | （复用 `Command/CommandHistory/PropertyChangeCommand`；可选新增 `CompositeCommand` 打包批动作，未实现） | 可选 |
...
- **后端不可用**：`LoadModel`/`CreateTensor`/`Submit` 返回 `nullptr` 并 `HE_CORE_WARN` 记录（当前未定义 `AIBackendUnavailable` 异常）；调用方按能力降级（GPU→CPU→Remote→功能禁用）。
- **设备能力不足**（无 CoopVec/linalg）：`GPUBackend` 退化为普通 compute dispatch，或转交 `CPUBackend`（**降级链为规划项，尚未实现**；`CPUBackend` 本身也未落地）。
- **远程 LLM 失败**（网络/限流/超时）：`Chat`/`ChatStream` 返回空串并记录错误（`DeepSeekClient::Chat` 内部 try/catch 兜底）；设计中的 `IAIInference::Failed` 状态与 `onError` 回调、Agent 侧退避重试尚未实现。
...
1. `Editor` 例程启动后，AI 观察面板能对当前 `World` 生成 `Snapshot`，且字段来自 `HE_ATTR_AI_VISIBLE`（面板未落地；现由 `WorldModel` 单测 + `07.AISamples` 覆盖）。
2. 一个 Mock Brain 驱动 `AgentComponent` 完成一次 `SpawnEntity` 并经 `CommandHistory.Undo()` 完整回滚。
3. `CPUBackend` 加载一个 ONNX 模型跑通推理（未落地）；`RemoteBackend` 对接一个真实 LLM 返回结果（当前 `ChatStream` 为"取回完整响应后按块回调"，非真实 SSE 增量流式）。
4. 神经渲染模块（`NeuralUpscaler`）通过 `IAIDevice`（而非直接调 SDK）完成一次推理并写入渲染资源。
