...
（当时抖动族 `hdr` / `prov0_ao_*` 仍会逐对变化，且已独立验证与 Nanite 无关）。

> **2026-09 画质阶段 0 更新**：这条限定**已不再需要**。抖动族的两处根因都已定位并修复 ——
> ① SSAO 与 Blur 共处同一离屏通道、Blur 采样自己正在写的附件（attachment feedback loop，UB）；
> ② 帧图 `lightingReads` 漏声明 AO 输出，`Lighting` 在 AO 纹理仍处于 `COLOR_ATTACHMENT` 布局时采样它。
> 修复后同一构建、同一配置连跑（OFF 3 趟 / ON 2 趟）**全部 18 个转储逐位一致，且与修复前的多数派结果相同**
> （纯同步修复，不改变稳态画面）⇒ 现在**整帧逐位可复现**，抖动族只剩历史上限的口径，不再是实际现象。
> 详见 `11.后处理链与抗锯齿实现分析.md` §7.3 与 `07.全局光照GI本质、实现与架构优化.md` §3.7.7。
