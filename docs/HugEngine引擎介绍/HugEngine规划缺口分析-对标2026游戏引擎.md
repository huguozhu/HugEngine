...
> **定位**: 审视《HugEngine技术全景与实施计划》(350+ 项) + 《HugEngine架构UML与可扩展性分析》 + 《HugEngine AI架构设计/》系列，对照 2025-2026 年最新商用引擎（UE5.5/5.6、Unity 6、Godot 4.x）与未来趋势，找出**规划层面**的缺失。
>
> **结论**: HugEngine 的规划是一份非常完整的"渲染引擎"规划（甚至超前，覆盖 SIGGRAPH 2025 / SM 6.10 / RTX Kit / 3DGS），且**渲染侧的三大规划已不再是纸面项**——Nanite 虚拟几何、Lumen 式 GI、GI 分层合成架构均已落地并归档为《已实现功能》；但距离"游戏引擎"架构仍缺一层——音频、运行时 UI、输入抽象、NPC 决策/感知、通用调试渲染原语五项未立项（物理、角色移动与碰撞/射线查询、导航寻路、骨骼网格与动画重定向、通用玩法组件族已于 2026-09 落地，见第 2 节）。同时渲染层自身仍有已商用/将商用的空白，另有一批**有代码证据的架构债务**（Vulkan 泄漏、Scene↔RHI 耦合、序列化半残，见 §6.1 与《架构UML与可扩展性分析》§17）。AI 一等公民规划是亮点，其运行时层（`Engine/AI`）与物理层（`Engine/Physics`，JoltPhysics）已于 2026-09 落地，但配套的安全/测试/成本仍然缺失。
>
...
| 横切架构（内存预算/时间/资源服务） | 仍缺位（时间系统 / AssetManager / GPU 内存预算 / 确定性） | ⚠️ 单点特性无法替代；相关"有代码证据的债务"见 §6.1 |
...
### 6.1 有代码证据的架构债务（不是"缺特性"，是"已实现的代价"）
...
- [ ] **后端抽象纪律**：清掉 `GI_RSM` 的 Vulkan 死依赖与 `ImGuiIntegration` 的 `VulkanDeviceAccess`，把 RHI 的 `void*` 逃逸口逐个换成强类型抽象——否则 D3D12/Metal 无从落地（证据见《架构UML与可扩展性分析》§17 P0-1）
- [ ] **Scene 组件与 GPU 资源解耦**：组件只持 CPU 数据 + 句柄，GPU 缓冲上移到 Render/Asset —— 顺带收敛"逐类型 ForEach"的根因（§17 P0-2）
- [ ] **序列化工厂接线**：`SceneSerializer::Load` 改用 `ClassInfo::factory`，3 份类型分派表收敛到一处并补齐 `u8` 等已注册类型 —— 成本最低、见效最快（§17 P0-3）
...
- [ ] 动画状态机（AnimGraph）/ BlendSpace / IK / 物理动画（**骨骼网格播放 / 剪辑混合 / 动画重定向已落地**，见 §3 第 6 项）
- [ ] Sequencer 过场系统
...
- [x] 崩溃处理（`Engine/Core/CrashHandler`：DbgHelp 符号化 + minidump）**已落地**；[ ] 遥测采集 / GPU fault 与设备丢失诊断仍缺
- [ ] AI 配套：安全边界（prompt injection / 资源爆炸防护）、AI 渲染级回归、模型与成本管理
- [ ] **性能验收阶段化**：为"已落地但未达标"的特性（如 Lumen L6 1080p/60fps 暂停项）设独立阶段与判据，避免以"能跑但不够快"累积
...
> 注：表中"架构规划文档"指承担原《HugEngine架构设计与任务划分》职责的文档——该文档（145 模块 WBS / 依赖拓扑 / 测试策略 / 风险 / MVP 裁剪 / 人力配置）已于 2026-09 删除。**现状**：《HugEngine架构UML文档》与《HugEngine架构可扩展性分析》已合并为 `docs/HugEngine引擎介绍/HugEngine架构UML与可扩展性分析.md`（架构总览 + 分层类图 + 关键时序 + 可扩展性评估/代码级债务），可承接"架构"一侧的引用（§6.1 即引用其 §17）；**WBS / 测试策略 / 风险登记 / 人力配置仍无承接文档**。
...
- `docs/HugEngine引擎介绍/HugEngine架构UML与可扩展性分析.md`（架构总览 / 分层类图与关键时序 / 可扩展性评估与代码级债务清单 —— §6.1 与 P0 债务项的证据来源）
