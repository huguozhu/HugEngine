...
> 基于 `Engine/Render/Pipeline` 三个现役管线 + 已删除的 `HybridRTPipeline`（历史记录）全部源码的逐行分析（2026-08-19），
> **补注（2026-09-19）**：`HybridRTPipeline` 已删除（RT 效果并入 Deferred 层栈），现役管线为
> Forward / Deferred / PathTracing 三条；本文第 6 节保留为**历史记录**，其余章节里涉及该类的
> 表述已就地标注。
> **补注（2026-09-21 复核）**：渲染层另有两个**不实现 `IRenderPipeline`** 的独立模块——
> `Engine/Render/Lumen/`（Lumen 式 GI：Surface Cache / SDF / Screen Probe，经 `LumenProvider` 注册进
> `DeferredPipeline` 的 GI 层栈）与 `Engine/Render/Nanite/`（Nanite 虚拟几何：`NaniteRenderer` 门面 +
> `NaniteCull` / `NaniteRaster` / `NaniteUpload` / `NaniteScene`，由 `DeferredPipeline` 驱动，
> 默认 `r.Nanite.Enable=0` 关闭）；`IRenderSubsystem` 侧另有 `Engine/AI/Neural/NeuralUpscaler`。
> 本文只覆盖 `Engine/Render/Pipeline`，上述模块的逐条说明见
> [HugEngine技术名词全目录.md](HugEngine技术名词全目录.md)。
> **行号说明（2026-09-21）**：原文所有 `文件:行号` 引用取自 2026-08-19 快照，此后源码大幅变动
> （ForwardPipeline.cpp 1009→1233 行、ForwardPipeline_FrameGraph.cpp 226→249 行、
...
|---|---|---|---|---|
| 源码规模（2026-09-21 复核） | .cpp 1233 行 + FG 249 行 | .cpp 811 行 + FG 1761 行 | 已删除（原 .cpp 952 行） | .cpp 579 行 |
...
3. **无 RT 设备回退**（`02.Cube.cpp`）：ImGui Combo 按 `supportsRayTracing` 收缩选项；mode≥2 且无 RT 时强制 `mode=1`（Deferred）并写回 CVar。
4. ImGui Combo 实时改写 CVar（`02.Cube.cpp`）。

---

## 3. 公共基础设施

### 3.1 IRenderPipeline 接口（IRenderPipeline.h）
...
**Compile 实际六步**（RenderGraph.cpp，注意头文件注释顺序与实际不一致）：
...
**ResourceState 转换表**（RenderGraph.cpp）：
...
**Execute 双路径**（RenderGraph.cpp）：帧首 `AdvanceTransientResources()` → 创建/导入资源（别名资源走瞬态）→
...
```
SceneRenderer::Prepare（SceneRenderer.cpp）
...
| 0 | 24-27 | RT_ShadowMask/RT_Reflection/RT_AO/RT_GI | RT 效果纹理（现由 Deferred 的 RT 源承载并真实绑定） |
| 0 | 25 | RectShadow | 矩形面光阴影（独立 binding） |
| 0 | 28/29 | GBufferF/G | disneyA/B MRT5/6 |
| 0 | 30 | u_SSBO[] / u_Materials[] / u_LightmapKey | per-frame set：bindless SSBO（最高 binding 承载 VARIABLE_COUNT）；Deferred Lighting set：光照图键（MRT7） |
| 0 | 31 | u_GIBlendParams | GI 各通道分层合成参数 UBO（3 通道 × 源数组） |
| 0 | 32 | u_Lumen | Lumen 输出（RGBA16_FLOAT，骨架阶段漫反射与镜面共用一张） |
| 2 | — | kGPUDescSet_Bindless | TLAS 等无绑定资源 |

> 注：同一 binding 号在不同描述符集里含义不同——24 在 Deferred Lighting set 是 RT 阴影遮罩、在 per-frame set 是 SpotShadow；25 在 Deferred Lighting set 是 RT 反射、在 per-frame set 是 RectShadow；30 在 per-frame set 是 bindless SSBO、在 Deferred Lighting set 是 LightmapKey。（2026-09-21 复核补入）

**Push Constant 结构**（static_assert 校验于 Material.h）：

| 结构 | 大小 | 关键字段 |
|---|---|---|
| PushConstantData（Forward） | 160B | viewProjMatrix、cameraPosition、lightCount、objectIndex、useInstanceID、iblIntensity、cluster 参数、useClustered、useBindlessMaterial、atmosphere、instanceSSBOHandle、instanceVisibleHandle、instancePad[2] |
...
`GPUObjectData` 208B（worldMatrix+内联材质+materialID+dielectricF0+disneyA/B/C）、
...
`DispatchCompute` 六步（ParticleRenderer.cpp）：**回读上帧 renderCount 缓存** →
...
- `PrecompileQueuePSO` 全引擎仅 **SSAO.cpp 三处**注册（Compute/图形 PSO 变体）；
- `StartPSOPrecompile()` 在 `DeferredPipeline.cpp` **被注释禁用**——本机 Intel Arc B370 驱动
  igc-default64.dll 预编译 worker 线程约 50% 概率 SIGSEGV；
- 帧内限流 `EnqueuePSOCreate` 仅 `DeferredPipeline.cpp` 的 GPL 变体演示使用，`NextFrame` 每帧
...
- **三缓冲 ×4**：ObjectBuffers（208B×1024）、LightBuffers（64B×8）、ShadowBuffers（256B×4）、
...
- PBR PSO：push constant 0..160B、depthTest LessEqual D32、单附件 RGBA16_FLOAT、仅 set=0；
- Secondary CB 池：`min(kMaxSecRecordLists=8, JobSystem::GetThreadCount())`；
- **PSO 注册表坑**（ForwardPipeline.cpp）：push_back 后必须重定位 desc.vertexShader/pixelShader
...
### 4.3 多线程命令录制（MTCR，ForwardPipeline.cpp）
...
- 每 worker：池内**专属** Secondary CB → `BeginSecondary(m_PBR_PSO)`（继承主 RenderPass）→
  SetViewport（Y 翻转）/SetScissor → 每 draw：BindDescriptorSet + DebugLabel + SetPushConstants(160B) +
  VB/IB + DrawIndexed（**每 draw 重绑描述符集**，无惰性绑定）；
- `JobSystem::ParallelInvoke` 等待全部 → 主 CB 按序 `ExecuteSecondary` 合并。各 worker 独占 CB 无锁。

### 4.4 材质 bindless 上传（ForwardPipeline.cpp）
...
5. shader 分支（PBR.frag.slang）：`useBindlessMaterial != 0` 读
...
| # | 坑 | 位置 |
|---|---|---|
| 1 | `m_UseRenderGraph` 实际默认 **true**，注释写"默认关闭"；所有 sample 显式置 false | ForwardPipeline.h |
| 2 | RG 路径缺失 AA pass：m_AntiAliasing 仅非 RG 路径执行 | ForwardPipeline.cpp |
| 3 | 非 RG 路径 ToneMap 在管线外（宿主执行） | 02.Cube.cpp |
| 4 | PSO 注册表 push_back 后指针悬空坑 | ForwardPipeline.cpp |
| 5 | 旧 PSO 延迟 3 帧销毁（GPU 可能仍用其 VkRenderPass） | ForwardPipeline.cpp |
| 6 | PCF 纹素大小硬编码 1/2048 | pbr_common.slang |
| 7 | CollectLights 逐光源一次 Map/Unmap | ForwardPipeline.cpp |
| 8 | 无光源时注入默认方向光（强度 5，shadowIndex=-1） | ForwardPipeline.cpp |
| 9 | `m_UseBindlessMaterial` 无 setter：bindless 材质路径代码完整但不可运行时开启 | ForwardPipeline.h |
| 10 | 物理光照编码 hack：`positionRange.w<0` 标记物理模式（lux/cd + 平方反比） | PBR.frag.slang |
| 11 | VS 与 FS 声明的是**同一份 160B** push constant 块（`viewProjMatrix` → `instancePad[2]`）——VS 用不到的字段（`iblIntensity`、cluster 参数、`useBindlessMaterial`、`atmosphere`、实例句柄）在 VS 块里也占位声明并注明"VS 不使用，仅占位对齐 FS"，两侧布局完全一致 | PBR.vert.slang, PBR.frag.slang |
...
### 5.2 初始化流程（DeferredPipeline.cpp，严格顺序）
...
复用 GBuffer set=0 layout）→ **DGC 初始化**（supportsDGC → `InitializeDGC(GBuffer PSO, GPUCulling::kMaxObjects, GPUCulling::kMaxObjects)`，即 2048/2048，两者同上限）→
...
| 4 | GB_Clear | Graphics | — | 8 MRT + gbDepth | DGC 上下文注入（§5.4） |
...
| 7 | LightmapKey：uv0.xy + objectIndex（光照图键，页号 = objectIndex） | RGBA16_FLOAT |
...
- 输入源：GBuffer 0-3/23/28-29/30（LightmapKey） + 阴影 4/10/11/9 + SSBO 17/18/22 + 聚集 7/8 +
  屏幕空间 19/20/21 + RT 24-27 + IBL 12-14 + RSM 间接光 5 + DDGI 网格参数 6 + GI 分层合成参数 31 +
  Lumen 输出 32；
...
- RT 效果 4 张纹理在 RT 源产出时由 Deferred 真实传入（`rtShadowMask` / `rtAO` / `rtReflection` / `rtGI`），未产出才保持 nullptr 走占位。

### 5.8 CollectLights（DeferredPipeline.cpp）
...
| # | 坑 | 位置 |
|---|---|---|
| 1 | **ClusteredShading 死路径**：grid/index 缓冲未分配，useClustered 恒 0，最多 8 光源线性渲染 | DeferredPipeline.h |
| 2 | **Phase2 的 Hi-Z 绑定被覆盖**：BuildHiZPyramid 设为 Hi-Z，GPU_Cull_Phase2 lambda 又改回全分辨率深度——金字塔构建纯属浪费 | DeferredPipeline_FrameGraph.cpp |
| 3 | **一帧错位风险**：Readback 帧首读上帧 count，而 IndirectCmdBuf 帧中被本帧覆盖；可见数减少时会多绘未初始化 command 条目 | GBufferRenderer_GPU.cpp |
| 4 | Intel Arc B370：PSO 预编译 worker 在 igc-default64.dll ~50% 概率 SIGSEGV → StartPSOPrecompile 注释禁用 | DeferredPipeline.cpp |
| 5 | Dispatch 后 `m_LastVisibleCount=0` 三处，全部依赖下次 Readback | GPUCulling.cpp |
...
| 13 | CPU 模式"jitteredVP"注释与实际不符：直接取 camera.GetViewProjMatrix() | GBufferRenderer_CPU.cpp |
| 14 | HDR 模式（A2B10G10R10）强制关闭 FXAA/SMAA/ColorGrading/CameraEffects（TAA 不受影响） | DeferredPipeline_FrameGraph.cpp |
| 15 | DGC 的 32B DGCDrawToken 死代码 | MeshBatcher.h |
...
### 6.2 初始化与降级链（原 `HybridRTPipeline.cpp`；现同在 `DeferredPipeline::Initialize`）
...
- **场景材质纹理 4×N RGBA32F**（BuildSceneMaterialTexture，RTPass.cpp）：
...
- **设计决策**（RTPass.h 注释）：必须用纹理而非 SSBO——slangc 在 ClosestHitKHR 访问
...
- **RTGIPass**：miss 时 `SampleDDGI(worldPos, dir)`（4 系数（band 0/1）SH 辐照度 + 8 探针三线性插值；探针网格默认**按场景包围盒自动拟合**（`GIProbeGrid` 的 `FitGridToBounds`），
  `8×4×8` / `cellSize=3.0` 只是自动拟合不可用时的**回退默认值**；每探针 4×float4 = 64 B）替代纯天空色；hit 取 ClosestHit 辐射度。
...
| 8 | kRTMaxPayloadSize=16 与 RTPass.h 注释矛盾（注释说 Reflection/GI 需要 32B，实际全 16B） |
...
### 7.2 初始化（PathTracingPipeline.cpp）
...
### 7.3 帧流程与 Pass 表（PathTracingPipeline.cpp）
...
- **三 dispatch 同一 RG Pass 的原因**（PathTracingPipeline.cpp 注释）：SSBO 对 RenderGraph
...
| 文档漂移 | m_UseRenderGraph 默认值与注释矛盾；RenderGraph 头注释顺序与实际不符；"5×MRT" 实际 8 张；payload 注释 32B 实际 16B | 全部 | 低 |
| 性能 | CollectLights 逐光源 Map/Unmap；多处 CPU-GPU 同步点 | Forward/Deferred | 低 |

---

*本文档由 5 个并行分析代理通读当时的四个管线全部源码生成，含 60+ 条实现细节与坑位记录。*
*（2026-09-19 补注：HybridRTPipeline 已删除，现役三条管线；第 6 节为历史记录。）*
*（2026-09-21 复核：源码规模与 GBuffer/绑定表按当前源码更正；行号引用统一改为不带行号的写法；补入 Lumen / Nanite 模块说明。）*
