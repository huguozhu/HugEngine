...
> 本文从数学、计算、工程三个层面阐述「实现 GI 的本质是什么」，并以 HugEngine 中已实现的 GI 技术（本文取 IBL / SSGI / SSR / RSM / DDGI / RTGI 六种作对照；此外 Lumen 亦已落地，见 [Lumen设计与实现.md](../已实现功能/Lumen设计与实现.md)）作为对照，回答「所有 GI 技术背后共通的东西」。与 [HugEngine全局光照GI实现原理.md](./HugEngine全局光照GI实现原理.md) 互为姊妹篇：后者讲「怎么实现」，本文讲「实现的是什么」。
...
| **降维** | 把 5D 光场压缩：SH 是角度降维，irradiance 把方向积分掉 | DDGI 的二阶 4 系数球谐（band 0/1）|
