...
>
> **本文件已归档到 `docs/已实现功能/`**（原在 `docs/计划实现功能/`）：步骤 1–36 与 38–41 已实现
...
**实现后的实测对照（步骤 29 的 `LumenScene::LogMemoryUsage()` 自报，1920×1080，2026-09-19）**：

| 资源 | 预算（上表） | 实测 | 说明 |
| --- | --- | --- | --- |
| Mesh SDF | ≈ 420 MB（100 mesh × 4.2 MB） | **404.00 MB**（101 个） | 与预算吻合；是 Lumen 显存的主体，也是帧时里最大的那一项（见下） |
| Global SDF clipmap | ≈ 134 MB（4 × 256³ R16F） | **32.00 MB**（2 层 × (R32U + R32F) 128³） | 首版只有 2 层、128³；分层与分辨率都低于预算 |
| Surface Cache atlas | ≈ 402 MB（4096² × 3） | **6.00 MB**（3 × 512²，64 个物理页） | 远低于上限 ⇒ **当前瓶颈是覆盖率/内容，不是容量**（页命中率 ~15%） |
| Lumen 输出 + 辐照度 | — | 31.64 MB（2 × 屏幕 RGBA16F） | 屏幕尺寸的中间纹理 |
| 探针缓冲（Screen Probe） | ≈ 0.4 MB | 7.00 MB（65536 槽 × 96 B，按容量分配） | 实测用量 5649 个探针 |
| 远场光追结果 + 切换 fade | — | 2.76 + 32.00 MB | 步骤 26/27 新增 |
| 命中点 / 光线结果 / 着色输出 ×3 | — | 16 + 16 + 48 MB | 步骤 22/23 的验收用中间量，**可回收** |
| **合计（Lumen 自持）** | — | **596.46 MB** | 其中"真正的 Lumen 数据"（atlas + SDF）约 436 MB，其余为中间/诊断缓冲 |

**帧时实测（同一次运行的 GPU 计时，滚动平均）**：

| 段 | 平均 | 说明 |
| --- | --- | --- |
| Lumen 计算 pass 合计 | **27.110 ms** | 峰值 2031 ms（首帧构建 101 个 mesh 距离场） |
| ├ SDF 构建 | 23.956 ms | **占 88%** —— 下一阶段的优化目标 |
| ├ 页表 + 捕获 + 反馈 | 0.274 ms | 每帧 8 页捕获预算 |
| ├ 探针 + 追踪 + 着色 + SH + 辐照度 | 1.794 ms | 其中**远场硬件光追 0.193 ms**（每帧 45192 条射线） |
| └ SDF 逐像素追踪可视化（调试视图） | 1.097 ms | 仅调试用途，可关 |
| 输出贴图 pass | 0.091 ms | 一次全屏采样 |

⇒ **L3/L4 的 GI 链路本身只花 1.79 ms**；显存与帧时的瓶颈都在 **SDF 的逐帧构建/流送**（L1 的既定
代价），这条结论同时给出了下一阶段优化的优先级。
...
| 23 SH 投影 | ✅ 已完成 | `ScreenProbeSampling.slang`（追踪/投影**共用**采样与随机数，避免两个 pass 积到不同方向）+ `Lumen_ScreenProbe_SHProject.comp.slang` + `RunScreenProbeSHProject()`：每探针一个线程，把 `traceRep` 条光线投成 **4 系数 × RGB** 写回 `ScreenProbe.shR/shG/shB`（布局步骤 20 已定，本步不改结构）。估计量 $ l_i = (2\pi/N)\sum L_j \cdot Y_i(d_j) $（均匀半球 $ \text{pdf} = 1/(2\pi) $）；辐照度 $ E(n) = \pi \cdot l0 \cdot Y00 + (2\pi/3)\sum l_m \cdot Y_m(n) $。**采样由 GGX 换成均匀半球**（本步唯一取舍）：均匀 pdf 让白炉下 $ l0 = 2\pi \cdot Y00 = \sqrt{\pi} $ **与方向、采样数无关** ⇒ "白炉误差为 0"可数值钉死；GGX 保留为 mode 0。另新增 `Lumen/LumenSH.h` C++ 镜像 + `Tests/TestLumenSH.cpp`（6 例 34 断言）把三条解析性质钉死：①`l0` 与方向/采样数无关；②整球白炉 $ E(n) \equiv \pi $ 且各向同性；③单侧白炉 $ E(n) = \pi/2 \cdot (1+\hat{n} \cdot n) $。**实测（白炉）**：有效探针 5649、有效光线 45192（100%）、`l0 = 1.7724609` vs $ \sqrt{\pi} = 1.7724539 $ ⇒ **相对偏差 4.0e-6**；白炉 `prov6_final` min = mean = max = **1.0000**。**踩坑**：SH pass 里按"页命中"再过滤会让每条探针只剩 0.9 条有效光线（步骤 22 的中性值语义要往下传），改为只按"是否打到几何"过滤后回到 93.7%；首次接线漏给布局加 `u_RayResult`（绑定 5）⇒ 有效光线读到 0 且 VUID 46 → 58。**验收**：白炉 1.0000、背靠背 dump 逐位一致、`lumen_passes=0`、单测 **228/5741** 全绿、VUID 46 |
| 24 合成到 Lighting | ✅ 已完成 | 两个 pass、不改 Lighting 一行：`Lumen_ProbeIrradiance.comp.slang`（每像素按"16×16 单元 → 探针"映射找归属探针，用其 4 系数 SH **沿像素法线**重建入射辐照度，兰伯特出射 `albedo/π·E(n)` 写进屏幕尺寸 RGBA16F）+ `Lumen_Irradiance.frag.slang`（全屏三角形把辐照度纹理贴到 Provider 输出；帧图 offscreen pass 只能把输出当颜色附件，compute 纹理不能直接当输出）。白炉仍走常量骨架 ⇒ 白炉读数与辐照度内容解耦。探针映射由 `ScreenProbe_Gather` 新增绑定 5 顺手写出（片元里遍历上万探针找最近不可接受）。**实测**：覆盖像素 2073600（100% 屏幕）；入射辐照度亮度 均值 0.54341 / 最大 1.81325；`prov6_final` rgb 均值 0.044 ≈ `albedo 0.25 / π × E`；`s24_lumen.png` 可见拱顶/立柱/三色旗帜的间接光，颜色与场景一致。**本步挖出三个真 bug**：①五个内部 pass 之间**一条屏障都没有**（都对帧图声明"自管 barrier"）⇒ 探针缓冲"布置写→追踪读→SH 写"竞态，背靠背追踪命中数一致但 `l0` 均值 0.3274 vs 0.3249、**98.6% 像素不同**；②SH 投影用**自己的**帧计数当随机种子，而追踪 pass 因建资源跳过若干帧 ⇒ 实测 trace 82 / sh 57 **差 25 帧**，投影积的是另一组方向；③随机流按**槽位下标**编号，而槽位由 `InterlockedAdd` 按线程完成顺序分配 ⇒ 位置↔方向配对逐次运行都变。三者修完后 `prov6_final/raw` 等全部**逐位一致**。另有定点溢出（×65536 求和超 u32）与 `InterlockedCompareExchange` 参数顺序写反两个小坑。**验收**：白炉 1.0000、背靠背逐位一致、`lumen_passes=0`、单测 228/5741 全绿、VUID 46 |
...
| 29 L4 退出判据 | ✅ 已完成（L4 闭环） | 无新增渲染功能，只做"可断言 + 可计量"。**① 正确混合**：合并 pass 对三类光线各加一条**不变量自检**（纯 SDF 必须 $ t_sdf<t0 $、纯光追必须 $ t_sdf \ge t1 $、SDF-未命中分支必须真未命中），实测**违例 0**；配合步骤 26 的 RT 71.2%/SDF 17.9%/合并后 76.4% 与步骤 27 的"阈值桶台阶 5.8%→3.3%、带外逐位不变"。**② 帧时**：Lumen 计算 pass 平均 **27.110 ms**（峰值 2031 ms = 首帧构建 101 个 mesh 场），拆分 **SDF 构建 23.956 / 页表+捕获+反馈 0.274 / 探针+追踪+着色+SH+辐照度 1.794（其中远场光追 0.193，45192 条射线）/ SDF 调试视图 1.097**，输出贴图 pass 0.091 ms ⇒ GI 本身只占 1.79 ms，**瓶颈是 SDF 逐帧构建**。**③ 显存**：新增 `LogMemoryUsage()` 逐项自报，Lumen 自持合计 **596.46 MB**（Mesh SDF 404 + clipmap 32 + atlas 6 + 输出与辐照度 31.6 + 命中点/光线/着色/fade 等诊断缓冲 112 + 其余 11）；与 §15.3 预算对照：Mesh SDF 吻合（404 vs 420），atlas 远低于上限（6 vs 402）⇒ 当前瓶颈是覆盖率而非容量，诊断缓冲 112 MB 列为可回收项。**验收**：白炉 1.0000、`lumen_passes=0`、四个转储逐位一致（本对运行 RT 抖动未出现）、单测 231/5757、VUID 46 |
| 30 探针表示升级 | ✅ 已完成 | DDGI 三阶 9 系数 → **二阶 4 系数**，三处同步改：`DDGI.comp.slang`（`kStride` 16→4、只投影 bands 0/1、删 `SH_Band2`）、`RT_DDGI.slang`（`kDDGI_Stride` 16→4、`EvalDDGI_SH` 去掉 `A_2` 的 5 项）、`GI_DDGI.h`（`kFloats4PerProbe` 16→4）。`kGIConfProbeGrid` 是"通道位 + 网格覆盖"语义、与内存布局无关，故不受影响。**验收**：白炉 **1.0000**；DDGI 单独画面 **0.09129 → 0.09118（−0.12%）**、**逐像素相关系数 0.99996**、差值均值 7.6e-4（连续分布，非块状错位）；相对基线增量 +0.01986 → **+0.01975（−0.6%）**。**收益**：探针 **256 B → 64 B**（本场景 1408 探针 ⇒ 360 KB → 90 KB），评估端 9 次乘法 → 4 次；表示收敛到与 Screen Probe 的 `shR/shG/shB` **同构**（步骤 31 的基础） |
...
| 37 性能与异步（L6 退出判据） | ⏸ **未达成：工作暂停**（测量与归因已完成，落点留档，见附四十四） | **做了测量基础设施**：逐 pass **CPU 录制**计时（`HE_CPU_PASSES=1`，`RenderGraph` 内）、管线 CPU 三段分解（重建帧图/编译/执行）、附属 pass 的 GPU 计时（`GITimer` 扩到 64 格，主/附属各一格）、**稳态口径**（mesh 场建完时把滚动平均清零，峰值保留）、整帧预算行（各 pass 合计 ⇒ fps 上限 + 最重的 pass + 附属 pass）、样例侧真实帧率与 CPU 侧耗时（`HE_NO_VSYNC=1`）、以及 `lumen_irradiance` 转储。**实测（1080p）**：GPU 各 pass 合计 **10.4~14.3 ms ⇒ 70~96 fps**，其中 Lumen 计算 pass **稳态 3.3 ms**（步骤 29 报的 27 ms 是**启动期平均**：首帧建 101 个 mesh 场峰值 2.0 s；稳态 SDF 构建 0.000 ms）；但**整帧是 CPU 受限**：关 Lumen 34.6 ms（28.9 fps）/ 开 Lumen 51.5 ms（19.4 fps），CPU 侧 100% 落在管线 `Render` 里，其中 `rg.Execute`（录制）30~48 ms、重建帧图仅 0.19 ms。逐 pass CPU 录制：**Shadow 28~33 ms**（既有，与 Lumen 无关）+ **Lumen_SDF_Build 16.4 ms**，后者 79% 是 `RunFeedback` 的槽位回读（8160 槽逐元素读未缓存主机内存，实测 12.95 ms）。**⇒ L6 的 60fps 未达成，但瓶颈在 CPU 侧录制（尤其阴影 pass），不在 GPU 也不在 Lumen 本体**；Lumen 的 CPU 成本已定位到具体一行（附四十四）。尝试过的修法（整块 memcpy）把 Lumen 计算 pass CPU 从 16.4 → 4.5 ms、整帧 51.5 → 35.3 ms，但**改变了请求集合**并随后观测到辐照度纹理变黑（CPU 统计仍正常）⇒ **已回退**，改为下一轮用"GPU→CPU 拷贝 + 栅栏"的确定性回读重做。**验收**：白炉 1.0000、关 Lumen ⇒ `lumen_passes=0`、单测 237/5792 全绿、VUID 46、Lumen 输出与步骤 35/36 的基线差 **1286 像素 1 个 f16 ULP**（同族末位抖动） |
| 38 白炉真值覆盖 | ✅ 已完成 | 固化在 `build/verify/acceptance_sweep.ps1` 的判据 ①：开 Lumen 白炉下 `prov6_final` 必须 **min = mean = max = 1.0000**（步骤 23 另有探针 $ l0 = \sqrt{\pi} $ 的解析判据）。**一条命令跑完五项验收并给出 PASS/FAIL**（`powershell -NoProfile -ExecutionPolicy Bypass -File build\verify\acceptance_sweep.ps1`）。**实测：PASS** —— 白炉 `min=1.0000 mean=1.0000 max=1.0000`。**踩坑记录**：脚本前三次都误报 FAIL，根因是 **PowerShell 函数的返回值 = 它的全部输出** —— `$lp = Run-Smoke ...` 会把函数自己的日志行一起收进来（得到 `"  [aq_off] lumen_passes=0 0"`），改成**线性脚本 + 从日志文件里正则取值**后正常 |
| 39 背靠背单源采样对照 | ✅ 已完成 | 固化在同一脚本的判据 ②：同配置两次运行、逐转储比对，**明确排除既有的抖动族**（`prov0_ao_*`（SSAO）、`hdr`、`radiance`），判据为"抖动族之外 **≤2 个 f16 ULP**"。**实测：PASS** —— 抖动族之外 5 个转储有差异、最大 **2 ULP**（步骤 27/28/35/36 均有同族记录） |
| 40 调试与可视化工具 | ✅ 已完成 | 现有工具链：页状态/反馈日志（步骤 16/17）、`lumen_sdf_trace`（步骤 12）、`lumen_card_coverage`（步骤 13）、`lumen_sc_atlas_albedo`（步骤 15）、`lumen_irradiance`（步骤 37，用来把"没画"与"画了但采样到空"分开）、逐 pass GPU 耗时（`GITimer`）与**逐 pass CPU 录制耗时**（`HE_CPU_PASSES`，步骤 37）、整帧预算（步骤 37）。每个已知故障模式都能被至少一个工具直接观察到（本轮"黑辐照度"就是靠 `lumen_irradiance` 归因的） |
| 41 单测与预设回归 | ✅ 已完成 | 单测 **237 例 / 5792 断言**全绿（`Tests/`：GI 数据层、Lumen SH/页表/追踪配置、降噪信号框架等）；预设回归每轮做：默认预设与基线比 **只允许既有抖动族不同**（本轮实测抖动族之外 0 项差异） |
...
- **实测**：见附四十二。探针噪声 `std/mean` **−39.4%**（仅空间 −35.1%）、画面级高频代理 **−21.6%**、
  负离群 min −0.0270 → −0.0009；白炉探针 l0 均值滤波前后同为 **1.77246**；历史走统一池
  （池 4 纹理 → 4 纹理 + 4 缓冲）。两条前提都兑现了：信号按层栈登记（`ProducedThisFrame()`）、
  转储判据同步收紧。**两处"另挂一套"是被消灭而不是被新增**：历史资源进池、信号进登记处。
...
- **实测**：见附四十三。4 个 `RT_*_Upscale` pass；`prov9_spec_final` 960×540 → 1920×1080、
  `prov10_final` 480×270 → 1920×1080；旧路的"模糊有效性"像素 3.84% → 0.0000%，
  RT 反射 `alpha<0` 85.61% → 81.86%；HDR 亮度均值 +1.88%（关 AO 通道后）。
  **没有**声称"比双线性更准"：与"全分辨率参照"的对照被证伪（那个参照不是同一个估计量）。
...
- sphere tracing：`SDF_RayMarch`（全局，两层）与 `SDF_RayMarchDetail`（逐 mesh 细节追踪），
  **半量安全步长** $ t += \max(0.5 \cdot d, \text{eps}) $。
...
2. **暴露了一个新的真问题：19 条穿漏**（此前 0~3 条被远场射线掩盖）。机制清楚：射线起点距表面
   ...
   （$ t += \max(0.5d, \text{eps}) $ 的 eps 下限在这里成了过冲源）。
...
查明：**收敛阈值 `eps = 0.25 × 近层体素 = 1.54`，而射线起点只离表面 1 个单位** ⇒ $ d(\text{origin}) < \text{eps} $，
...
（227 / 9 / 14、p50 1.080）—— 说明起点外移 4 单位后 $ d(\text{origin}) \approx 4 > \text{eps} = 1.54 $，本来就不存在
...
- $ \text{tRef} < 50 $（近场命中）：p50 ≤ 1 体素（当前应能达标，近场探针精度 0.16~0.19 体素）；
- $ \text{tRef} \ge 50 $：单独统计，并明确它由 layer 1（24.64 体素）负责，指标按远场标准看。
...
- 也就是真正剩下的缺陷是**高估**（→ 穿漏），不是低估。来源有两处，按收益排序：
  ...
  2. **跳步洪泛的 fixpoint 本身是"高估"**（$ d(\text{seed}) + \lvert x-seed \rvert \ge d(x) $），需要按 UE 的做法补一轮
...
来源是"跳步洪泛的 fixpoint 本身偏高"：$ d(\text{seed}) + \lvert x−seed \rvert \ge d(x) $ 恒成立，越远越松。
...
（$ d(\text{seed}) + \lvert x−seed \rvert \ge d(x) $，越远越松）。粗场之所以还能用，是靠"1 个近层体素的容差 + 跨 16 个 mesh 取 min"。
...
### 附三十：步骤 23「SH 投影」——白炉下 0 阶系数与解析值 √π 相等（相对偏差 4.0e-6）

**实现**：`Lumen_ScreenProbe_SHProject.comp.slang` + `LumenScene::RunScreenProbeSHProject()`，
接在步骤 22 之后、同一个 compute pass 内；每探针一个线程，把 `traceRep` 条光线投成
**4 系数 × RGB = 12 floats**，直接写回 `ScreenProbe` 的 `shR/shG/shB`（步骤 20 已定好布局，本步不改结构）。

**估计量**：光线是**均匀半球采样**（pdf = 1/(2π)），于是 $ l_i = (2\pi/N) \cdot \sum_j L_j \cdot Y_i(d_j) $；
辐照度按 clamped cosine 卷积系数重建：$ E(n) = \pi \cdot l0 \cdot Y00 + (2\pi/3) \cdot (l1 \cdot Y1 + l2 \cdot Y2 + l3 \cdot Y3) $。

**为什么把采样从 GGX 换成均匀半球（这是本步唯一一处取舍）**：步骤 21 用 GGX 半向量采样，
其 pdf 也能让估计量无偏，但**白炉下 0 阶系数没有解析值**（要除以 pdf 后求和）。
均匀半球的 $ l0 = 2\pi \cdot Y00 = \sqrt{\pi} $ **与采样方向、采样数完全无关** ⇒ 步骤 23 的验收口径
"白炉下误差为 0" 可以被数值钉死，而不是"看起来很小"。采样与随机数抽到
`Lumen/ScreenProbeSampling.slang` 里由**追踪与投影共用**，保证两个 pass 积的是同一组方向
（不另开方向缓冲：省 16 MB/帧，也避免两份采样代码漂移）。GGX 分支保留为 mode 0。

**实测（`lumen_step23` / `s23a` / `s23b` / `s23f`，1080p，exit=0，VUID 46 = 基线）**：

```
常态：探针 SH 投影（步骤 23）: 有效探针 5649，有效光线 42345（93.7%）；l0 均值 0.321328（辐照度量级，无解析值）；
      SH 重建辐照度 vs 逐光线求和 平均相对差 0.1005（5647 个探针）
白炉：探针 SH 投影（步骤 23，白炉）: 有效探针 5649，有效光线 45192（100.0%）；
      l0 均值 1.7724609（解析值 √π = 1.7724539）；l0 平均绝对偏差 0.000e+00（定点量化下限 1.5e-05）；
      均值相对偏差 4.00e-06；SH 重建辐照度 vs 逐光线求和 平均相对差 0.0912（5649 个探针）
```
...
| 口径 | 结果 |
| --- | --- |
| **白炉下 SH 的 0 阶系数** | GPU 回读 `l0 = 1.7724609`，解析值 $ \sqrt{\pi} = 1.7724539 $，**相对偏差 4.0e-6**（float 累加量级） ⇒ "误差为 0" ✅ |
| 白炉整体读数 | `gi_s23f_prov6_final.f16` min = mean = max = **1.0000** ✅ |
| 背靠背单源逐项一致 | `lumen_sc_atlas_albedo` / `lumen_card_coverage` / `lumen_sdf_trace` / `albedo` **SHA-256 逐位一致**；`hdr` 不同（时域累积，预期） ✅ |
| 既有预设不回归 | `gi_blend_diffuse_lumen=0` ⇒ `lumen_passes=0` ✅ |
| 单测 | 221 → **228** 例、5707 → **5741** 断言全绿（新增 `Tests/TestLumenSH.cpp`，6 例 34 断言） ✅ |

**新增单测（`Tests/TestLumenSH.cpp` + `Engine/Render/Lumen/LumenSH.h` 镜像）把三条**解析**性质钉死**：

1. **白炉下 l0 与方向、采样数无关**：任取法线与 $ N \in \{1, 8, 64\} $，$ (2\pi/N)\sum Y00 $ 恒等于 $ \sqrt{\pi} $（相对误差 $ < 1e-12 $）；
2. **整球白炉**（= 引擎 `HE_FURNACE=1` 的语义，整球辐射度恒为 1）：$ l0 = 4\pi \cdot Y00 = 2\sqrt{\pi} $、$ l1..3 = 0 $
   ⇒ 带内重建 $ E(n) \equiv \pi $ 且**与方向无关**，这是"误差为 0"的最强形式；
3. **单侧（半球）白炉**：$ l0 = \sqrt{\pi} $、$ l_m = Y1 \cdot \pi \cdot \hat{n}_m $ ⇒ $ E(n) = \pi/2 \cdot (1 + \hat{n} \cdot n) $。
   这条把"**探针的白炉重建并不会自动给出各向同性的 π**"钉住 —— 单侧白炉本就是各向异性的，
   只在 $ n = \hat{n} $ 时等于 π。写下来是为了避免后面把这条**带限/单侧**差异误判成实现错误。

**过程中修掉/澄清的两点**：

1. **不能在 SH 投影里按"页命中"再过滤一次光线**。步骤 22 对缺页已经返回**中性值 0.18**（不是黑），
   那正是它要继续往下的语义；若在投影里再按 `w > 0.5` 过滤，实测每条探针只剩 **0.9 条**有效光线
   （8 条里 0.9 条），SH 等于没积。改成只按"这条光线有没有打到几何"过滤后，有效光线回到 **93.7%**。
   缺页率高的影响因此从"SH 全是零"降级为"高频细节被中性值抹平"（见 §附二十九 的覆盖率归因）。
2. **首次接线时忘了给 SH pass 的布局里加 `u_RayResult`（绑定 5）**：shader 里声明了、布局里没有，
   运行期读数变成"有效光线 0"，验证层同时多出 12 条 VUID（46 → 58）。
   ⇒ 记下来：**新增绑定必须同时改三处**（shader 声明、C++ 布局、`UpdateDescriptorSet`），
   VUID 条数是这三处是否一致的可靠探针。

**已知边界（不阻塞步骤 24）**：光线仍是 8 条/探针（`traceRep` 下限），白炉下"SH 重建 vs 逐光线求和"
仍有 9.1% 的平均相对差 —— 这是**有限采样 + 二阶带限**的固有差，不是实现误差（单测里 512 条时降到 < 5%）；
常态下 10.1% 的差另含"缺页中性值 0.18 抹平细节"的成分。时序累积（步骤 34 起）会按 `1/√帧数` 压低它。

**下一轮**：步骤 24 合成到 Lighting（从 SH 采样出 per-pixel 入射辐照度、接到 `LightingInputs`），
随后 25 L3 退出判据（与 SSGI/DDGI 的背靠背对照，要求"差异可归因"）。
### 附三十一：步骤 24「合成到 Lighting」——Lumen 输出第一次有了场景一致的间接漫反射

**实现（两个 pass，不改 Lighting 一行代码）**：

- `Lumen_ProbeIrradiance.comp.slang`：每个屏幕像素按"**16×16 单元 → 探针**"映射找到归属探针
  （映射由 `ScreenProbe_Gather` 在布置时顺手写出，新增绑定 5；若在片元里遍历上万探针找最近，
  代价无法接受），再用该探针的 4 系数 SH **沿该像素自己的 GBuffer 法线**重建入射辐照度 `E(n)`，
  兰伯特出射 $ \text{albedo}/\pi \cdot E(n) $ 写进屏幕尺寸 RGBA16F 纹理。用像素法线求值等于免费拿到法线插值 ——
  SH 本来就是方向函数，代价只有每像素 4 次乘加。
- `Lumen_Irradiance.frag.slang` + `LumenScene::DrawIrradiance()`：帧图的 offscreen pass 只能把输出
  当**颜色附件**写入（compute 写好的纹理不能直接当输出），所以用一条全屏三角形把辐照度纹理采样
  出来写进 Provider 输出。`LumenProvider::Render` 因此变成：白炉走常量骨架 pass（输出 1.0），
  非白炉走这条贴图 pass。**白炉读数与辐照度内容解耦**：辐照度算错也不会悄悄改变白炉结果。

**实测（`lumen_step24` / `s24i1` / `s24i2`，1080p，exit=0，VUID 46 = 基线）**：

```
逐像素辐照度（步骤 24）: 覆盖像素 2073600（100.0% 屏幕）；入射辐照度亮度 均值 0.54341、最大 1.81325
Provider 输出（prov6_final）: rgb 均值 (0.0441, 0.0463, 0.0389)（= albedo/π × E，与 albedo 均值 0.25 一致）
```

可视化 `build/verify/s24_lumen.png`（1920×1080，`f16_rgba_png.py` 生成）里能直接看到拱顶、立柱与
蓝/绿/红三色旗帜的**间接漫反射**，且颜色与场景一致 —— 这满足"关闭其它源、只开 Lumen 时屏幕上
出现与场景一致的间接漫反射"的验收口径。画面里的 **16×16 方块**是"每单元一个探针、探针间不做
空间插值"的直接结果（计划里写明首版可"直接采样最近探针，后续再插值"），不是错误。

**本步挖出的三个真 bug（都是可复现的，不是推测）**：

1. **五个 Lumen 计算 pass 之间没有屏障 ⇒ 探针缓冲有竞态**。`Lumen_SDF_Build` 这一个 pass 里连续
   Dispatch 了 SDF 构建 → 页表 → 捕获 → 反馈 → 布置 → 追踪 → 着色 → SH → 辐照度，它们**互相读对方
   刚写的缓冲**，却因为"自持资源 + 自管 barrier"而对帧图不声明依赖 —— 于是**一条屏障都没有**。
   探针缓冲上正好是"布置写 → 追踪读 → SH 写"的写后读/读后写链。
   **症状**：背靠背两次运行的追踪命中数完全一致（42366 = 42366），但探针 SH 的 `l0` 均值不同
   （0.3274 vs 0.3249）、逐像素辐照度 **98.6% 的像素不同**（最大差 0.19，均值差 0.018）。
   **修法**：在每个 pass 的 `SetPipeline` 之前插一条显式的计算→计算**全局**屏障
   （`LumenScene::ComputeBarrier`；用全局形式，逐资源写屏障漏一个就是一次竞态）。
   修完之后 `prov6_final/raw` 与 `lumen_sc_atlas_albedo`、`lumen_sdf_trace`、`albedo` **全部逐位一致**。
2. **SH 投影用的随机种子与它拿到的光线不是同一批**。追踪 pass 每个"真正派发"的帧用自己的帧计数
   当种子再 +1，而投影 pass 每帧都派发；两者创建资源时各跳过若干帧，帧计数越差越多 ——
   实测 **trace 82 / sh 57，差 25 帧**。也就是说投影积的是**另一组方向**的辐射度（系数看着正常，
   实际是错的积分）。修法：投影用"追踪上一次实际派发时用的种子" = `追踪帧计数 - 1`。
   【这是"共享采样头"也救不了的一类错：方向算法一致，但**种子来源不一致**。】
3. **随机流按槽位下标编号 ⇒ 探针与方向的配对逐次运行都不同**。布置 pass 用 `InterlockedAdd` 分配
   槽位，线程完成顺序不同 ⇒ 同一探针两次运行可能落在不同 slot；随机流跟着 slot 走时，
   "位置 ↔ 方向"的配对每次运行都会变。修法：随机流按**探针身份**编号（`tileIndex×4 + 单元号`，
   布置时已写进 `meta`），与槽位解耦。修完追踪命中数从"每次不同"变成"两次都是 42366"。

**另外两个小坑（同一类，一并记下）**：

- 逐像素亮度求和用定点 ×65536：2073600 像素 × 0.2 × 65536 ≈ 2.7e10 **超过 u32**，回绕后平均亮度
  读成 0.0018（真值约 0.2）。累加改 ×1024（和约 4e8），最大值仍用 ×65536（不累加）。
- `InterlockedCompareExchange` 的参数顺序是 **(dest, compare, value, out original)**，第一版把
  compare 与 value 写反 ⇒ 永远只把 dest 往小里改，最大值读数恒 0。

**验收证据**：

| 口径 | 结果 |
| --- | --- |
| 只开 Lumen 时出现场景一致的间接漫反射 | `s24_lumen.png` 可见拱顶/立柱/三色旗帜的间接光，颜色与场景一致 ✅ |
| 白炉真值 | `gi_s24f_prov6_final.f16` min = mean = max = **1.0000**（白炉仍走常量骨架，与辐照度内容解耦） ✅ |
| 背靠背单源逐项一致 | `prov6_final` / `prov6_raw` / `lumen_sc_atlas_albedo` / `lumen_sdf_trace` / `albedo` **SHA-256 全部逐位一致**（修完屏障之后） ✅ |
| 既有预设不回归 | `gi_blend_diffuse_lumen=0` ⇒ `lumen_passes=0` ✅ |
| 单测 | 228 例 / 5741 断言全绿 ✅ |

**顺带记录（不是本步引入的）**：`prov0_ao_raw` 背靠背有 0.42% 像素不同（最大差 0.08、均值差 1.5e-5），
是 AO pass 自身的 run-to-run 抖动；Lumen 侧修完屏障后已完全消除，这条留在阶段 E 的优化项里。

**已知边界（不阻塞步骤 25）**：探针之间不做空间插值（画面有 16×16 方块感）；辐射度暂用命中点
albedo（直接光照待后续步骤）；探针 SH 只到二阶（低频）。

**下一轮**：步骤 25 —— L3 退出判据（"半球追踪产生漫反射 GI"已在本步拿到画面证据；再补与
SSGI/DDGI 的背靠背对照，要求"差异可归因"）。
### 附三十二：步骤 25「L3 退出判据」——Lumen 的间接漫反射量级与 DDGI 同级（+0.0192 vs +0.0199）

L3（§12）的判据是两条：**① 半球追踪产生漫反射 GI**；**② 与 SSGI/DDGI 的背靠背对照可解释差异**
（不要求一致，只要求差异可归因）。两条都用"同一场景、同一相机、同一帧号（120）、只切换漫反射
层栈"的对照跑出来：`gi_blend_diffuse_w0=IBL / w1=DDGI / w2=SSGI / w3=RTGI`（GILab 的槽位顺序），
另加独立的 `gi_blend_diffuse_lumen` / `gi_blend_diffuse_rsm` 键。

**① 半球追踪确实产生了漫反射 GI**

| 证据 | 数值 |
| --- | --- |
| Lumen 的漫反射输出覆盖 | `prov6_final` 亮度非零像素 **99.7%**（1920×1080） |
| 量级 | 亮度均值 **0.04507**、中位 0.03726、p90 0.07320（≈ $ \text{albedo}/\pi \cdot E $，与 albedo 均值 0.25 自洽） |
| **与场景材质一致** | `prov6_final` 与同帧 GBuffer albedo 的亮度相关系数 **0.6468**（R/G/B 通道 0.744 / 0.631 / 0.785）⇒ 间接光跟着表面颜色走（旗帜的蓝/绿/红都进了 GI，见 `s24_lumen.png`） |

**② 与 SSGI / DDGI 的对照（相对"漫反射栈为空"的基线，HDR 亮度均值）**

| 配置 | HDR 亮度均值 | 相对基线增量 |
| --- | --- | --- |
| 基线（漫反射栈为空） | 0.07143 | — |
| **只开 Lumen** | 0.09060 | **+0.01917（+26.8%）** |
| 只开 SSGI | 0.07143 | **+0.00000** |
| 只开 DDGI | 0.09129 | **+0.01986（+27.8%）** |

**差异归因（逐条，都是实测）**：

- **Lumen 与 DDGI 的增量同级**（+0.01917 vs +0.01986，差 3.6%）。两者是完全不同的算法
  （世界空间 SDF 半球追踪 vs. 探针网格辐照度），能在同一场景给出同量级的间接漫反射，
  互为交叉验证 —— 这正是"差异可归因"要的形状：**不一致的地方是分布，不是标度**。
- **SSGI 的增量恰好为 0，且这不是"没输入"**：SSGI 的输入 `radiance`（前帧 HDR 辐射度，480×270）
  **100% 非零**（均值 0.0714、最大 1.33），但它的漫反射输出纹理 `prov3_raw/final` **逐像素全为 0**
  （两种配置都验过：漫反射栈只放 SSGI、以及 IBL+SSGI）。这是**仓库既有的 SSGI 链路问题**
  （不是本步引入：同键位的配置与 `build/verify/ssgical_cal_ssgi.cfg` 逐键一致），
  Lumen 侧只把它记为对照结论，留给 SSGI 自己的任务线。
- **DDGI 没有通道纹理输出**（`DDGIProvider::GetDiffuseOutput()` 按设计返回 `nullptr`，注释写明
  "compute pass，无通道纹理输出"），所以它只能做 HDR 级对照 —— 这是**接口层的差异**，不是算法差异。
- **一处需要说明的偏差**：`Lumen 的 prov6` 与"HDR 相对基线的逐像素增量"的相关系数只有 **0.4882**，
  比值均值 0.01917/0.04507 ≈ **0.43**（不是 1.0）。也就是说合成端对 Lumen 的漫反射做了约 0.43
  的整体缩放，且这个缩放**不是逐像素常数**。这属于**合成/归一化环节**（层栈权重归一化、
  `gi_intensity`、边缘淡出）的现象，不是 Lumen 输出本身的问题 —— 判据是"Lumen 输出是否为
  场景一致的间接漫反射"，本步已用"输出 ∝ 场景材质"（相关系数 0.65）证明；合成增益的逐像素
  形状留给 §10 的合成归一化任务。

**验收**：L3 两条判据均达成 ✅。白炉 `prov6_final` = 1.0000、`gi_blend_diffuse_lumen=0` ⇒
`lumen_passes=0`、背靠背转储逐位一致、单测 228/5741 全绿、VUID 46 = 基线（均在本步的对照跑里复核）。

**工具**：`build/verify/lumen_smoke.ps1 -Extra '<键值对>'` 可任意切换层栈；新增分析脚本用途见本节
表格（亮度统计 + 逐像素相关/增量归因，均为一次性内联计算，未落成常驻工具）。

**下一轮**：阶段 E / L4 —— 步骤 26 远场 TraceRay（`rayDistance ≥ maxSDFDistance` 时改走 HW RT），
复用 `RTPass` 的 TLAS 与场景材质纹理。
### 附三十三：步骤 26「远场 TraceRay」——顺带查出"近场 SDF 的命中距离不是几何距离"

**实现（四块）**：

1. `Lumen/Lumen_FarField.rgen.slang` + `Lumen/LumenFarField.slang`（共享 payload/参数）：对**步骤 21 的
   同一条探针光线**再发一条硬件光追。方向由共享的 `ScreenProbeSampling.slang` 用"探针身份 + 光线号
   + 帧号"给出，与 SDF 追踪必然一致；命中/未命中着色**复用 RTGI 的 `RT_GI.rchit` / `RT_GI.rmiss`**
   （量纲与 RTGI 同源，步骤 27 的切换才不会有阶跃）。每条光线写两个 `float4`：
   `[2i] = (命中距离, 命中标志, 命中辐射度亮度)`、`[2i+1] = (命中点世界坐标, 1)`。
2. `LumenFarFieldPass`（继承 `RTEffectPass`，与 `DDGITracePass` 同一取舍）：复用 RT 管线 + SBT +
   set0 生命周期；输出是 SSBO 而不是纹理（基类那张 1×1 纹理只是占位）。
3. `Lumen_FarFieldCompare.comp.slang`：**逐光线**比较 SDF 与三角形的命中距离，按"近带/远带"和
   相对误差直方图分类统计（整数定点原子加，与线程顺序无关）。
4. `Lumen_FarFieldMerge.comp.slang`：按阈值把两者合并 —— `SDF 命中且 t < 阈值` 保留 SDF，否则采用
   光追的命中点与距离，**直接写回步骤 22 的输入**（`u_RayHitPos` 与光线结果缓冲）。合并单独一步
   而不是让 rgen 改写，是因为验收要求"在改写之前"逐光线比较。

**实测（`lumen_step26` / `s26a` / `s26b`，1080p，exit=0，VUID 46 = 基线）**：

```
远场光追（步骤 26）: 光线 45192；两侧都命中 5696（SDF 命中率 17.9%、RT 命中率 71.2%）；
    只有 SDF 命中 2379 / 只有三角形命中 26488；两者都未命中 10629；
    全部命中光线 相对差 均值 0.7718 最大 1.0000，≤5% 占 2.2%、≤20% 占 8.5%
  分带对照（分界 50 单位）: 近带 4863 条 —— 相对差 均值 0.8514、≤20% 占 1.9%；
    远带 833 条 —— 相对差 均值 0.3071、≤20% 占 47.4%
  SDF 命中距离: 均值 55.97；其中 t < 1 的自交命中 307 条（占 SDF 命中 3.8%）
合并之后 → 步骤 22: 命中光线 34553（76.4%，此前只有 SDF 的 8079 = 17.9%）；页命中 5343（15.5%）
```

**本步最重要的一条结论：近场 SDF 的"命中距离"此前根本不是几何距离。**

- 原先探针 march 的 `eps = 1 个近层体素 = 21.3 世界单位`。march 只要判定"离某个表面在一个体素以内"
  就停，于是**命中率 94% 全是假命中**：命中距离均值只有 **16.4 单位**，而硬件光追打到的真实表面在
  几十到几百单位外（相对差直方图有 42% 落在最后一桶 `[0.94, 1.0]`）。
  ⇒ 步骤 22 用这些"命中点"去卡片/页里取材质，取到的位置与真实表面差了十几到几十个单位 ——
  这正是步骤 19/22 里"选卡歧义 / albedo 对不上"的一大来源。
- 把 eps 改成**几何量级**（`0.1 个体素 ≈ 2.1 单位`）之后：SDF 命中率掉到 **17.9%**、命中距离均值升到
  **55.97 单位**（这才是"贴着表面"的量级）。也就是说：**全球 clipmap 的近层体素 21.3、远层 42.7，
  在本场景尺度上根本不足以承担远场求交** —— "近场 SDF + 远场 HW RT"不是优化，而是让这条链路
  **成立**的必要条件。这正是计划把步骤 26/27 放在 L3 之后的原因，本步用数据把它钉住了。
- 合并之后的总体命中率回到 **76.4%**（SDF 17.9% + 光追补齐），且命中点全部来自真实三角形或
  几何量级的 SDF 命中 ⇒ 步骤 22/23/24 的输入第一次是几何上站得住的。

**对前面几轮的修正（必须写下来）**：

| 位置 | 原记录 | 修正后 |
| --- | --- | --- |
| 步骤 21 命中率 | 40836（90.4%） | 这是 `eps = 1 体素` 下的**假命中**；`eps = 0.1 体素`（几何量级）后 SDF 命中 8118（18.0%），远场由步骤 26 的光追补齐到 76.4% |
| 步骤 21 命中距离 | 未记录 | 均值 16.4 单位（体素 eps）→ 55.97 单位（几何 eps） |
| 步骤 22 页命中率 | 8.1%（基于假命中点） | 15.5%（34 ms 采样点来自真实表面，命中点更集中） |
| 步骤 24 辐照度均值 | 0.54341 | 0.44021（SDF 假命中消失后，远场由光追提供） |

**本轮踩的五个坑（都记下来，三个是同一类）**：

1. **"结果缓冲非空"不能当前置条件** —— 而结果缓冲正是被门控的那次调用创建的。这个坑本轮踩了
   **三次**：`RunFarFieldRT` 用 `IsValid()` 当门（结果缓冲还没建 ⇒ 永远提前返回）、`Trace()` 里又用
   一次 `IsValid()`（同样死锁）、`CreateFarFieldCompareGPUObjects` 里绑了还不存在的缓冲（空指针崩）。
   ⇒ 记法：**资源创建路径上的门只能看"更早建立的资源"**（管线/描述符集），绝不能看本次要建的资源。
2. **加速结构没人构建**：`AS_Build` 的注册条件是 `AnyRTSource() || ShouldRunDDGI()`，而 Lumen 的远场
   光追**两者都不是**（Lumen 不是 RT 效果源）。症状是"光追在跑、命中率 0.0%、SDF 命中率 93.8%"。
   加 `diffuse.Has(Lumen)` 一项后 0% → 71.4%。⇒ 新增 AS 消费者时必须同时更新"谁触发 AS_Build"。
3. **rgen 写两个 `float4`/光线，而 C++ 只按一个分配** ⇒ 越界写。症状很隐蔽：不是崩溃，而是
   "RT 命中率从 71% 变成 44%"这种**说不通但稳定**的读数（踩坏了相邻显存）。⇒ 缓冲区大小与
   shader 的写布局必须成对修改，改一处就是这种"稳定但错误"的结果。
4. **布局改成两条后，对照 shader 的索引没跟着 ×2** ⇒ 它把"命中点坐标"当成了"命中距离"，于是
   出现 44% 这种伪命中率。与 3 是同一类：**改布局必须同时改所有读写方**。
5. **一次字符串替换静默没生效**：我把"合并"插到着色之后，等于白跑（写回的值下一帧就被覆盖），
   而替换失败时构建、运行、读数百思不得其解。⇒ 每次批量替换后**必须核对行号顺序**（这次就是
   靠"785 < 791"才发现的）。
...
| 口径 | 结果 |
| --- | --- |
| 远场命中与三角形场景一致 | 光追直接在 TLAS 上打三角形：RT 命中率 71.2%（半球探针室内场景的合理量级）；命中着色复用 `RT_GI.rchit`（与 RTGI 同源） ✅ |
| 逐光线交叉验证 | 与 SDF 的对照给出可归因的差异（SDF 假命中/远场不可用，见上表），**不是**"看起来差不多" |
| 白炉真值 | `gi_s26f_prov6_final.f16` min = mean = max = **1.0000** ✅ |
| 背靠背单源逐项一致 | `prov6_final` / `lumen_sc_atlas_albedo` / `lumen_sdf_trace` / `albedo` **SHA-256 逐位一致** ✅ |
| 既有预设不回归 | `gi_blend_diffuse_lumen=0` ⇒ `lumen_passes=0` ✅ |
| 单测 | 228 例 / 5741 断言全绿 ✅ |

**已知边界（留给步骤 27）**：合并用的是**硬阈值**（$ t_sdf < 50 $ 保留 SDF，否则用光追），阈值处会有
不连续 —— 步骤 27 要做的是 `traceRep` 维度上的切换与 N 步 overlap fade。另外 SDF 仍负责近场，
而它的近层体素是 21.3 单位，所以"近场"的几何精度仍有上限（近带 ≤20% 一致率只有 1.9% 就是它的代价）。

**下一轮**：步骤 27 —— `traceRep` 切换与 overlap fade，把阈值处的阶跃消掉（沿射线扫过阈值时探针
辐照度连续）。
### 附三十四：步骤 27「traceRep 切换与 overlap fade」——阈值处的台阶从 5.8% 压到 3.3% 并把过渡摊开

**实现（切换逻辑只挂在 `traceRep` 上，与 `shadeRep` 无关 —— 正是 §6 的副作用说明）**：

- **权重**：$ w = \text{smoothstep}(t0, t1, t_sdf) $，`t0/t1 = 阈值 × (1 ∓ 半宽)`，默认阈值 50、半宽 0.2 ⇒
  重叠带 `[40, 60]`。用 smoothstep 而不是线性，是为了两端一阶连续（线性在 t0/t1 处仍有斜率突变）；
  半宽 0 时退化成硬切换，保留 A/B 对照能力（环境变量 `HE_LUMEN_FARFIELD_OVERLAP` /
  `HE_LUMEN_FARFIELD_THRESHOLD`）。
- **谁混合什么**：`Lumen_FarFieldMerge` 只负责**把两个命中点都交出去**（主点 + 远场点 + 权重），
  由 `Lumen_SurfaceCacheSample` 各取一次材质再按权重插值。**不能插值位置**——两个表面之间的点是
  空气，取其材质没有物理意义；混合的是"看到什么"，不是"在哪"。
- **连续性证据**：着色 pass 把每条光线**实际交给 SH 的值**按**稳定的距离量**（有 SDF 命中时用 SDF
  距离，否则用光追距离）分桶（5 世界单位一桶、覆盖 20..100，阈值 50 = 第 6 桶），CPU 侧打出曲线。

**A/B 实测（同一场景同一相机同一帧号，只改重叠带半宽）**：

| 桶（世界单位） | 硬切换（半宽 0） | 重叠带 0.2 |
| --- | --- | --- |
| 35–40 | 0.20121 | 0.20121 |
| 40–45 | 0.20201 | 0.20227 |
| 45–50 | 0.20046 | 0.20232 |
| **50–55（含阈值）** | **0.21208（相对上一桶 +5.8%）** | **0.20902（+3.3%）** |
| 55–60 | 0.20540 | 0.20504 |
| 60–65 | 0.19845 | 0.19845 |

- 硬切换在**含阈值的那一桶**有一个 **+5.8%** 的台阶（前后桶 0.2005 → 0.2121 → 0.2054）；
  开重叠带后同一桶降到 **+3.3%**，且变化被摊到 `[40, 60]` 整段（40-45 +0.5%、45-50 +0.03%、
  50-55 +3.3%、55-60 −1.9%）⇒ **过渡取代了台阶**，band 之外（0–3 桶与 8–15 桶）两配置**逐位相同**，
  说明 fade 的作用范围严格限制在重叠带内（这正是 overlap fade 的定义）。
- 切换分类（半宽 0.2）：纯 SDF 3503 / 纯光追 28248 / **重叠带混合 445**；带内 44 条的远场页有内容
  （真正两侧材质插值），401 条远场页缺内容（朝远场路径的兜底值插值 —— 与"纯光追"在该点的取值
  完全同一个表达式，所以两端仍然连续）。

**本轮踩的三个坑（都在同一处：切换的"取值"语义）**：

1. **fade 描述符从来没被绑上**。它由着色 pass 创建，而远场 pass 的 set 是在**更早的一帧**建的 ——
   那一帧着色还没跑过，`if (m_RayFadeBuf)` 直接跳过。症状极具迷惑性：合并 pass 的统计说"445 条在
   重叠带内"，着色 pass 一条也读不到（`w>0` 计数恒 0，CPU 侧直接读缓冲也是 0）。
   ⇒ 修法：**在每帧派发前重绑**，而不是在 set 创建时绑一次。这是本轮第四次踩"创建顺序"这一类
   （前三次见 §附三十三）。
2. **插值必须插"该路径实际输出的值"，而不是"材质"**。主点缺页时它的输出是**中性值 0.18**，而
   `chosenAlbedo` 是 (0,0,0)。第一版写 `lerp(chosenAlbedo, altValue, w)` ⇒ 主点缺页的射线在带内被
   拉向 0（变暗），曲线在 **t0 处反而出现一个更深的台阶**（实测 0.2020 → 0.1417，−30%）。
   改成 `lerp(primaryValue, altValue, w)`（两侧都是"路径输出值"）后曲线才正常。
3. **分桶必须按稳定量**。第一版按"最终命中距离"分桶，而切换后该距离会从"SDF 的几十"跳到"光追的
   几百" —— **分桶依据自身就断了**，曲线必然出现假阶跃。改用"有 SDF 命中时用 SDF 距离"这类不随
   切换变化的量。
...
| 口径 | 结果 |
| --- | --- |
| 沿射线扫过阈值无阶跃 | 含阈值桶的相对台阶 **5.8% → 3.3%**，且变化摊到整个重叠带；带外逐位不变 ✅（残留 3.3% 是"远场页缺内容比例"与"SDF 距离不是真实距离"两个已知边界的合成，见下） |
| 白炉真值 | `gi_s27f_prov6_final.f16` min = mean = max = **1.0000** ✅ |
| 既有预设不回归 | `gi_blend_diffuse_lumen=0` ⇒ `lumen_passes=0` ✅ |
| 单测 | 228 例 / 5741 断言全绿 ✅ |
| 背靠背逐项一致 | `lumen_sc_atlas_albedo` / `lumen_sdf_trace` / `albedo` / `gb_normal` / `gb_worldpos` **逐位一致**；**`prov6_final` 有 4 个像素差 1 个 f16 ULP**（见下） ⚠️ |

**必须记下来的一条边界（新出现）**：`prov6_final` 不再逐位一致 —— 2 073 600 个像素里 **4 个**不同，
差 **3.05e-05（≈1 个 f16 ULP）**，平均差 2.2e-11，**且全部落在同一个 16×16 单元（同一个探针）**。
所有统计（追踪命中数、切换分类、分桶曲线）两次运行完全相同 ⇒ 输入一致。最可能的原因是**硬件光追
的 TLAS 每帧重建、BVH 结构与遍历顺序由驱动决定**，同一批射线两次运行的求交距离可能差最后一两位；
一个探针的 8 条光线里有一条差 1 ULP，就会让该探针的 SH 系数差 1 ULP，于是那个单元内的 4 个像素跟着
差一档。**这属于引入硬件光追后的固有限制（驱动级），不是逻辑错误**；判据"背靠背逐项一致"在引入
RT 之后应表述为"除 RT 相关的最后一位抖动外逐项一致"。若要严格逐位复现，需要把远场命中点量化
（或固定 BVH）—— 列为后续项，不在本步强做。

**已知边界（不阻塞步骤 28）**：残留 3.3% 来自两处 —— ①远场页缺内容（page 命中率 ~15%），于是两侧
"值"的差异里含"中性值 vs 真实材质"的成分；②SDF 的命中距离不是真实距离（§附三十三），所以"同一个
距离桶"里的两条路径本就在看不同的几何。彻底消掉需要给远场也配上可用的材质源（Radiance Cache /
步骤 30 之后的 out-of-band 摘要）。
...
Cache）。
### 附三十六：步骤 29「L4 退出判据」——混合按定义执行（违例 0）、帧时 27.1 ms、Lumen 自持显存 596 MB

L4（§12）的判据两条：**① SDF 近 + RT 远正确混合**；**② 帧时与显存增量有记录**。

**① "正确混合"不再靠肉眼：把分类的定义条件做成运行期断言**

`Lumen_FarFieldMerge` 里对三类光线各加一条**不变量自检**（判据就是分类的定义）：

| 类别 | 定义条件 | 违例计数 |
| --- | --- | --- |
| 纯 SDF（$ w == 0 $） | 必须 $ t_sdf < t0 $（近场以内） | **0** |
| 纯光追（两侧都命中且 $ w == 1 $） | 必须 $ t_sdf \ge t1 $（远场以外） | **0** |
| SDF 未命中分支 | 必须真的 $ \text{sdfHit} == 0 $ | **0** |

实测日志：`分类不变量自检（步骤 29，应恒为 0）: 违例 0（近场越界 0 / 远场未到上界 0 / 该分支 SDF 却命中 0）`。
配合步骤 26 的"RT 命中率 71.2% / SDF 17.9% / 合并后 76.4%"与步骤 27 的"阈值桶台阶 5.8% → 3.3%、
带外逐位不变"，L4 的两条内容（近场由 SDF、远场由光追、切换连续）都有了**可断言的读数**。

**② 帧时与显存增量（`HE_DUMP_GI` 跑法，1920×1080，帧 120 后的滚动平均）**

```
Lumen GPU 耗时: 计算 pass 平均 27.110 ms / 峰值 2031.005 ms；输出贴图 pass 平均 0.091 ms
   拆分: SDF 构建 23.956 ms / 页表+捕获+反馈 0.274 ms /
         探针+追踪+着色+SH+辐照度 1.794 ms（其中远场光追 0.193 ms，每帧 45192 条射线）/
         SDF 调试视图 1.097 ms
```

- **峰值 2031 ms 是首帧**（101 个 mesh 的 128³ 距离场一次性构建）；稳态平均 27.1 ms 里
  **SDF 构建占 23.96 ms（88%）** —— 也就是说 L3/L4 的整条 GI 链路（探针 → 追踪 → 着色 → SH →
  逐像素辐照度）只花 **1.79 ms**，其中**远场硬件光追 0.193 ms / 45192 条射线**。
  ⇒ 优化目标非常明确：**下一阶段要压的是 SDF 的逐帧构建/流送，而不是 GI 本身**（这也是 §15.3
  预算表里 Mesh SDF 404 MB 的必然代价：几何一等大，逐帧重算就贵）。
- 输出贴图 pass 0.091 ms（一次全屏采样，可以忽略）。

**显存清单（`LumenScene::LogMemoryUsage()`，资源建齐后自动报一次）**

| 资源 | 尺寸 | 占用 |
| --- | --- | --- |
| Mesh SDF | 逐 mesh 128³ R16F | **404.00 MB** |
| Global SDF clipmap | 2 层 × (R32U + R32F) 128³ | 32.00 MB |
| Surface Cache atlas | 3 × 512² RGBA16F | 6.00 MB |
| Lumen 输出 + 辐照度纹理 | 2 × 屏幕 RGBA16F | 31.64 MB |
| 命中点 / 光线结果缓冲 | 2 × (65536×16 × 16 B) | 32.00 MB |
| 着色输出 × 3（atlas / GBuffer 对照 / 最优候选） | 3 × 16 MB | 48.00 MB |
| 远场切换 fade | 2 × 光线 × 16 B | 32.00 MB |
| 探针缓冲 / 远场结果 / 单元映射 / SH 参考 / 卡片与页表 | — | 约 11 MB |
| **合计（Lumen 自持资源）** | | **596.46 MB** |

与 §15.3 的设计预算对照，两条值得记下：

- **Mesh SDF 404 MB 与预算的 420 MB 吻合**（101 个 mesh × 4.2 MB）—— 这是 L1 的既定代价，也是
  帧时里最大的那一项。
- **Surface Cache atlas 只有 6 MB**（预算表按 4096² × 3 通道算是 402 MB）：当前是 64 个物理页
  × 512²，远低于设计上限。也就是说**现在的瓶颈不是 atlas 容量，而是覆盖率/内容**（页命中率 ~15%，
  见 §附三十三/三十四）。
- **诊断缓冲合计 112 MB**（命中点 32 + 着色输出 48 + fade 32），比"真正的 Lumen 数据"（atlas 6 MB）
  大一个量级。它们服务于步骤 22/23/27 的验收（albedo 对照、归因实验、切换权重），是**可回收**的：
  列进后续的显存优化项（不阻塞功能推进）。

**验收证据**：白炉 `prov6_final` min = mean = max = **1.0000**；`gi_blend_diffuse_lumen=0` ⇒
`lumen_passes=0`；`prov6_final` / `lumen_sc_atlas_albedo` / `lumen_sdf_trace` / `albedo`
**SHA-256 逐位一致**（本轮这一对运行里 RT 的最后一位抖动没有出现 —— 它是**间歇性**的：步骤 27/28
的对照组里出现 4 / 19 个像素、本次为 0，进一步支持"驱动级最后一位"的判断）；单测 **231 例 /
5757 断言**全绿；VUID 46 = 基线。

**L4 结论**：SDF 近 + RT 远混合成立且可断言（违例 0）；帧时与显存都有逐项记录。**阶段 E / L4 闭环**。

**下一轮**：进入阶段 F / L5 —— 步骤 30 探针表示升级（DDGI 三阶 9 系数 → 二阶 4 系数、RGB 独立），
随后 31 输入改为 Screen Probe + 时间混合、32–33 Radiance Cache 的其余部分。
### 附三十七：步骤 30「探针表示升级」——9 系数 → 4 系数，画面相关系数 0.99996、探针显存降到 1/4

**改动（三处必须同时改，少了任何一处都是"静默偏移"）**：

| 位置 | 改动 |
| --- | --- |
| `GI/DDGI.comp.slang`（投影端） | `kStride` 16 → **4**；`SHBasis` 只投影 bands 0/1（4 系数）；删除 `SH_Band2`；`WriteSH` 只写 4 个槽位；累积/归一化/时间混合全部按 4 系数 |
| `RT_DDGI.slang`（评估端） | `kDDGI_Stride` 16 → **4**；`EvalDDGI_SH` 去掉 `A_2` 的 5 项；`SampleDDGI` 只读 4 个槽位 |
| `GI_DDGI.h`（C++ 侧） | `kFloats4PerProbe` 16 → **4**（缓冲尺寸由它推出：1408 个探针 × 64 B） |

**为什么是这三处**：投影端写什么、评估端读什么、C++ 分配多大，是**同一个不变量**的三份表述。
只改其中一处不会崩，只会让画面悄悄偏离 —— 所以三处都在注释里互相点名。

**与置信度掩码的关系**：`kGIConfProbeGrid` 是"通道位 + 网格覆盖"语义（`GITypes.h` 的掩码位 +
`DeferredLighting.frag` 的网格内外判定），**与探针内存布局无关**，故本次升级不触碰它 ——
这正是计划里"保持与 `kGIConfProbeGrid` 置信度掩码一致"的含义。

**验收（两条都在同一对运行里量到）**：

| 口径 | 结果 |
| --- | --- |
| 白炉仍为 1.0000 | `gi_s30f_prov6_final.f16` min = mean = max = **1.0000** ✅ |
| DDGI 单独使用的画面不回归 | HDR 亮度均值 0.09129 → **0.09118（−0.12%）**；**逐像素相关系数 0.99996**；差值绝对值均值 7.6e-4、最大 0.038（24% 的像素有 >1e-3 的微小变化，空间上连续分布，不是块状错位）✅ |
| 相对基线的增量 | 9 系数 +0.01986 → 4 系数 **+0.01975**（差 −0.6%）—— band2 在本场景的 DDGI 贡献里只占 0.6%，砍掉它换来的 4× 显存很划算 |

**收益（L5 的第一步）**：探针缓冲 256 B/探针 → **64 B/探针**；本场景网格拟合后是 16×8×11 = 1408 个
探针 ⇒ **360 KB → 90 KB**。DDGI 探针是 Radiance Cache 的雏形，这一步同时把它的表示**收敛到与
Screen Probe 的 `shR/shG/shB` 同构** —— 步骤 31"输入改为 Screen Probe"就建在这个同构上。
评估端从 9 次乘法降到 4 次（低频漫反射场景里 band2 的贡献本来就小）。

**工具**：`gi_blend_diffuse_w1=1` 只开 DDGI（配合 `w0/w2/w3/rsm/lumen=0`）即可复现上面的对照。

**下一轮**：步骤 31 —— Radiance Cache 的输入改为 Screen Probe + 时间混合。
### 附三十八：步骤 31「输入改为 Screen Probe + 时间混合」——静态 21 帧收敛到 1%、轨道相机下读数不累积

**改动：投影流程一行没动，只换"逐样本辐射度从哪来"**

- `GI_DDGI::SetScreenProbeInput(...)`（新接口，与 `SetTracedRadiance` 同类）：把 Lumen 的
  **探针缓冲**、**16×16 单元 → 探针映射**、当帧相机 VP、屏幕尺寸与距离门限交给 DDGI。
- `DDGI.comp.slang`：每个 DDGI 探针先把自己的**世界位置投到屏幕**，落在哪个 16×16 单元就取那个
  单元归属的 Screen Probe；随后在它自己的 Fibonacci 采样方向上**求值该探针的 4 系数辐射度 SH**
  （$ L(d) = \sum l_i Y_i(d) $），作为逐样本辐射度进入原来的 SH 投影与时间混合。
  这就是"把输入换成 Screen Probe"的最小语义 —— 之所以能这么小，是因为步骤 30 刚把两边收敛到
  **同一个表示（二阶 4 系数、RGB 独立）**。
- **一帧延迟**：帧图里 DDGI 段在 Lumen 段之前注册，所以读的是**上一帧**的探针缓冲。这既避开循环
  依赖，也是"用上一帧屏幕信息"的标准做法（与本仓库其它 GI 源一致）。
- **单侧 → 整球**：Lumen 的探针只在法线半球采样（∫ 域 2π），DDGI 的投影在整球（4π）上做，
  直接代入只剩一半能量；乘 `kScreenProbeSymmetricScale = 2`（"看不见的那半边与看得见的一样"这一
  标准近似）补回立体角。方向性仍来自 Screen Probe —— 那才是本步要的东西。

**"history 重投影"在世界空间缓存里的含义（本步最实在的一处设计）**

DDGI 的探针钉在世界空间，**相机移动时不需要屏幕空间那种重投影**；真正需要防的是反向的问题：
Screen Probe 是屏幕空间的，相机一转，某个 DDGI 探针可能投到完全不同的表面上。两条规则合起来解决：

1. **深度一致性门限**（`FetchScreenProbe`）：比较"相机到 DDGI 探针的距离"与"相机到屏幕探针的距离"，
   差超过 `1 个格距 + 20% 相对松弛` 就不采纳 —— 同一条相机射线上同深度 ⇒ 大致是同一表面。
   **第一版用的是"屏幕探针与探针的世界距离 < 1 个格距"，实测采纳数恒为 0**（本场景格距 248 世界
   单位，网格点常常离任何可见表面都有几百单位）—— 这条被实测直接证伪并替换。
2. **没有可用屏幕探针就整条继承历史**（而不是写入一个"接近零"的新估计）。

实测抓取分步（Lumen + DDGI 同时在栈，静态相机）：**进入 1408 / 屏内 382 / 单元有探针 382 /
过深度门限 87**。
...
| 口径 | 结果 |
| --- | --- |
| **室内静态场景 30 帧内收敛** | 探针 SH 相对变化均值：帧 3 = 0.507 → 帧 8 = 0.097 → 帧 21 = **0.0079** → 帧 25 = 0.0040 → 帧 40 = 0.0060 → 帧 60 = 0.0047 ✅（**21 帧就进入 1% 以内**；第 31 帧有一次 0.0445 的小回弹，来自 Lumen 侧内容仍在变——页捕获/材质覆盖，随后再次收敛） |
| **相机移动时无拖影累积** | `HE_CAMERA_ORBIT=1`（6 秒圆形路径）跑 120 帧：相对变化**始终有界在 0.005–0.011，不随时间增长**；屏幕探针采纳数随视角变化（30→96），"继承历史"≈0；末帧 HDR 目视**无拖影/涂抹**（`s31_orbit_hdr.png`） ✅ |
| 换输入后的画面（静态 A/B） | traced 0.08957 → screenprobe **0.09501（+6.07%）**、**逐像素相关系数 0.99762**、`\|Δ\|` 均值 0.00679 ⇒ 同一个 GI、差异平缓（不是结构性变化） ✅ |
| DDGI 单独使用不回归 | **+0.01975**，与步骤 30 记录**逐位相同** —— 没有 Lumen 就没有屏幕探针，自动回落到自追踪路径 ✅ |
| 白炉 / Lumen 关 / 单测 | 白炉 `prov6_final` = **1.0000**；`gi_blend_diffuse_lumen=0` ⇒ `lumen_passes=0`；单测 **231 例 / 5757 断言**全绿；VUID 46 = 基线 ✅ |

**已知边界**：`HE_CAMERA_ORBIT` 按**墙钟** deltaTime 推进 ⇒ 该模式下画面不保证逐位可复现（静态模式
不受影响，背靠背仍是 16 个像素差 2 个 ULP，属步骤 27 起记录的 RT 最后一位抖动）。另外屏幕探针只
覆盖可见表面（387/1408 个探针投进视锥，其中 87 个过深度门限），所以**世界空间缓存里大部分探针仍在
用自追踪的兜底** —— 这正是步骤 32"插值与距离权重"与步骤 33 L5 判据要处理的方向。

**工具**：`HE_DDGI_INPUT=traced|screenprobe`（默认 screenprobe）切换输入来源；`HE_CAMERA_ORBIT=1`
打开相机动画。收敛读数在 `GI_DDGI::Render` 里逐帧打印（前 40 帧每帧，之后每 60 帧）。

**下一轮**：步骤 32 —— 插值与距离权重（三线性 + 距离权重，网格边界平滑；复核
`GIProbeGrid::FitProbeGridToBounds` 的拟合参数）。
### 附三十九：步骤 32「插值与距离权重」——两种插值方案都被实测证伪；格子感是**分辨率**造成的

本步的计划写法是"三线性 + 距离权重插值，网格边界平滑"。按既有习惯，**先建度量、再做改动**：
新增两个离线工具，把"格子状伪影"拆成两个可测的量。

| 工具 | 量的是什么 |
| --- | --- |
| `build/verify/grid_artifact.py` | **幅度**：把像素按网格相位（$ ((wp[a]−o) \bmod \text{cell})/\text{cell} $，原点未知时一维穷举取最大）分 8 箱，箱均值的标准差 / 总均值。测的是"值在格周期上有多大起伏" |
| `build/verify/crease_metric.py` | **缝**：对 DDGI 贡献图做水平二阶差分取绝对值，再按相位分箱 —— 线性 tent 在格平面上**一阶导数不连续**，那条"缝"在二阶差分上是尖峰 |

两者都用"**DDGI 单独 − 无 GI 基线**"的差值图：本场景有重复拱廊，画面自身在格周期上本来就有能量
（基线调制 64.6%），只有差值才是 DDGI 的东西。

**方案 A（计划里写的距离权重）被证伪**：`w = 三线性 / (dist² + bias)`（DDGI 论文里为"探针埋在几何里"
准备的）实测把网格对齐调制从 **35.67% 推高到 57.12%**。原因是它把权重进一步压向**最近的那颗探针**
⇒ 结果更像最近邻插值，而最近邻正是格子感的来源。**已回退。**

**方案 B（smoothstep 替换线性 tent，想让跨格一阶连续）实测无差别**：幅度调制 36.09%、缝强度 19.61%，
对照线性 tent 的 35.67% / 19.54% —— **两个指标都测不出改善**。既然拿不出证据，代码里也就不留
"改不动指标"的改动，`RT_DDGI.slang` 保持纯三线性（只留一段注释说明这两次证伪）。

**根因（本步真正的产出）：格子感是网格分辨率的函数，不是插值权重的函数。** 三次拟合的实测：

| 最长轴格数 | 格距（世界单位） | 探针数 | **网格对齐调制** | DDGI pass 平均 |
| --- | --- | --- | --- | --- |
| **16（当前默认）** | 248.06 | 1408 | **35.67%** | 0.036 ms |
| 24 | 161.78 | 4224（3.0×） | **22.42%（−37%）** | 0.039 ms |
| 32 | 120.03 | 9408（6.7×） | **20.11%（−44%）** | 0.052 ms |

⇒ 把 `ddgi_fit_cells` 从 16 提到 24，伪影幅度降 37%，代价是探针 3×（DDGI pass 仅 +0.003 ms，
但 `DDGI_Trace` 的射线数 45056 → 135168，按步骤 26 实测的"45192 条 = 0.19 ms"估算约 **+0.4 ms**）。
**默认值保持 16**（不改既有预设的画面与开销），把这条已经量化的建议写进文档；`ddgi_fit_cells`
本来就是 cfg 键，需要时一行改。

**拟合参数复核（计划的第二项）**：`FitProbeGridToBounds` 的日志现在把原点与末探针一并打出来，
可以直接对上：本场景包围盒 max (1799.91, 1429.43, 1105.43)、格距 248.06 ⇒ 末探针
(1799.91, 1609.96, 1297.76)：**最长轴（X）正好覆盖**，Y/Z 各多出不到一格（`ceil` 的结果，符合
$ \text{count} = \text{ceil}(\text{size}/\text{cell})+1 $ 的定义）。没有"覆盖不到"的轴。

**"网格外淡出正确"这一条在本场景是空验证**：实测 2 073 600 个有效像素里**没有一个**落在网格外
（自动拟合把整个场景包围盒都罩住了），所以这条路径根本没被走到。机制上它是 `kGIConfProbeGrid`
覆盖掩码（`DeferredLighting.frag` 的网格内外判定）在网格外把 DDGI 的置信度置 0，代码在、场景用不到
—— 如实记为"未被本场景覆盖的验收项"。

**验收证据**：白炉 `prov6_final` min = mean = max = **1.0000**；`gi_blend_diffuse_lumen=0` ⇒
`lumen_passes=0`；回退后的 DDGI 单独画面与步骤 31 **逐像素平均差 2e-6**（只有 625 个像素 >1e-3，
来自求和顺序与 RT 最后一位抖动）；单测 **231 例 / 5757 断言**全绿；VUID 46 = 基线。

**工具**：`build/verify/grid_artifact.py`（幅度）、`build/verify/crease_metric.py`（缝）；
DDGI pass 的 GPU 耗时随本步一起进了步骤 29 的计时日志。

**下一轮**：步骤 33 —— L5 退出判据（**室内/室外稳定 GI**；与"关闭 Lumen、仅 DDGI"的对照可解释）。
### 附四十：步骤 33「L5 退出判据」——室内/室外都稳定；与"仅 DDGI"的差按**材质可得性**归因

L5（§12）的判据两条：**① 室内/室外稳定 GI**；**② 与"关闭 Lumen、仅 DDGI"的对照可解释**。
"室外"用样例本来就支持的相机键（`cam_pos_*` / `cam_yaw` / `cam_pitch`）把相机放到建筑**上方外侧**
（`(0, 3200, 0)`、pitch −1.30），于是画面以天空/外立面为主 —— 无需改代码即可复现。

**① 稳定 GI（两种场景各一条读数）**

| 场景 | Lumen 覆盖 | 入射辐照度亮度均值 | 漂移 | 非法值 |
| --- | --- | --- | --- | --- |
| **室内**（默认相机） | 100.0% 屏幕 | 帧 40 = 0.43958 → 帧 80 = **0.44022** | **0.15% / 40 帧** | 输出无 NaN、无负值 |
| **室外**（建筑上方俯视） | 33.6%（只有几何，天空不产生探针） | 帧 40 = 0.42841 → 帧 120 = **0.42935** | **0.2% / 80 帧** | 输出 rgb min 0.0041 / max 1.639，**NaN 0、负值 0** |

- 室外的 33.6% 覆盖**正是正确行为**：天空像素在 GBuffer 里没有几何 ⇒ 没有探针、`alpha < 0` 走
  "本条无数据"的约定，合成端 skip（`s33_out_lumen.png` 里天空是暗的、只有两座塔有间接光）。
- DDGI 侧同时收敛到 0：探针 SH 相对变化 2e-5 → **0.00000**（帧 60 / 120）。

**② 与"仅 DDGI"的对照（同一相机、同帧号、只切漫反射层栈）**

| 场景 | 基线（无 GI） | 只开 Lumen | 只开 DDGI | 两者之差 |
| --- | --- | --- | --- | --- |
| 室内 | 0.07143 | 0.08748（**+0.01605 / +22.5%**） | 0.09118（**+0.01975 / +27.6%**） | Lumen 比 DDGI 低 **19%** |
| 室外 | 0.13380 | 0.13866（**+0.00486 / +3.63%**） | 0.13896（**+0.00516 / +3.86%**） | Lumen 比 DDGI 低 **6%** |

**差异的归因（这就是"可解释"要的东西）**：

- 室内差 19%，来自**命中点材质的可得性**：Lumen 的探针只覆盖可见表面（步骤 31 实测 1408 个探针里
  只有 87 个能拿到屏幕探针），命中点材质取自 Surface Cache，而页命中率只有 ~15% ⇒ 大部分命中
  返回中性值 0.18；DDGI 的 32 条光追射线直接取命中点出射辐射度，没有这层损失。
- 室外差缩到 6%：室外 GI 由**天空主导**，两条路径都从 IBL/天空取值（Lumen 的探针 SH 主要是天空
  辐射度，DDGI 的未命中射线直接查 IBL）⇒ 上面那条"材质可得性"的差异**自然消失**。
- 也就是说：**两者之差随"命中点材质在总能量里的占比"变化** —— 室内大、室外小，与预期一致；
  绝对量级（+0.005 ~ +0.020）在两种场景下都与"场景自身的间接光占比"同阶，没有异常放大或塌缩。

**验收证据**：白炉 `prov6_final` min = mean = max = **1.0000**；`gi_blend_diffuse_lumen=0` ⇒
`lumen_passes=0`；室内外两套相机下 Lumen 输出均无 NaN/负值；单测 **231 例 / 5757 断言**全绿；
VUID 46 = 基线（室外相机同样复核过）。

**L5 结论**：室内/室外 GI 稳定（漂移 < 0.2%），与"仅 DDGI"的差异可逐条归因到"命中点材质
...
### 附四十二：步骤 35「Lumen 信号接入框架」——探针噪声 −39.4%、画面高频 −21.6%；两种"均值修正"都没用；附带两处转储/重投影的真事实

**① 判据与做法**。这一步要的是"探针的 3×3 YCoCg AABB 空间滤波 + 时域 EMA，并且**登记为框架内信号、
历史走统一分配**"，而不是"再挂一套 Lumen 专用降噪器"。落地形态：

- **空间级**（`ScreenProbe_Filter.comp.slang`）：以**单元**（16×16 像素）为步长取 3×3 邻域（查
  `单元 → 探针` 映射），对每个 SH 系数在 **YCoCg** 里算 μ/σ，构造 AABB = $ [\mu−\gamma\sigma, \mu+\gamma\sigma] $，
  把每个邻居**截断**后均匀平均。合并在 2×2 单元上的探针会在邻域里被取到多次 ⇒ 等效于**按覆盖面积加权**。
- **时域级**：把探针世界位置用**上一帧** viewProj 投回上一帧屏幕 → 上一帧单元 → 上一帧探针；
  位置差 ≤ 容差（容差 = 比例 × 视图深度，透视投影下 `clip.w` 就是视图深度）且法线 $ \text{dot} \ge 0.9 $
  才采纳历史；历史值也截断到**同一个 AABB**（不截断就会把上一帧的旧估计拖进来 = 拖影）后做 EMA。
- **载体与所有权**：结果写进独立的 `m_ProbeFilteredBuf`（96 B/探针，下游只读它），历史是
  **两份**探针镜像（80 B/探针）+ **两份**单元映射（乒乓），全部由步骤 34 的 `DenoiseHistoryPool`
  分配 —— 池的日志从"4 张纹理"变成"**4 纹理 + 4 缓冲**，共 20428 KB"，"谁有多少历史"仍然只有一处答案。

**为什么必须读写分离 + 历史乒乓**：3×3 邻域互相重叠，就地读写的结果会取决于线程调度；时域级要在
同一次 dispatch 里"读上一帧的探针 j、写本帧的探针 i"，i≠j 一般成立但同块缓冲既读又写仍是竞态。
这两条都不是理论担心 —— §附二十四 的探针缓冲竞态就是这么来的（背靠背命中数一致、`l0` 均值不同）。

**② 三档模式 A/B（同一构建、同一配置，帧 80；`HE_LUMEN_PROBE_FILTER=off|spatial|full`）**：

| 模式 | 探针 l0 亮度 std/mean | 探针 l0 均值 | 逐像素辐照度均值 | 辐照度最大 | Lumen 输出 rgb 均值 | Lumen 输出 min | 画面高频代理 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `off`（= 没有这一步） | 0.3592 | 0.27163 | **0.44022** | 1.81325 | 0.03480 | −0.0270 | 0.002885 |
| `spatial` | **0.2332（−35.1%）** | 0.27394 | 0.42447 | 1.26506 | 0.03350 | −0.0046 | 0.002321（**−19.5%**） |
| `full`（默认） | **0.2177（−39.4%）** | 0.27485 | 0.42463 | 1.08617 | 0.03321 | **−0.0009** | 0.002261（**−21.6%**） |

逐像素标准差（Lumen 输出）同步从 0.02125 → 0.01735 → **0.01658**（−22.0%）。

**`off` 一档就是"改前行为"的直接证据**：它的逐像素辐照度均值 **0.44022** 与步骤 33（L5 退出判据）
在帧 80 记录的 **0.44022** 逐位相同 —— 也就是说"模式下等于没有这一步"不是承诺，而是读数。

**③ 两次"均值修正"尝试都没有把偏差修回来（实测，不是判断）**。AABB 截断对**偏斜**分布不是无偏的
（长尾被削掉），所以按 ReSTIR 的常规做法试了两条修正路径：

- **模式 3（keepmean）**：用"截断前/后的 Y 之和"的比把亮度均值补回来（限幅 0.5~2）。结果
  噪声 −39.2%（与默认档持平），探针 l0 均值 0.27400、Lumen 输出均值 0.03317 —— **没有回到 `off` 的 0.03480**。
- **γ = 2**（放宽 AABB ⇒ 截断通道数 93683 → 21145）：噪声 −39.2%，Lumen 输出均值 0.03313 —— **仍没回来**。

⇒ 两条都说明：**输出均值 −4.6% 不是 AABB 截断造成的**（截断量差 4 倍，均值几乎不动），而更像是
**空间平均本身**经过步骤 24 的"最近探针"重建之后的效应：探针层面的 l0 均值其实**升了 1.2%**
（0.27163 → 0.27485），到了逐像素却降 4.6% —— 差异只能来自"一个探针服务哪些像素"的权重结构
（合并探针覆盖 4 个单元、邻域平均把相邻表面的探针混在一起，若亮表面探针的像素权重低于暗表面，
加权后均值就下移）。**这条归因尚未被独立证实**，所以默认仍取设计文档写的"纯 YCoCg AABB"
（模式 2），γ/α/模式都留了环境变量；下一次复核只需把"按像素权重加权的探针均值"打出来即可**证实或证伪**。

**④ 白炉仍然精确（这是本步最硬的一条）**：白炉下所有探针的 SH 几乎相同 ⇒ σ ≈ 0 ⇒ AABB 退化成一个点
⇒ 截断与平均都是恒等，时域 EMA 两端也相同 ⇒ 恒等。实测：**滤波前后探针 l0 均值都是 1.77246**
（相对 $ \sqrt{\pi} $ 的偏差 4.00e-06，与步骤 23 的读数同值），噪声 0.0000 → 0.0000，
`prov6_final` min = mean = max = **1.0000**。也就是说这一步**不是靠"白炉走另一条路"来保标度的**，
而是核本身在白炉条件下退化为恒等。

**⑤ 两处"看起来有、其实没跑"的读数被修掉（都是步骤 34 埋下的伏笔）**：

1. **信号层只能描述纹理信号** ⇒ 探针 SH 是**缓冲**，登记时只能塞假纹理指针。现在 `DenoiseSignal`
   增加 `inputBuffer/outputBuffer` 与 `note`，`LogSummary` 打印 `Lumen_ScreenProbe [缓冲] 5649x1`
   + "核 3x3 单元 YCoCg AABB + 时域重投影 EMA"（而不是两个对它没有意义的双边 sigma）。
2. **`IsValid()` ≠ "本帧产出"** ⇒ 新增 `IGIProvider::ProducedThisFrame()`（按层栈判定，
   默认回落到 `IsValid()` 保持既有行为），SSGI/SSR/RT/Lumen 各自实现；**06.GILab 的转储判据同步换成它**。
   实测：默认配置（Lumen 与 RT 都不在层栈）下 `prov6_*`、`prov8_ao_*`、`prov9_spec_*`、`prov10_*`
   四组转储**不再落盘** —— 此前它们落的是"上一帧或从未使用"的纹理，而转储本身看不出这一点。
   这不是画面回归：同一次对照里其余转储（GBuffer / IBL / RSM / `ssr` / `radiance`）**逐位一致**，
   只有既有的 `prov0_ao_*`（SSAO）与 `hdr` 仍在抖动（附四十一已证其跨轮次存在）。

**⑥ 顺带记下两处既有的 resize 缺口（本步未改，留给后续排期）**：

- `RunProbePlacement` 的 `m_ProbeBound` **从不在 resize 时复位** ⇒ 单元映射缓冲 `m_CellProbeBuf`
  仍按旧视口的 `cellsX*cellsY` 分配，而着色器的 `dims.w` 用的是**当帧**的单元数 ⇒ 视口变大时
  `u_CellProbe[...]` 会写到缓冲之外（**潜在越界写**，1080p 固定视口下看不到）。
- `DeferredPipeline::OnResize` 既没有调用 `m_DenoiseHistoryPool.OnResize`，也没有调用四个
  `RTDenoiser::OnResize`（RT pass 自身也没有 resize 路径）⇒ 窗口尺寸变化后光追降噪器的历史纹理
  仍是旧尺寸。本次**故意没有只加池的 `OnResize`**：那会把 RT 降噪器持有的历史纹理释放掉而它们
  不会重建（use-after-free）。正确顺序是"先把 RT pass 的 resize 路径补齐，再让池随视口释放"。
  本步只做了自己范围内的部分：`LumenScene::OnResize` 会把探针滤波的两级资源、历史指针、
  历史标记与上一帧 viewProj 全部作废（单元网格变了 ⇒ 历史内容也不再对应同一批探针）。

**⑦ 验收证据**：白炉 `prov6_final` **1.0000**；`gi_blend_diffuse_lumen=0` ⇒ `lumen_passes=0`；
背靠背两次运行 Lumen 输出 `prov6_final` 仅 **1330 像素差 1 个 f16 ULP**（与步骤 27/28 同源的
RT 末位抖动），`lumen_sdf_trace`/`lumen_card_coverage`/`lumen_sc_atlas_albedo` 与 `radiance`/`ssr`/
RSM/GBuffer **逐位一致**；默认预设（无 Lumen）除既有 SSAO/HDR 抖动外逐位不变；
单测 **237 例 / 5792 断言**全绿（新增 2 例：缓冲类信号可判定、历史池在拿到设备前不分配）；
VUID 46（默认配置 41 ≤ 基线）。

**下一轮**：步骤 36 —— 半分辨率升采样与有效性（复用步骤 34 的 `DenoiseUpscale`；对象是 4 条
亚分辨率 RT 信号 —— RT Shadow/RTAO/RT 反射 = 960×540、**RTGI = 480×270** —— 以及探针侧的
低分辨率输出）。

### 附四十三：步骤 36「半分辨率升采样与有效性」——4 条 RT 信号重建到全分辨率；有效性掩码从"滤波副产物"变回"逐像素判定"；一个"全分辨率参照"被证伪；RTAO 链路自相矛盾

**① 做法：复用而不是新写**。步骤 34 已经交付了 `PostProcess/DenoiseUpscale` + `Denoise_Upscale.frag.slang`
（3×3 源邻域 × 深度/法线双边 × 目标空间高斯，且保住 `alpha<0` 语义），本步只是把它接到光追链的**出口**：
`RTEffectProvider` 的降噪链尾追加一级，启用条件按**实测尺寸**判（主输出 < 深度图 ⇒ 亚分辨率）。
它没有进 `m_Stages` —— 它不是"又一级滤波"，而是链的出口形状，放进 `m_Stages` 就得每帧增删。
接上之后 `FinalOutput()` 自动变成全分辨率 ⇒ **合成端与转储一行都不用改**（这正是步骤 34 把
"输出取链尾"做成唯一出口的回报）。

**② 实测：结构**（`gi_shadow=2;gi_blend_ao_w1=1;gi_blend_diffuse_w3=1;gi_blend_specular_w2=1`）：

```
RG pass: RT Shadow        → RT_Shadow_Denoise    → RT_Shadow_Upscale
RG pass: RTAO             → RT_AO_Denoise        → RT_AO_Upscale
RG pass: RT Reflection    → RT_Reflection_Temporal → RT_Reflection_Denoise → RT_Reflection_Upscale
RG pass: RTGI             → RT_GI_Temporal       → RT_GI_Denoise         → RT_GI_Upscale
统一降噪框架[GI]: 当帧信号 4 个 —— 有屏幕空间引导 4、需重建升采样 4、有时域速度 4
   RT Shadow      960x540 → 1920x1080 ... 需升采样
   RTAO           960x540 → 1920x1080 ... 需升采样
   RT Reflection  960x540 → 1920x1080 ... 需升采样
   RTGI           480x270 → 1920x1080 ... 需升采样      ← 1/4 分辨率
转储：prov9_spec_final 960x540 → 1920x1080；prov10_final 480x270 → 1920x1080
```

`HE_RT_UPSCALE=0` 时上表的 `*_Upscale` pass 与"需升采样"标记**同时消失**（同一构建、背靠背），
即开关确实作用在链的出口形状上，而不是"多跑了一个没人读的 pass"。

**③ 有效性掩码：从"滤波副产物"回到"逐像素判定"（本步最硬的一条）**。合成端读 alpha 的**符号**
判有效性（§10 的框架级契约）。旧路让合成端对**半分辨率**纹理做线性采样 ⇒ 读到的 alpha 是
**插值值**：实测 **3.84%** 的屏幕像素 alpha 既不是 +1 也不是 −1，也就是"有效性判定变成了滤波的
副产物"；新路逐像素输出精确 ±1（模糊有效性 **0.0000%**）。直接后果：RT 反射的 `alpha<0` 占比由
**85.61% → 81.86%** —— 找回的 3.75 个百分点正是"3 个邻居有数据、1 个没有"被插值拖到负值的边界像素。

| 形态 | 模糊有效性像素 | `alpha<0` 占比 | RT 反射 rgb 均值 | HDR 亮度均值 |
| --- | --- | --- | --- | --- |
| `HE_RT_UPSCALE=0`（旧：亚分辨率 + 线性采样） | **3.84%** | 85.61% | 0.02297 | 0.11353 |
| 默认（重建升采样） | **0.0000%** | 81.86% | 0.02753 | 0.11567（**+1.88%**） |

"画面无跳变"的读数就是这个 **+1.88%**（关掉 AO 通道后的净数）：它远小于"通道开/关"的量级，
而且差值在**边缘/平坦**上的 RMS 比只有 **1.21** ⇒ 这点变化的主体是"有效性覆盖被修正"，
而不是"边缘变锐"（如果引导重建真的在锐化，边缘上的差应当显著高于平坦处）。**因此本步不声称
"比双线性更准"** —— 见下一条，那是被实测挡住的。

**④ 一次被证伪的"参照"**。我加了 `HE_RT_FULLRES=1`（把四种效果切到全分辨率产出）想拿它当
重建精度的参照物，三档（全分辨率参照 / 半分辨率+双线性 / 半分辨率+重建）摆在一起量 RMS。
结果**参照本身不成立**：全分辨率档的信号水平与半分辨率档差得离谱 —— RT 反射 rgb 均值
0.03498 vs 0.02293（**−34%**）、RTGI 0.08282 vs 0.00991（**−88%**）、HDR 0.12558 vs 0.11358（**−9.6%**）。
也就是说 RT 效果的**估计量本身依赖分辨率**（射线/像素配比、miss 回退、逐像素随机流都会变），
"全分辨率 = 同一估计量的更细采样"这个前提不成立。于是"RMS 相对参照"这一列（rel 2.1~3.2）**没有意义**，
文档里不留这个数当结论，只留这条证伪：**要评判重建质量，必须先有一个分辨率无关的参照**（例如
"同一批半分辨率样本，分别用两种重建搬到全分辨率"），这是下一步（若还要做质量对照）的前置条件。
`HE_RT_FULLRES` 开关本身留下（它是一个可用的诊断入口），但不再声称它是参照。

**⑤ RTAO 这条链自相矛盾（既有缺陷，本步只记录不修）**。同一个配置下 RTAO 的三个转储互相打架：

| 转储 | 形态 |
| --- | --- |
| `prov8_ao_raw`（pass 自身输出，960×540 RGBA16F） | 全部为 0，另加 **12.5%** 的纹素 4 个通道全 NaN |
| `prov8_ao_final`（`HE_RT_UPSCALE=0`，= 时域降噪输出） | 与 raw 同形态（0 + 12.5% NaN） |
| `prov8_ao_final`（重建升采样后，1920×1080） | **1.0 全覆盖、0 个 NaN** |

前两行的 NaN 不是本步引入的：**步骤 34 的转储（`s34multi`）就是逐位相同的形态**（64800 个 NaN / 518400）。
第三行说明"重建链把 0+NaN 变成了 1.0"，这**既可能是** AABB/clamp 对 NaN 的语义顺手清洗，
**也可能**是那张升采样目标纹理根本没被写入（未初始化内存恰好是清过的 1.0）—— 两者用现有转储
分辨不了（需要把"降噪级输出"也落盘）。**因此本步的证据一律排除 AO 通道**，并把这条链列为待诊断项：
它的症状（"AO 恒为空/恒为无遮蔽"）在画面上表现为"没有环境光遮蔽"，不报错、不崩、不触发校验层，
属于本仓库反复出现的那类"看着有、其实没跑"。

**⑥ 顺带记下：`GITimer` 不覆盖辅助 pass**。四种效果的降噪/升采样 pass 都不在 `GITimer.Begin/End`
的范围内（帧图只给主 pass 计时），所以本步新增的 4 个全屏 pass 的代价在现有读数里**看不到**。
步骤 37（L6 退出判据 = 60fps）要做帧时账，必须先把这个缺口补上，否则预算表会系统性少算。

**⑦ 验收证据**：白炉 `prov6_final` min = mean = max = **1.0000**；`gi_blend_diffuse_lumen=0` ⇒
`lumen_passes=0`；**背靠背全部转储逐位一致（0 像素差，含 HDR）**；默认预设（无 Lumen、无 RT）
除既有的 `prov0_ao_*`（SSAO）与 `hdr`/`radiance` 抖动外逐位不变；单测 **237 例 / 5792 断言**全绿；
...
布局转换这条线已收口（46 → 42 行校验），剩余 7 条需要"各 pass 自报存储图像清单"这类通用机制。

#### 附四十四补记（第三步）：回读加速第二次尝试仍然失败，但把范围收窄到了"采样侧缺 UA→SR 转换"

在布局转换补齐之后（本附四十四补记第二步），重新试了"整块 `memcpy` + 双缓冲读上一帧槽位"
（预期 Lumen 计算 pass CPU 16.4 → 4.5 ms、整帧 51.5 → 35.3 ms）：**仍然整幅变黑**
（`lumen_irradiance` 与 `prov6_final` 的 rgb 全 0、alpha=1，而 CPU 侧统计一切正常）；
回退后立刻恢复（rgb 0.0338/0.0355/0.0303，与基线同值）。⇒ 两点结论：

1. **不是布局未定义行为引起的**：布局转换补齐后该现象照旧。
2. **范围收窄到"采样侧的布局转换"**：本轮补的是**写入侧**（`Undefined → GENERAL`），
   而这条纹理随后被**图形 pass 采样**（输出 pass 采样辐照度贴图）—— `GENERAL` 虽然与采样兼容，
   但"compute 写 → 图形采样"之间**没有任何 UA→SR 转换**，而 CPU 跑得越快、飞行帧越多，
   这条缺失的转换就越容易显形（逐元素读的 13 ms 恰好把 CPU 拖慢到"看不出问题"）。
   ⇒ 下一步应在 `DrawIrradiance` 前插入 UA→SR、并在下一帧写入前插回 SR→UA（atlas / 调试视图
   同理），**然后**再重试回读加速；在那之前不动回读实现（保持 12.95 ms 的已知代价，
   但读数确定、画面正确）。

#### 附四十四补记（第四步）：按上面的假设补了 UA→SR / SR→UA，仍然黑，且校验层反而升到 53

按第三步的结论动手：`RunProbeIrradiance` 每帧写前转 `ShaderResource → UnorderedAccess`
（首帧 `Undefined → UA`），`DrawIrradiance` 采样前转 `UnorderedAccess → ShaderResource`。
**结果：辐照度与输出仍然整幅为黑**（rgb 全 0、alpha=1），而**校验层从 42 升到 53 行** ——
也就是说这对转换本身就不被当前 RHI 接受（它有自己的布局跟踪器 `TextureLayoutTracker`，
逐 pass 手写转换很可能与它冲突：它认为图像在 A，我声明它在 B）。

**三点结论（都已实测，不是推测）**：
1. 回读加速（`memcpy`）与"黑辐照度"之间的因果关系是**稳定的**：三次独立尝试（单缓冲 memcpy、
   双缓冲 memcpy、双缓冲 memcpy + 布局转换）都复现，回退即恢复（rgb 0.0338/0.0355/0.0303 同基线）。
2. **不是**"写入侧缺 layout 转换"引起的（第二步修完照旧），也**不是**加两条手写 UA↔SR 能修的
   （第四步反而把校验行数推高）。
3. 因此这条线的正确入口在 **RHI 层**：布局应当由 `TextureLayoutTracker` 统一持有并按帧插入
   （而不是各 pass 手写 `PipelineBarrier`），否则"手工转换"与"跟踪器认知"必然互相打架。
   在那之前，回读保持逐元素实现（**12.95 ms/帧的已知代价**），画面与读数都正确。
   ⇒ **L6 的 CPU 瓶颈因此仍未解决**，但已经把范围收到"RHI 布局跟踪 vs 手写转换"这一处。

#### 步骤 37 工作暂停（收尾记录，2026-09）

按用户指示，步骤 37 到此**暂停**、不再继续推进，状态在进度表里标为 **⏸ 未达成：工作暂停**。
收尾时的确切状态：

- **功能**：步骤 1–36 与 38–41 已完成并有验收证据；步骤 37 里"接入 async compute（中段 compute 段）"
  这一**功能项未做**，摊销预算与分级画质也未做。
- **指标**：1080p/60fps **未达成**。GPU 侧已达标（各 pass 合计 10.4–14.3 ms ⇒ 70–96 fps、
  Lumen 计算 pass 稳态 3.3 ms）；整帧 CPU 受限（19–29 fps），最大项是**阴影 pass 的 CPU 录制
  28–33 ms**（既有、与 Lumen 无关）与 `Lumen_SDF_Build` 16.4 ms（内含 `RunFeedback` 回读 12.95 ms）。
- **下一次接手时的起点（按建议顺序）**：
  1. **阴影 pass 的 CPU 录制**（整帧最大项）：逐 mesh 的 `snprintf` + `SetDrawDebugLabel` 与
     "逐元素写未缓存主机内存"两处已定位到代码位置（`CSMTechnique::RenderCascade` 的 `rm` 闭包，
     `CSMTechnique.cpp:170-196`），下一步是给这两段各加计时把它们分开，再决定"调试标签移出内层
     循环/按需开关"或"对象缓冲改批量上传"。
  2. `RunFeedback` 回读的 12 ms：先把 Lumen 自持纹理的布局所有权交给 RHI 的 `TextureLayoutTracker`
     （而不是逐 pass 手写转换），再重试"确定性回读"。
  3. async compute 支持"中段 compute 段 + 跨队列 signal/wait"。
  4. 摊销预算与分级画质。
- **既有缺陷（不属 41 步，未修，均已记档）**：RTAO 链路自相矛盾（raw = 0 + 12.5% NaN，
  重建后 1.0 全覆盖）；SSGI 在本场景输出恒 0；SSAO/`hdr` 的运行间抖动（跨轮次既有）；
  探针单元映射缓冲在 resize 后可能越界写（`m_ProbeBound` 从不复位）；剩余 7 条 storage 布局校验告警。
- **未执行**：`git push` 全程未获授权。

#### 附四十四补记（步骤 37 第二轮）：自持存储图像的布局转换

上一轮那个"统计正常、图像整幅为黑"的形态，本轮找到了根因方向并做了一半修复：

- **根因方向**：Lumen 的内部 pass 之间只有**全局内存屏障**（`ComputeBarrier`：不传纹理、不做布局转换），
  而 Lumen 的纹理都是**自持**的（不经过帧图的瞬态资源管理，帧图因此也不会替它们转换）
  ⇒ 它们停在 `UNDEFINED` 上。校验层在提交时报
  `expects VK_IMAGE_LAYOUT_GENERAL — instead, current layout is VK_IMAGE_LAYOUT_UNDEFINED`
  （基线 46 行里有 **10 行**是它），而按规范此时的存储写是**未定义行为** ——
  这正是"着色器算了、统计写了、图像却是黑的"最合理的解释（本步的 `lumen_irradiance` 转储
  就是靠它把"没画"与"画了但采样到空"分开的）。
- **为什么"统一在首帧转一次"行不通**：mesh 场是**逐帧建出来的**（每帧 4 个，共 101 个），
  晚一帧再用 `Undefined → GENERAL` 转换会把**已经写好的场内容一起丢掉**（首帧转换只覆盖到当时
  存在的 1 张）。正确位置是**建纹理的当场**（首次写入之前）。
- **本轮做了一半**：`LumenSDF::BakeOne` 里建 mesh 场纹理的当场转换；共享中间纹理
  （`m_MeshScratch` / `m_MeshSeed`）在首次使用时转换（它们创建时没有命令列表）；
  `LumenScene` 首帧对当时已存在的自持存储图像转一次（atlas / 辐照度 / 调试视图 / 覆盖率）。
- **第二轮补齐**：clipmap 各层（scratch/field/seed）在**首次注入之前**转换、atlas 三张在
  **首次捕获之前**转换、辐照度纹理在**首次写入之前**转换 —— 三处同样遵循"创建点没有命令列表，
  就在首次写入的调用点上转"的口径。**实测：校验层 46 → 42 行**（`expects … GENERAL` 的告警
  10 → 7 条），图像与上一提交的差仍为 6819 像素 ≤2 ULP（转换本身不改变内容，只是把
  "未定义行为"变成"定义行为"），白炉 1.0000、单测 237/5792 全绿。**剩下 7 条的句柄落在
  另一段分配范围**（不在 Lumen 已登记的自持图像之列）⇒ 要定位它们需要各 pass 自报
  "自己把哪些纹理当存储图像写"，这项工作留在下一步（它是同一族缺陷的通用解法，
  而不是再逐张补转换）。
- **实测**：图像与基线差 **≤2 个 f16 ULP**（6819 像素）、白炉 1.0000、单测 237/5792 全绿；
  **校验层行数 46 → 42**（`expects GENERAL` 告警 10 → 7 条）—— 剩下的 7 条句柄不在 Lumen
  登记的自持图像之列，要定位需各 pass 自报"把哪些纹理当存储图像写"（同一族缺陷的通用解法）。