...
| 23 SH 投影 | ✅ 已完成 | `ScreenProbeSampling.slang`（追踪/投影**共用**采样与随机数，避免两个 pass 积到不同方向）+ `Lumen_ScreenProbe_SHProject.comp.slang` + `RunScreenProbeSHProject()`：每探针一个线程，把 `traceRep` 条光线投成 **4 系数 × RGB** 写回 `ScreenProbe.shR/shG/shB`（布局步骤 20 已定，本步不改结构）。估计量 `l_i = (2π/N)Σ L_j·Y_i(d_j)`（均匀半球 pdf = 1/(2π)）；辐照度 `E(n) = π·l0·Y00 + (2π/3)Σ l_m·Y_m(n)`。**采样由 GGX 换成均匀半球**（本步唯一取舍）：均匀 pdf 让白炉下 `l0 = 2π·Y00 = √π` **与方向、采样数无关** ⇒ "白炉误差为 0"可数值钉死；GGX 保留为 mode 0。另新增 `Lumen/LumenSH.h` C++ 镜像 + `Tests/TestLumenSH.cpp`（6 例 34 断言）把三条解析性质钉死：①`l0` 与方向/采样数无关；②整球白炉 `E(n) ≡ π` 且各向同性；③单侧白炉 `E(n) = π/2·(1+n̂·n)`。**实测（白炉）**：有效探针 5649、有效光线 45192（100%）、`l0 = 1.7724609` vs `√π = 1.7724539` ⇒ **相对偏差 4.0e-6**；白炉 `prov6_final` min = mean = max = **1.0000**。**踩坑**：SH pass 里按"页命中"再过滤会让每条探针只剩 0.9 条有效光线（步骤 22 的中性值语义要往下传），改为只按"是否打到几何"过滤后回到 93.7%；首次接线漏给布局加 `u_RayResult`（绑定 5）⇒ 有效光线读到 0 且 VUID 46 → 58。**验收**：白炉 1.0000、背靠背 dump 逐位一致、`lumen_passes=0`、单测 **228/5741** 全绿、VUID 46 |
...
**估计量**：光线是**均匀半球采样**（pdf = 1/(2π)），于是 `l_i = (2π/N)·Σ_j L_j·Y_i(d_j)`；
辐照度按 clamped cosine 卷积系数重建：`E(n) = π·l0·Y00 + (2π/3)·(l1·Y1 + l2·Y2 + l3·Y3)`。

**为什么把采样从 GGX 换成均匀半球（这是本步唯一一处取舍）**：步骤 21 用 GGX 半向量采样，
其 pdf 也能让估计量无偏，但**白炉下 0 阶系数没有解析值**（要除以 pdf 后求和）。
均匀半球的 `l0 = 2π·Y00 = √π` **与采样方向、采样数完全无关** ⇒ 步骤 23 的验收口径
...
| 口径 | 结果 |
| --- | --- |
| **白炉下 SH 的 0 阶系数** | GPU 回读 `l0 = 1.7724609`，解析值 `√π = 1.7724539`，**相对偏差 4.0e-6**（float 累加量级） ⇒ "误差为 0" ✅ |
...
1. **白炉下 l0 与方向、采样数无关**：任取法线与 `N ∈ {1, 8, 64}`，`(2π/N)Σ Y00` 恒等于 √π（相对误差 < 1e-12）；
2. **整球白炉**（= 引擎 `HE_FURNACE=1` 的语义，整球辐射度恒为 1）：`l0 = 4π·Y00 = 2√π`、`l1..3 = 0`
   ⇒ 带内重建 `E(n) ≡ π` 且**与方向无关**，这是"误差为 0"的最强形式；
3. **单侧（半球）白炉**：`l0 = √π`、`l_m = Y1·π·n̂_m` ⇒ `E(n) = π/2·(1 + n̂·n)`。
   这条把"**探针的白炉重建并不会自动给出各向同性的 π**"钉住 —— 单侧白炉本就是各向异性的，
   只在 `n = n̂` 时等于 π。写下来是为了避免后面把这条**带限/单侧**差异误判成实现错误。
...
- `Lumen_ProbeIrradiance.comp.slang`：每个屏幕像素按"**16×16 单元 → 探针**"映射找到归属探针
  ...
  兰伯特出射 `albedo/π · E(n)` 写进屏幕尺寸 RGBA16F 纹理。用像素法线求值等于免费拿到法线插值 ——
...
| 量级 | 亮度均值 **0.04507**、中位 0.03726、p90 0.07320（≈ `albedo/π·E`，与 albedo 均值 0.25 自洽） |
...
- **权重**：`w = smoothstep(t0, t1, t_sdf)`，`t0/t1 = 阈值 × (1 ∓ 半宽)`，默认阈值 50、半宽 0.2 ⇒
...
| 纯光追（两侧都命中且 `w == 1`） | 必须 `t_sdf ≥ t1`（远场以外） | **0** |
| SDF 未命中分支 | 必须真的 `sdfHit == 0` | **0** |
...
- `DDGI.comp.slang`：每个 DDGI 探针先把自己的**世界位置投到屏幕**，落在哪个 16×16 单元就取那个
  ...
  （`L(d) = Σ l_i Y_i(d)`），作为逐样本辐射度进入原来的 SH 投影与时间混合。
...
| 工具 | 量的是什么 |
| --- | --- |
| `build/verify/grid_artifact.py` | **幅度**：把像素按网格相位（`((wp[a]−o) mod cell)/cell`，原点未知时一维穷举取最大）分 8 箱，箱均值的标准差 / 总均值。测的是"值在格周期上有多大起伏" |
...
`count = ceil(size/cell)+1` 的定义）。没有"覆盖不到"的轴。
...
- **空间级**（`ScreenProbe_Filter.comp.slang`）：以**单元**（16×16 像素）为步长取 3×3 邻域（查
  `单元 → 探针` 映射），对每个 SH 系数在 **YCoCg** 里算 μ/σ，构造 AABB = [μ−γσ, μ+γσ]，
  把每个邻居**截断**后均匀平均。合并在 2×2 单元上的探针会在邻域里被取到多次 ⇒ 等效于**按覆盖面积加权**。
- **时域级**：把探针世界位置用**上一帧** viewProj 投回上一帧屏幕 → 上一帧单元 → 上一帧探针；
  位置差 ≤ 容差（容差 = 比例 × 视图深度，透视投影下 `clip.w` 就是视图深度）且法线 `dot ≥ 0.9`
...
（相对 √π 的偏差 4.00e-06，与步骤 23 的读数同值），噪声 0.0000 → 0.0000，
