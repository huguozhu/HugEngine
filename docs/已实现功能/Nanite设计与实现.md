...
> 最后更新: 2026-09-23（任务 1–27 全部落地；其后新增 §14.40–§14.42；2026-09-23 按画质阶段 0
> 的结论更新了「已知缺口」②，任务与判据本身未变）
> 状态: **设计定稿且实现完成** —— §14.8 的 27 项任务全部实现并通过验收；计划外的三项后续也已收口
> （§14.40 深度键平局确定性修复、§14.41 删除旧 API `NaniteAssignClusterMaterials`、
> §14.42 `materialBin` 生效的可行性核查与「明确不做」）。旧 §12 Task 1-10 的详细字段与判据仍有效，
> 对应关系见 §14.9
>
> **本文件已归档到 `docs/已实现功能/`**（原在 `docs/计划实现功能/`）：`HugEngineTests`
> **331 例 / 71291 断言**全绿（2026-09-23 画质阶段 0 第②项新增 3 例相机抖动断言后由 328/71231 增至）；
> `build/verify/acceptance_sweep.ps1 -OnlyNanite` 判据 ⑥⑦⑧
> **连续两轮 PASS**。**已知缺口（刻意未做，不是遗漏）**：
> ① `materialBin` **尚未**真正改变光栅遍历顺序（理由、代价与将来的前置见 §14.42）；
> ② 模块接管档的抖动族 `hdr` / `prov0_ao_*` 曾非逐位可复现，已独立验证与 Nanite 无关
> （Nanite 全关时呈现同一指纹）。**2026-09 画质阶段 0 已查明并修复根因**：SSAO 与 Blur 共处同一
> 离屏通道（attachment feedback loop）+ 帧图 `lightingReads` 漏声明 AO 输出；修复后同一构建连跑
> （OFF 3 趟 / ON 2 趟）**全部 18 个转储逐位一致**，详见 `docs/HugEngine引擎介绍/11.后处理链与抗锯齿实现分析.md` §7.3；
> ③ `#14` 覆盖率语义差的深度分桶未做；④ 验收脚手架未纳入版本控制、本仓库无 CI（§14.34 第 17 行）。
>
...
| 原引用 | 改写为 |
|---|---|
| 计划文档开头 `**Spec:**` 指向旧《Lumen + Nanite 完整设计规范》 | `**Spec:** docs/已实现功能/Nanite设计与实现.md`（本文件） |
...
**Spec:** `docs/已实现功能/Nanite设计与实现.md`（本文件；原文指向旧《Lumen + Nanite 完整设计规范》，
见 §0.3）
...
> **✅ 已修复（2026-09-21）**：本条正是 **§14.40** 收口的那一项 —— 三趟"确定性赢家选择"落地后，
> **4 张 GBuffer 在观察到的所有重复对里逐位相同**（修复前是"每次都不同"）。上面的记录保留为
> **修复前的事实**，不要据此认为现状仍是"并非逐位可复现"。
...
- 模块接管档**两次运行并非逐位可复现**（§14.31 ⑩，软光栅深度键平局由 UAV 写序决定）⇒
  ...
  > **✅ 该前置条件已解除（2026-09-21，§14.40）**：三趟"确定性赢家选择"落地后，接管档（阈值 64）
  > 的 **4 张 GBuffer 在同配置重复运行之间逐位相同**，"抖动族之外 0 项差异"这句话现在**在接管档也成立**
  > （抖动族 `hdr` / `prov0_ao_*` 仍按既有口径允许不同）。
...
- 模块接管档**两次运行并非逐位可复现**（软光栅深度键平局由 UAV 写序决定，§14.31 ⑩）⇒ 这是**既有问题**，
  ...
  > **✅ 已修复（2026-09-21，§14.40）**：接管档的 4 张 GBuffer 现在于重复运行之间**逐位相同**
  > ⇒ 这条限制**不再需要**；"抖动族之外 0 项差异"现在**在接管档也成立**。
...
3. 平局确定性**已在 §14.40 修复**（三趟确定性赢家选择）；`#14` 的深度分桶**明确不做**（§14.34 末尾第 4 条允许）。
4. 验收脚手架**仍未纳入版本控制**（§14.34 第 17 行第 ③ 条），本节的"两轮 PASS"仍无法从克隆重现。

### 14.40 软光栅深度键平局的确定性修复：三趟确定性赢家选择（2026-09-21）

> 本节收口 §14.31 ⑩ 记录的既有问题（"模块接管档两次相同运行并非逐位可复现"），
> 也解除了 §14.35 ④ 的那条前置条件（"要让'0 项差异'在接管档也成立，**必须先解决平局确定性**"）。

**① 问题（§14.31 ⑩ 的原文事实）**

- 深度键编码 = `(asuint(ndcZ) & 0xFFFFFF00) | (triLocal & 0xFF)`（`softRasterDepthKey`）：
  **24 位深度 + 8 位簇内三角形下标**。
- 第 2 趟（写 GBuffer）只做等值复检 `u_DepthKey[p] == key` ⇒ **两个不同簇**的三角形若深度位模式相同
  **且簇内下标相同**，键就完全相同 ⇒ 两个候选**都**通过复检 ⇒ **谁最后写谁赢**（由 UAV 写序决定）。
- 实测（§14.31 ⑩）：接管档同配置两次运行 **20 个转储里 7 个不同**（恰好 4 张 GBuffer + `hdr` + `prov0_ao` ×2）。

**② 为什么不加宽键（被否决的方案）**

把键加宽到 64 位（`深度32 | 身份32`）并做 64 位 `InterlockedMin` 是最直接的写法，但它要求
`shaderBufferInt64Atomics`（`VK_KHR_shader_atomic_int64`）**设备特性**，还得为不支持的设备准备回退路径。
本模块的设计纪律是**零额外设备特性**（见 `Nanite_SoftRasterDepth.comp.slang` 头部），因此否决。

**③ 采用的方案：三趟，用「稳定身份」做确定性仲裁**

在第 1 趟与第 2 趟之间插入一趟 B。三次派发**都录在既有 `Nanite_CullChain3` 的 pass 体内**
（帧图不为零帧图资源的 pass 排序负责）⇒ **帧图 pass 列表一个都没变**：

| 趟 | 着色器 | 做什么 |
|---|---|---|
| A | `Nanite_SoftRasterDepth.comp` | 逐簇逐三角形光栅化，`InterlockedMin(u_DepthKey[p], key)` |
| **B（新增）** | `Nanite_SoftRasterWinner.comp` | 重跑**完全相同**的光栅化；只对 `u_DepthKey[p] == key` 的像素做 `InterlockedMin(u_WinnerID[p], candidateID)` |
| C | `Nanite_SoftRaster.comp` | 重跑同样的光栅化；等值复检**再加一道** `u_WinnerID[p] == candidateID` 才写 GBuffer |

- **赢家因此唯一**：键最小者；键相同时候选号最小者。
- `u_WinnerID` 是 `R32_UINT` storage buffer（与深度键同尺寸），初值 `0xFFFFFFFF`
  —— **复用既有的常驻清零源 `m_DepthKeyZeroSrc`**（它本来就是一份全 `0xFF` 的镜像），
  多一次 `CopyBuffer` 即可，**不新增清零源**。
- winner 趟**复用第 2 趟的 `m_SoftColorLayout` / `m_SoftColorSet`**，只给布局加一个 binding 21
  ⇒ 不新建描述符集、不重做 bindless 登记。

**④ 候选号必须是「稳定身份」—— 这里踩过一次（必须记下来）**

第一版把候选号写成 `(gid.x << 8) | triLocal`，即用**可见簇列表的槽位**当身份。**这是错的**：
可见列表是剔除链用 `InterlockedAdd` 追加出来的，**槽位顺序由 GPU 线程调度决定、两次运行不同**
...
| 字段 | 上限常量 | 位宽 |
|---|---|---|
| `ref.cluster` | `kNaniteMaxBVHClusters = 16384` | 14 |
| `ref.instance` | `kNaniteMaxBVHInstances = 64` | 6 |
| `triLocal` | `kNaniteMaxClusterTriangles = 64` | 6 |

`candidateID = (ref.cluster << 12) | (ref.instance << 6) | (triLocal & 0x3Fu)` —— 合计 **26 位**，单射。
C++ 侧三条 `static_assert` 把上限钉住（`NaniteTypes.h`，注释写明"改大上限必须同步改着色器的位段"）。

**⑤ 实测证据（本机复跑，不是转述）**

| 档位 | 同配置两次运行的 `cmp_dumps.py`（多次重复对） |
|---|---|
| **阈值 16（默认）** | **4 张 GBuffer + 全部非抖动族转储 `identical`**；抖动族只差 **12 / 4 / 4** 个元素（`hdr` / `prov0_ao_final` / `prov0_ao_raw`，`maxULP` 5~6、`meanAbs ≈ 1e-9`）。**另一对重复对上抖动族是 0**（19 个转储全 `identical`）⇒ 抖动族**逐对变化** |
| **阈值 64（接管档）** | **4 张 GBuffer + 全部非抖动族转储 `identical`**；抖动族差 **44574 / 15116 / 15116** 个元素（`maxULP` 580~619、`meanAbs ≈ 1e-5`）；另一对是 **102146 / 34666 / 34666** |

- 唯一会不同的三个文件恰好就是 `acceptance_sweep.ps1` 自己界定为"**任意两次运行都允许不同（TAA/SSAO/HDR 抖动）**"的
  **抖动族**。⇒ **准确结论（按本项目既有口径）**：**"抖动族之外 0 项差异"**；
  **4 张 GBuffer 在观察到的所有重复对里从未出现过差异**。**不宣称**"接管档完全逐位可复现"。
- 与修复前对比（同一配置的重复对，由本轮实测取得）：
  - **阈值 16**：修复前同配置重复对差 **100** 个元素（`albedo 31 / gb_lightmapkey 27 / gb_normal 1 /
    gb_worldpos 9 / hdr 24 / prov0_ao ×2 各 4`）⇒ 修复后这 4 张 GBuffer **恒为 0**；
  - **阈值 64**：修复前 task 23 期的同配置重复对 `t23_soft64_r1` vs `r2` 差 **9865196** 个元素。
  - 另与 §14.31 ⑩ 的原始记录对照："20 个转储里 7 个不同，恰好 4 张 GBuffer + `hdr` + `prov0_ao` ×2"
    ⇒ **4 张 GBuffer 从"每次都不同"变成"逐位相同"**，这正是判据 ⑧ 稳定性的根源。
- 帧图 pass 列表指纹：**参与比对的四次运行全部**是
  `750CC247BF8B9C3DA2DEA6B7E893BED91E611145663CC037D0F198699F5C3E6F`（14 pass）
  ⇒ **pass 列表没变，冻结指纹不用重冻**。
...
**⑦ 顺带查实：判据 ⑦ 的"确定性"其实不覆盖画面**

`nanite_cull_diff.ps1` 的 determinism repeat **只比较 `cull3` 与 `visible_wiring` 两行**（不含 `soft_raster`）。
⇒ 既有判据 ⑦ 的"确定性"**不覆盖 GBuffer**；本次修复之后，才第一次能用"转储逐位比较"作为确定性证据。

**⑧ 未做 / 如实标注**

1. 抖动族（`hdr` / `prov0_ao_*`）在**两档都出现过**差异（阈值 16 是 0~20 个元素、阈值 64 是 7 万~17 万，
   逐对变化），已按既有口径允许。**它与本修复无关 —— 已独立验证**：把 Nanite **完全关闭**
   （`nanite_enable=0`，此时模块一个 pass 都不注册，见判据 ⑥a）的两对**同配置**重复运行，呈现
   **完全相同的指纹**（只有 `hdr` + `prov0_ao_final` + `prov0_ao_raw` 不同，其余 17 个转储逐位相同）：

   | 对照（两侧都 `nanite_enable=0`，cfg 逐字节相同） | `hdr` | `prov0_ao_final/raw` | 合计 |
   |---|---|---|---|
   | `t20_off` vs `t20b_off` | 118592 | 45207 ×2 | **209006** |
   | `tkdbg_off` vs `tkneg_off` | 754744 | 282181 ×2 | **1319106** |

   ⇒ 残留**不是** Nanite 引起的，**也不是** §14.40 引入的。**量级在不同对照之间差别极大**
   （7.5 万 / 20.9 万 / 131.9 万）⇒ 更像 HDR/SSAO 路径里的一处**竞态**而非固定数值漂移。
   **未再深挖**（在 Nanite 范围之外）；仅作为一条**未验证的线索**记录：本轮另有一次观察（由子代理报出、
   **我未独立复核**）称差异像素都落在几何覆盖区内且规模随覆盖量单调 —— 若将来要查，从这里入手。
2. **代价**：软光栅多一趟完整光栅化，外加一张与深度键同尺寸的 `R32_UINT` winner 缓冲
   （1080p 约 8.3 MB）。
3. 平局本身（多个候选键相同）**依然存在**，只是**赢家唯一**了；要减少平局得改键编码或光栅粒度，
   属新工作项。

**⑨ 单测与验收（本轮实测）**

- `HugEngineTests`：**exit 0**，`329 用例 / 71245 断言 / Status: SUCCESS!`
  —— 与本任务之前的基线**逐字一致**（本次没有新增/删除用例与断言）。
- `acceptance_sweep.ps1 -OnlyNanite`：**连续两轮均 `ACCEPTANCE SWEEP: PASS`**，且两轮数字**逐项相同**：
  - 判据 ⑥：6a `off passes=12 nanite_leak=0 sha=1C15AB72E688B530`；
    6b `passes=14 nanite_passes=2 preexisting_set_changed=False`；
    6c `pairs=20 differing_outside_jitter=0`
  - 判据 ⑦（`CULL DIFF: PASS`）：五档全 `OK`（`hiz1 mismatch=4 == occl_mip_sum=4` 且 `extra_gpu=0`）；
    `default` / `hiz1` 各跑两次**读数逐字符一致**
    —— 这也反过来确认了本条 ⑦：**该判据不比较 `soft_raster` 行**，所以 `depth_key_ties` 的
    顺序依赖不会打破它
...
### 14.42 `materialBin` 真正生效的可行性核查：**明确不做**（2026-09-21）

> 背景：§14.40 修掉平局确定性之后，"**先修根因再谈优化**"这个前提已经满足，本应随之让
> `NaniteSettings::materialBin` 真正改变光栅遍历顺序。本节给出核查结论：**明确不做**，并写清理由与代价。

**① 先看清"当前顺序"到底是什么**

`LogMaterialBinReadback` 里 `material_switches`（当前口径）数的是**相邻可见簇引用之间的材质切换**，
顺序取自 `visible`（GPU 可见簇列表）中每个 `ref.cluster` 的先后；参照数 `material_switches_bin`
是把**同一批**可见簇按 bin 位次重排后的切换数（CPU 侧算出的**假设值**）。

**② 关键发现：这个顺序不是任何"遍历序"，而是"竞争序"**

可见簇列表由剔除链用 `InterlockedAdd` **并行追加**产生 ⇒ 它的先后由 **GPU 线程调度**决定，
既不是"资产顺序"、也不是"BVH 遍历顺序"。**§14.40 的候选号教训正是这同一条事实的另一面**
（我第一版就是误把它当成了稳定身份）。⇒ 想靠"重排资产簇顺序"或"重排 BVH 叶子表"去影响它
**在原理上不可能**：叶子表怎么排，追加序仍是竞争序。
**要真正生效，唯一的路是每帧在 GPU 上显式把可见列表按材质重排。**
...
- **值多少（两项都指向"不值"）**：
  1. **收益机制在软光栅路径上不存在**：软光栅走 **bindless** 材质
     （`softRasterEvaluateMaterial(cluster.materialID, …)`），**没有 per-draw 描述符切换** ——
     这也是本文档此前一直把 `descriptor_switches` 记为**结构性上界**（而不是被计数的事件总数）的原因。
     按材质分组剩下的唯一收益是**采样/纹理访存的局部性**，而它**从未被测量**。
  2. **整帧是 CPU 受限**：§14.36 的实测结论是 GPU 9~43 ms 对墙钟 126~146 ms
     ⇒ 即便 GPU 侧局部性变好，也换不成帧时收益。
- ⇒ **结论：明确不做。** 本项保持为**只读的收益证据**（`material_switches` 当前序 **361** 对 bin 序 **41**，
  以及 `clusters_per_material` / `descriptor_switches`）—— 它回答的是"按材质分组这件事**值多少**"，
  而不是"要不要现在做"。

**④ 将来若要做，前置是什么**

1. 先给**测量计划**：证明软光栅的材质采样/纹理访存确实是热点（profiler 的缓存/访存计数，
   或在阈值 64 档做 A/B 的帧时与 pass 时对比）。
2. 再实现排序，且排序要**确定性**、并纳入"重复运行逐位可复现"的验收（§14.40 的同一把尺）。
3. 届时 `materialBin=0/1` 的画面必须**逐位相同** —— 这条**现在是可判定的**（§14.40 之前**不可判定**，
   因为那时画面本身就不逐位可复现）。这是"先修根因"带来的直接好处，即使优化还没做。

**⑤ 本项原定验收证据的现状（如实标注）**

- "`material_bin=0/1` 画面逐位相同"：**方法已就绪**（§14.40 让它可判定），但**没有可判定的对象**
  —— 开关目前根本不改变顺序；
- "`material_switches` 下降"：现在只有**假设值之间的差**（361 → 41），**不是**实测的运行期切换数下降
  ⇒ **不能**当作"优化已生效"的证据。
