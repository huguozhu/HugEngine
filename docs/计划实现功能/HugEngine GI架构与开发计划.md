...
| **T** | 低 | **「效果本次未产出」时描述符仍绑定真实纹理**：`LightingPass` 无条件把 RSM 位置/通量图、SSGI 输出、聚光灯阴影图等绑到 set=0，而这些 pass 的门控是「是否在层栈里 / 是否有该类光源」⇒ 这些描述符指向**从未被写入**的纹理。当前无害（着色器按权重与光源数自行守卫），但只要守卫被改动就会读到未初始化显存 —— 正是 S 所描述的危险形态。**由 S 的检测机制在 06.GILab 首次跑出**，固定 4 处，清单见「S 的修复与实测」 | `LightingPass.cpp:300-333`；归 §10.1 任务 23 |
...
的形态。归 §10.1 任务 23：**告警只有在基线清零后才有信噪比**。
...
**A 组 · 先让验收可信**（第 1 项已完成；第 2、22 项未完成前，其余验收仍缺判据）
...
| **23** | **§9.2-T · 未产出的效果改绑占位纹理** | 小 / 中 | 22 完成后，检查跑出的 4 处「描述符指向从未写入的纹理」全部属「效果本次未产出」。逐一改为绑定占位纹理（或让该 pass 无条件产出确定值），把 22 的告警基线清零 —— **告警只有在基线为空时才有信噪比** |
...
| **消费者门控写漏 → 未初始化纹理被采样** | **静默的物理错误 + 读数跨构建不可复现**（本次 §9.2-Q/R 正是如此被放大的） | 已落地 RHI「已写入」登记 + 一次性告警（§9.2-S）：任何被采样却从未写入的纹理都会报出尺寸/格式/binding；固定基线 4 条待清零（§9.2-T / 任务 23）。且 GI 源**吃进去**的中间量也要纳入纹理级对照（§11.3.1） |
