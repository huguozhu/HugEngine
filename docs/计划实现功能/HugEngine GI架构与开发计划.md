...
>
> ⚠️ **再更正一处（任务 14 的实测，2026）**：本节下方把 DDGI 描述成"探针网格上的平滑多次
> 弹射解"——**当前默认路径下它不是**。RSM 不在漫反射层栈时 `u_Flags.x=0`，`DDGI.comp.slang`
> 的 IBL 回退只按 `dir` 采样辐照度、根本不用 `samplePos`，而 Fibonacci 方向对每个探针相同
> ⇒ 整片探针的 SH 逐位一样（实测：格距 531 / 248 / 120 三种拟合的画面贡献两两相差 0.0015%，
> 见 §10.2 任务 14）。也就是说这一列 DDGI **是一个不含场景空间信息的方向函数**，
> 而不只是"低通过的 GI"。这反而把"两源弱相关"解释得更彻底；**归一化仍是正确的合成方式**
> 这条结论不受影响。让探针网格真的带空间信息是任务 17。
...
| DDGI | compute | 探针数**由场景包围盒自动拟合**（任务 14 起；Sponza 实测 16×8×11 = **1408 探针**、格距 248，改前固定为 8×4×8 = 256） × `numSamples` 次单步采样。✅ **任务 12 起可按 `updateStride` 时间维分摊** | **0.020～0.021 ms**（256 探针时为 0.019～0.025：探针数 ×5.5 后读数没有可判别的变化 —— 这个 pass 本来就只占百分之几毫秒） |
| SSGI | 全屏 offscreen | 全屏或**半分辨率** × 16–32 采样 | 16 采样 **0.436 ms**，64 采样 **1.271 ms**（随工作量线性） |
| SSR | 全屏 offscreen | 全屏或半分辨率，Hi-Z march | 0（§9.2-W：恒 miss，实际不产出） |
...
| `AS_Build` | TLAS 重建 | **整个场景每帧重建**（只要开了任一 RT 源） | **0.088 ms**（比任何单个 RT 效果都大，但绝对值仍很小） |
...
> **RSM 在层栈里时另算**：它的两个 pass 合计 0.18 ms，把 Lighting 从 0.43 抬到 0.51 ms
> —— 而它对画面的贡献目前是 0（§9.2-AA），这部分成本是纯支出，直到任务 30 修好量级。
...
| **W** | **高** | **SSR 一个命中都没有**（做 §9.2-E 的 A/B 时顺带发现）：新增的镜面通道落盘显示 SSR 输出**逐像素全为 0**，且有效性 alpha **100% 为 −1**（= 全部 miss），改前改后皆然。因此镜面层栈里"有 SSR"与"没有 SSR"在画面上完全等价 —— 与 §9.2-L（SSGI 恒为 0）同一种失效形态：**开关为真、pass 在跑、纹理在写，但内容恒为空**。**已定位到分支**：把 `u_UseHiZ` 强制为 0（走线性 march）后同一配置立刻产出 **34.66% 有效命中**，说明**坏的是 Hi-Z 层次 march 这条默认路径**。原因是**深度约定与引擎相反**：Hi-Z 是 `min` 深度金字塔（最近），而着色器按 **reverse-Z**（近=1/远=0）判断"`射线深度 > Hi-Z` ⇒ 射线在几何之前 ⇒ 大步推进"；引擎实际是 **zero-to-one**（近=0/远=1，同一着色器的天空判据 `depth >= 1` 就是证据，`GPUCull` 的遮挡判据也按同一约定）。方向反了之后射线一路"推进"到 `t > 1` 直接跳出，永不判命中。线性回退路径的命中判据同样可疑（`rayPos.z > rpZ`——射线还在几何之前——却直接记为命中 0.5） | `GI/SSR.frag.slang:77-93`（Hi-Z 分支）、`98-114`（线性分支）；`Culling/HiZDownsample.comp.slang`（min 金字塔）；最小复现见 §10.1 任务 25 |
| **X** | 中 | **镜面层栈为空时画面出现 463 量级的异常亮点**：同一场景、同一相机、只改层栈，三个配置的 HDR 读数为 —— 三通道全空 `mean 0.2022 / max 463.32`；只放 `specular={IBL}` `mean 0.0695 / max 42.20`；只放 `ao={SSAO}`（specular 仍空）`mean 0.2022 / max 463.32`。即**亮点只在 specular 层栈为空时出现**，一旦有任一镜面源（哪怕是世界空间的 IBL 预滤波，且该像素不需要它）就消失；位置固定在画面内（本相机下 (416, 996)），量级相当于直接光高光的几十倍。**排除**：不是 albedo/半分辨率/白炉（未开炉）、不是后处理（读的是 Lighting 的 HDR 目标，在 tonemap 之前）。**推断方向**：与"通道计数为 0 时 `specNum/max(specDen,1e-4)` 仍参与、或空通道时着色器对某纹理的采样未守卫"有关，需按 §9.2-S 的"未写入纹理"思路查一遍空通道分支 | 复现配置与读数写在 §10.1 任务 27 |
| **Y** | 中 | **同一份「写进配置文件的键」在不同写法下落到了不同的兜底路径**：`dump_gi.ps1` 与标定脚本都要产出「漫反射层栈为空」的基线，两者写出的 `gi_blend_diffuse_w0..w3` 全为 0 的配置**逐字节等价**，但实测基线稳定地分成两组 —— 0.05720（`dump_gi.ps1` 与手工复制同一文件的运行，三次一致）与 0.07554（标定脚本的 cfg，文件在运行后被示例程序回写为 `gi_blend_diffuse_w0=1.000000`）。后者说明该次运行里空层栈被**兜底补上了 IBL**（`GIRegistry::Degrade` 的兜底），前者没有。⇒ **"空层栈"这个配置状态本身不可靠**：同一份配置能落到两种不同的有效层栈，而读数相差 32%。这会直接污染任何以"空栈"为基线的做差实验（本轮双源判据因此改为全部走同一份采样设施）。**后续证据（任务 11.1 的 A/B 期间取得，把范围缩小了）**：同一个**二进制**、同一份**字节内容相同**的 cfg 连跑四次，得到 0.057733 / 0.057194 / 0.057193 / 0.057166 —— 组内一致（±0.05%）、组间差 **1%**；改前二进制的同配置读数落在第二组。也就是说它至少有"同二进制同配置也分成两组"的一面，更像与**运行状态**（首次运行 / PSO 缓存预热 / 帧时序）相关的效应，而不是单纯由配置文件内容决定。要查清的是：该效应从哪条链路上来（提示：与帧时序相关的东西只有异步计算提交计数与时间维状态），以及**空层栈到底该不该被允许** | 复现见 §10.1 任务 28；另见 §11.3 的采样注意事项（每次对照必须用私有 cfg 副本） |
| **AA** | **高**（部分已修） | **RSM 间接光整条链路实际不产出**（任务 16 的测量中发现）。实测指纹：`S_rsm = lum(HDR{diffuse=RSM}) − lum(HDR{空漫反射栈})` 在**三个不同构建**上都只有 **1e-8 量级**（噪声底），而同一配置下 Lighting 的 GPU 耗时从 0.433 ms 涨到 **0.882 ms** —— 即**这段 VPL 求和在真实地跑，结果却什么也没进画面**，与 §9.2-L/W 同一种失效形态（开关为真、pass 在跑、成本在付、内容恒空）。根因不止一个，按影响排序：① **通量读错了缓冲**：`GI_RSM::RenderRSMPass` 把**对象缓冲**同时绑到 `binding 1 (u_Lights)` 与 `binding 2 (u_Objects)`，于是 `RSM_Generate.frag` 里的 `u_Lights[0]` 实际是 `GPUObjectData[0]`（世界矩阵被当成颜色/强度解释），通量是人造值。**已修**（新增 `GI_RSM::SetLightBuffer`，Deferred/Forward 两个调用点都传真实光源缓冲；同时把 `CollectLights` 提前到帧图开头，否则 RSM 只能读到上一帧的光源数据）。② **受光项的量级是场景尺度相关的**：累加项是 `flux · cos·cos / d²`，而本场景的接收点与 VPL 相距数百到数千世界单位 ⇒ `1/d² ≈ 1e-6`，再乘经验常数 `RSM_VPL_ENERGY = 0.046875`（那个常数是按 **25/16 点积分**归一、并隐含"场景约 60 单位"的假设推出来的）⇒ 整项落在 1e-8 量级、低于 16 位浮点转储在该亮度下的分辨率。**未修**（需要 VPL 面积/能量项与一次标定，属独立任务 30）。③ 生成端与消费端对 `flux` 的**通道约定不一致**：`RSM_Generate.frag` 写的是 `(N·0.5+0.5).rgb + flux.a`，新建的 `RSM_Indirect` 按这个约定读（rgb=法线、a=通量），而 `DDGI.comp.slang` 的探针更新把 `flux.rgb`（其实是编码法线）当辐射度用。④ 生成端注释说通量含 `albedo * lightColor`，实际只写了 `intensity * NdotL`（标量、没有颜色）⇒ 这一项目前只能是灰度的。**修 RSM 质量时应四项一起过一遍** | `GI/GI_RSM.{h,cpp}`（①②）；`Pipeline/DeferredPipeline_FrameGraph.cpp`、`ForwardPipeline.cpp`（①）；`GI/DDGI.comp.slang`（③）；`Shader/GI/RSM_Generate.frag.slang`（④）；复现与判据见 §10.2 任务 16 与 `Tools/gi/rsm_indirect_check.ps1` |
...
| **2** | **D1 · 偶发崩溃根因获证** | 未知 / 中 | 根因未证意味着已修项可能只是其中一个实例；长跑类验收会被它污染。长跑 soak + `HE_TRACE_FB=1`。**本轮已修掉同类的偶发卡死（acquire 信号量复用，1/24 → 0/40）并补齐 PDB + soak 工具；但访问违例 64 次零复现，且 `HE_CRASH_TEST` 自检路径失效（不产 dump、不进过滤器）——先把证据链修好** |
...
| **14** | ~~**§9.2-K · DDGI 网格外退化为贴边常数外推**~~ —— ✅ **已完成** | 低 / 低 | 探针网格为固定参数，覆盖不到的区域**静默缺失**低频 GI 且无 falloff / 无效标记。两半一起做：覆盖置信度位 + 网格按场景包围盒自动拟合。**判据**：固定网格下贡献 0.019595 → 噪声量级（2.9e-08 / −3.9e-07）；拟合后回到 **0.019596**、与改前逐位一致；拟合规则由单元测试覆盖（166 例 / 4189 断言全过）；新增回归检查 `Tools/gi/ddgi_grid_check.ps1` 4 配置 5 判定全过。**顺带量化了任务 17 的提升空间**（见 §10.2） |
...
| **17** | **B4 / M5.2-A · DDGI 光追 march** | 中 / 中 | 只提升单一源质量。第 1 项已解决（§11.3.1），DDGI 的读数现已确定性可复现，可以安全改动。**任务 14 把这个源的"提升空间"量化了**：当前默认配置（RSM 不在漫反射层栈 ⇒ `u_Flags.x=0`）下，`DDGI.comp.slang` 的 IBL 回退路径只按 `dir` 采样 `u_IBLIrradiance`、**完全不用 `samplePos`**，而 Fibonacci 方向对每个探针都一样 ⇒ **整片探针场的 SH 逐位相同**。证据：格距 531 / 248 / 120 三种拟合（192 / 1408 / 9408 个探针）下 DDGI 的画面贡献**两两相差 0.0015%**（`Tools/gi/ddgi_grid_check.ps1`）⇒ 这个源当前等价于"IBL 辐照度的方向函数"，探针位置没有进入结果。要让网格真的带空间信息，必须在探针更新里做真实的可见性 march（本任务） |
...
| **28** | **§9.2-Y · "空层栈"这一配置状态不可靠（兜底把 IBL 悄悄补回）** | 中 / 中 | 同一份逐字节等价的配置（漫反射四个权重全 0）在两次实验中落到两种有效层栈：一次保持空（基线 0.05720，三次可复现），一次被兜底补成 `{IBL}`（0.07554，且运行后示例程序把配置文件回写为 `gi_blend_diffuse_w0=1.000000`）。差别达 **32%**，足以污染任何"以空栈为基线"的做差实验。要做的是把这条路径查清并消除二义性：`GIRegistry::Degrade` 的兜底在什么时机、对哪个对象生效？为什么两次运行不同？以及**空层栈到底该不该被允许**（若不允许，就把它变成一个显式的、可复现的状态，而不是看运气）。判据：同一份配置连续 5 次运行必须给出同一个有效层栈与同一个读数。**已排除一个候选成因**：任务 15 把 Lighting 的逐帧轮换资源改成每飞行帧一份描述符集/UBO 之后，两组读数**照样**出现（改前/改后各跑 4 次：改前 0.05719370/0.05773345/0.05719362/0.05717655，改后 0.05719373/0.05719364/0.05773368/0.05773326，都是组内 <0.01%、组间约 1%）⇒ 跨帧重绑不是它的原因 |
...
| **30** | **§9.2-AA · RSM 链路的量级与通道约定**（任务 16 发现） | 中 / 中 | RSM 间接光**整项不产出**（`S_rsm` 恒为 1e-8，低于 16 位浮点转储的分辨率），而成本照付 0.188 ms/帧 —— §9.2-L/W 同一族的第四例。四项一起过：① 受光项 `1/d²` 在本场景尺度下衰减到 1e-6（经验常数只做了 VPL 数量归一，隐含 60 单位场景的假设）；② 生成端与 DDGI 消费端对 `flux` 的**通道约定不一致**（DDGI 把编码法线当辐射度）；③ 通量只有标量、没有 albedo 与光源颜色（注释与代码不符）⇒ 只能是灰度；④ 光源视锥仍硬编码 `sceneCenter=(0,3,0)`/`sceneRadius=60`（任务 14 已能算场景包围盒）。判据与半分辨率保真度的重测要求写在 §10.2 |
...
- **7（E · SSR/SSAO 硬编码投影）**：照 SSGI 已修的路径（`SetCamera` + UBO 补 `u_View`）。
  ...
  > 顺带发现的 **§9.2-W（SSR 恒 miss）** 让 SSR 这一半无法用"合成结果变化"来验证 ——
...
- **11.1 · 去重 + 参数可配**（小 / 低，**有即时收益**）—— ✅ **已完成**
  - 做了什么：把 `SSGIProvider` / `SSRProvider` 里逐行同构的附属 pass 合并成
    `GI/SpatialDenoiseAux.h` 一份实现（数量/输入/输出/PreBind/Render 全在一处）；
    给 `Denoiser` 补 `SetDepthSigma/SetNormalSigma`，并在**管线的一处**按信号赋值
    （此前连"改哪里"都不存在）。顺带修掉 `Denoiser.cpp` 里一行乱码注释。
  - **参数取值为什么仍是 10 / 8（实测结论，不是漏改）**：把 SSGI 放宽到 2 / 2
    （双边权重更不"挑边"、邻域更多参与）后，高频代理 `mean|Δx|` 只从 0.000984 降到
    0.000935（**−5%**），整体 `std/mean` 反而 1.7921 → 1.8137（基本不变），而收敛后的
    HDR 读数偏移 −1.5%。⇒ **瓶颈是 5×5 的核本身与缺少时域累积，不是这两个权重**；
    要真正"按信号取值"得先有 11.3 的框架和一条质量判据，否则就是拿一个量不出来的旋钮
    冒充改进。这条实测也直接指导 11.3 的选型（该往"更大的核/时域"走，而不是调权重）。
  - 判据（背靠背单源采样逐项一致）：见 §10.1 的 11.1 行 —— `ssgi` 与 `both` 两个变体
    在改前/改后二进制之间相差 1e-3 个百分点量级，**在抖动内**；`none`/`ddgi` 上出现的
    0.09%~1% 差异与本次改造无关（SSGI 不在其层栈里，且同一二进制同一 cfg 连跑四次即
    分成两组，属 §9.2-Y）。白炉 1.0000、单元测试 161/161 与 3941/3941、其余检查全过。
  - **过程教训（已写进 §11.3 的采样说明）**：A/B 对照必须给每次运行一份**私有 cfg 副本** ——
...
**25 · §9.2-W SSR 的 Hi-Z march 恒 miss**

- **现象与指纹**：`specular={SSR}` 且其余层栈为空时，SSR 的输出纹理**逐像素 RGB=0、
  有效性 alpha 100% 为 −1**（= 全部 miss），因此 HDR 与"镜面层栈里没有 SSR"逐位相同。
  与 §9.2-L 同类：开关为真、`IsValid()` 为真、pass 在列表里、纹理在写 —— 但内容恒为空。
- **定位方式**（可复现）：把 `GI_SSR::Render` 里 `pc.useHiZ` 临时强制为 0（走线性 march）
  重编，同一配置立刻得到 **34.66% 有效像素**（`mean_rgb` 从 0 变为 9.3e-3）。
  ⇒ 坏的是 Hi-Z 这条**默认**路径（`m_HiZTex` 非空时恒被选中），线性回退路径能出结果。
- **根因**：深度约定不一致。Hi-Z 是 `min` 深度金字塔（`Culling/HiZDownsample.comp.slang`
  取 2×2 最小值），而 `SSR.frag.slang` 按 **reverse-Z** 判断（注释写"近=1.0 / 远=0.0"）：
  `rpNDC.z > hiZ ⇒ 射线在几何之前 ⇒ t 前进、level+1`。引擎实际是 **zero-to-one**
  （`glm::perspectiveRH_ZO`，近=0/远=1）—— 同一着色器自己的天空判据 `depth >= 1` 就是证据，
  `GPUCull` 的 `IsOccluded` 也按 larger=更远 解释。于是判据方向反了：射线每一轮都"前进"，
  `t > 1` 直接 break，永远走不到命中判定。
- **修法**：按 zero-to-one 改写 Hi-Z 分支（射线深度**小于**该层最小深度才说明在几何之前、
  才允许大步推进，否则回退细化），并同时复核线性分支的命中判据
  （`rayPos.z > rpZ` 表示射线仍在几何之前，不该记为命中）。
- **判据**：同一配置下 SSR 有效像素占比由 0 变为与线性路径同量级；反射在**平面镜**前与
  解析镜像一致（GILab 场景里可用地板/墙做目视与数值双重确认）。
- **回归**：镜面层栈三种配置（空 / IBL / IBL+SSR）的 HDR 读数与告警不变的部分必须逐项一致；
  白炉 1.0000；单元测试全绿。
**2 · D1 崩溃根因** — 长跑 soak + `HE_TRACE_FB=1`（`delay=0` 即同帧销毁的直接指标）。
**本轮进展（未结项）**：

已查实并修掉一条**偶发卡死**的根因（与崩溃同属 D1 这一类「偶发失败」）：
...
- **Release 现在生成 PDB**（`CMakeLists.txt`：`CMAKE_MSVC_DEBUG_INFORMATION_FORMAT=ProgramDatabase` + Release 链接器显式 `/DEBUG`）。此前 Release 不带调试信息，崩溃报告里每一帧都是「(无符号信息)」，等于没有证据。已确认 `build/bin/Release/06.GILab.pdb` 落在崩溃处理器搜索的目录里（exe 同目录）
- **崩溃处理器改为先落 minidump、再做符号化**：符号解析（DbgHelp + StackWalk64）可能很慢甚至卡住，而它只是锦上添花；minidump 是唯一不可再生的证据
- **新增 soak 工具** `Tools/gi/soak_launch.ps1`：批量启动并逐次判定 OK / CRASH / TIMEOUT / NODUMP，把"偶发"变成可度量的频率，并把每次的崩溃报告单独留存

**仍未解决（下一步）**：

1. 原始的**偶发访问违例**本轮 **64 次启动零复现**（24 次基线 soak + 40 次修复后 soak），无法定位
2. **`HE_CRASH_TEST=1` 自检路径当前失效**：主动崩溃后进程挂住、既不产出 minidump 也不进入异常过滤器（crash log 里只有"已安装"一行）。也就是说 **D1 证据链本身不可靠** —— 这是"根因一直拿不到"的结构性原因，应作为第 2 项的下一步：先让自检路径稳定产出 dump + 报告，再去抓真实崩溃
...
| `Tools/gi/ddgi_grid_check.ps1` + `ddgi_grid_check.py` | **DDGI 探针网格覆盖检查**（任务 14 / §9.2-K 的回归测试），四个配置 × 各自 `none` 基线：固定网格（`ddgi_grid_auto=0`，罩不住场景）贡献必须塌到噪声量级、三种拟合（8/16/32 ⇒ 192/1408/9408 探针）必须恢复贡献、且**三种分辨率之间两两相差 < 1%**（后者是任务 17 的前提：当前 IBL 回退路径的探针场是均匀的）。配置里**必须把 RSM 权重置 0**，否则 DDGI 走 `useRSM=1` 路径，探针位置真的参与运算，第三类判定就不再成立 |
| `Tools/gi/rsm_indirect_check.ps1` + `rsm_indirect_check.py` | **RSM 间接光检查**（任务 16 的回归测试），两例：空漫反射栈 / `diffuse={RSM}`，都带 `HE_PASS_TIMING=1`。三条判定——① 半分辨率 `RSM_Indirect` pass 在跑且耗时非零（这是任务 16 的全部要点）；② Lighting 含 RSM 对不含 RSM 的涨幅 < 40%（改前 +88%，改后 +21%）；③ RSM 光栅化 pass 在。**`S_rsm` 只报告不断言**：它当前在 1e-8 量级（§9.2-AA 未闭合），把"约等于 0"写成断言等于把缺陷冻结成期望行为。**镜面栈必须非空**（否则触发 §9.2-X 的 463 亮点，把要看的量淹掉） |
...
- **A/B 对照必须给每次运行一份私有的 cfg 副本。** 示例程序退出时会把 `HE_GILAB_CONFIG`
  ...
  被误读成代码差异（任务 11.1 的第一次 A/B 就因此得到一个 −2% 的假差异）。这也是 §9.2-Y
  那条"空层栈状态不可靠"的一半来源。
...
| 风险 | 影响 | 对策 |
|---|---|---|
| 偶发崩溃根因未证 | 长跑 + 读回类验收被污染 | 已修最可疑项（FB 同帧销毁）与**同类的偶发卡死**（acquire 信号量复用，1/24 → 0/40）；Release 现已生成 PDB、崩溃处理器先落 minidump；但 `HE_CRASH_TEST` 自检路径失效（不产 dump、不进过滤器）——**先修好证据链**，再抓真实崩溃 |
