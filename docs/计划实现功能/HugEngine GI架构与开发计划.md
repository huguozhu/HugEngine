...
| **P2** | **SSGI-CAL · SSGI 标度与量纲标定**（由 §9.2-L/P 与 §3.3 暴露） | 中 / 中 | 几何前置（§9.2 M/N/O）已完成；剩余是**量纲缺失**——shader 不含入射辐射度项，归一化因此只给 SSGI 很小权重。**这是当前 GI 合成链上的主要缺口，也是 P5 退场后的接棒项**；需新增颜色输入与帧图依赖。⚠️ 权重具体数值**待重测**（受 §11.3.1 影响） |
| **P2** | ~~**§9.2-L · SSGI 完全不生效**~~ —— ✅ **已修复** | 小 / 低 | Provider 覆写改名导致输入恒空；顺带说明「开关状态」不足以证明某源真的参与合成 |
| **P2** | ~~**§9.2-M/N/O · SSGI 采样方向与可见性**~~ —— ✅ **已修复** | 中 / 中 | TBN 变换方向反了（主因）、世界/view 空间混用、可见性判据方向相反；三项合计令产出 ×5.2、正贡献像素 53.4% → 99.5% |
