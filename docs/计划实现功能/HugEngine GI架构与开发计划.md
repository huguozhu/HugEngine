...
| DDGI | compute | **256 探针**（8×4×8）× `numSamples` 次单步采样。✅ **任务 12 起可按 `updateStride` 时间维分摊**（每 N 帧更新一轮探针，每帧样本量降为 1/N；默认 1 = 既有行为） | **0.019～0.025 ms**（分摊到 1/4 时读数没有可判别的下降：这个 pass 本来就只占百分之几毫秒，**收益在本网格尺寸下低于噪声**） |
...
| `Tools/gi/cost_report.ps1` | **GI 耗时成本表**（§3.5 的数字来源 / 任务 13 的判据）：跑三种配置（`{SSGI}`、`{RTGI}`、`{DDGI,RTGI}+RT 反射+RT 阴影`）并打印各 GI 项与 `AS_Build` 的 GPU 耗时。读法：**只有真跑过的项才非零**（不在层栈的源没有 pass；IBL 只在脏时烘焙），所以 0 表示"没跑"而不是"没测" |
