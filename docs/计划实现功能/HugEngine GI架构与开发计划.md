...
> ⚠️ **该表中「DDGI / SSGI 量级比」一行不可引用**：DDGI 的绝对量级后来发生了无法解释的
> 12 倍迁移（0.01225 → 0.00102，原因未定位，见 **§11.3.1**）。同表其余各行是尺度无关量
> （相关系数、能量占比），**不受影响**——本节的结论只依赖后者。按最新读数，两者量级
> 实际只差约 1.3–1.8 倍。
...
> **P5 顺带暴露出的真正阻塞点（不是合成规则）**：归一化下等权重时 SSGI 只占输出值的很小
> 一份（早先按 67.7 倍记为 **1.5%**，该倍数**待重测**，见 §11.3.1），同时把 DDGI 自身贡献
> **折半**——即 SSGI 每帧付一整幅屏幕空间 pass 的成本，却几乎看不见（§3.5「白付一份全量
> 成本」）。叠加 §3.4 的量纲问题，**应先做 §10 `SSGI-CAL` 标定，而不是改合成模式**：
> 在标定完成前调整混合规则，等于给一个未标定的启发式量加权。
...
> 主要缺口（§3.3）：标度未统一时，归一化把 SSGI 压到很小的贡献（早先记为约 **1.5%**，该
> 数值**待重测**，见 §11.3.1）——开着却几乎看不见。
...
| 状态 | SSGI 原始纹理亮度 | 合成贡献 | 正贡献像素 | 纹理非零像素 | DDGI/SSGI ⚠️ |
|---|---|---|---|---|---|
| 起点（仅 L 已修） | 3.935e-4 | 1.791e-4 | 53.4% | 82.6% | 105 |
| 只修 O | 1.190e-3 | 5.554e-4 | 93.1% | 99.2% | 36.8 |
| 修 O + N（M 未修） | 9.436e-4 | 4.450e-4 | 70.1% | 89.8% | 51.0 |
| **M + N + O 全修** | **2.052e-3** | **9.356e-4** | **99.5%** | **99.997%** | **22.9** |

> ⚠️ **末列「DDGI/SSGI」不可引用**（见 §11.3.1：DDGI 的绝对量级发生了无法解释的 12 倍迁移）。
> 表中**前五列只涉及 SSGI 自身**，是同一环境下背靠背测出的相对变化，**结论不受影响**。
...
> 遗留：三处几何缺陷修完后，SSGI 仍明显弱于 DDGI（倍数**待重测**，见 §11.3.1；早先记为
> 22.9 倍），且累加项用的仍是命中点 **反照率**而非入射辐射度（§9.2-P）。**这两件事不是
...
> 已完成 / 已退场的任务不再占用编号：完成项见 **§8.1**，提交索引见 **附录 A**，
> 其详情见 **§10.2**。

### 10.1 剩余任务（从 1 起编号）

**A 组 · 先让验收可信**（这两项未查清前，其余任务的验收都缺判据）

| # | 任务 | 规模/风险 | 理由 / 依赖 |
|:---:|---|---|---|
| **1** | **DDGI 绝对量级不可复现的根因**（§11.3.1） | 未知 / 中 | **它污染所有以 DDGI 为参照的验收**——§11.3.1 已写明"在查清之前，任何「SSGI 比 DDGI 弱 N 倍」的结论都不成立"，而第 10 项的验收标准正是"与 PT 参考同量级"。可疑机制已缩小到 DDGI 的时域回路（`blendAlpha` + 前帧 HDR 反馈），但未证实 |
...
| # | 任务 | 规模/风险 | 理由 / 依赖 |
|:---:|---|---|---|
| **12** | **AMORTIZE · 时间维分摊**（DDGI 每 N 帧更新 + 时域复用） | 中 / 低 | 把 256 探针的全量更新摊到多帧。⚠️ **它与第 1 项是同一个机制**（`blendAlpha` 时域回路），**动手前应先看第 1 项的结论**——否则可能是在加固一个行为未知的回路 |
...
| **17** | **B4 / M5.2-A · DDGI 光追 march** | 中 / 中 | 只提升单一源质量。⚠️ **第 1 项未查清前不适合再动 DDGI**（否则量级变化无法归因） |
...
**1 · DDGI 绝对量级不可复现的根因**（§11.3.1）

- 已排除 7 项（代码变化 / 场景 / 收敛 / pass 顺序 / 历史初始化 / `falloffDistance` / 配置），
  见 §11.3.1 的排查表。
- 剩余唯一可疑机制：**DDGI 的时域回路**——探针 SH 走 `blendAlpha=0.85` 的时间混合，
  而它采样的「前帧 HDR」由 Lighting 产物下采样而来（`GI_CaptureRadiance`），
  **构成一个跨帧反馈环**。要判断的是：这个环的增益是否**接近 1**（⇒ 稳态强烈依赖初始条件
  与内存布局），以及是否被「RTGI 的 miss 回退即 DDGI」（第 3 项）进一步放大。
- 验证手段：把「前帧 HDR」换成**固定输入**（或强制断开反馈），看稳态是否随之改变。
  若改变 ⇒ 反馈环成立，需要给 DDGI 一个明确的能量约束，而不是靠 `blendAlpha` 收敛。
- **它阻塞第 10 项的验收标准，也是第 12、17 项的前置。**
...
- 动机：§9.2-L 修好后 SSGI 已真正生效，但实测（同一场景、单源层栈）DDGI 仍明显强于 SSGI，
  ...
  > ⚠️ **倍数与权重占比待重测**：早先记为「强 22.9 倍 / 权重 4%」，但 DDGI 的绝对量级后来
  > 发生了无法解释的 12 倍迁移（§11.3.1）。**定性的结论（SSGI 远弱于 DDGI、量纲不可比）
  > 不受影响**，定量数值须在重新测定后再填。
...
- **验收判据**：同一环境下**背靠背**的单源采样逐项一致（pass 集合与顺序、各 `provN_raw/final`
  纹理数值）+ 白炉 1.0000 + 单元测试全绿。**该判据是相对比较，不受 §11.3.1 影响。**
...
#### 11.3.1 ⚠️ 测量可靠性：DDGI 的绝对量级目前不可跨构建复现

**现象**：同一次调查过程中，DDGI 对单源做差的贡献从一个稳定值变成了另一个稳定值——
早先记录 **0.01225**，后来（含现在）稳定在 **0.00102**，相差约 12 倍。

**已排除**（逐项实测）：

| 排查项 | 结果 |
|---|---|
| 代码变化 | 检出当时的提交 `0147add` 重测，**同样是 0.00102**——不是后续改动造成的 |
| 场景/相机/光照是否变了 | **否**：空栈基线 `none` 的绝对读数前后**逐位相同**（0.100791） |
| 收敛不足 | **否**：预热 60 / 240 / 600 帧读数恒为 0.00102，已收敛 |
| 帧图 pass 名字与顺序 | **否**：把 pass 名改回原名无影响；已用 `HE_TRACE_PASSES` 确认每帧执行且顺序不变 |
| 探针历史未初始化 | **否**：把历史缓冲零初始化后读数几乎不变（0.0010248 对 0.0010211）。这条同时**否定**了「时域回路是单位增益积分器、稳态只是初值记忆」的假设 |
| `falloffDistance` 影响合成权重 | **否**：`loadStack` 只传权重，该项恒为 0，权重就是 1.0 |
| 配置模板与采样配置 | **否**：逐键核对一致（`gi_blend_diffuse_w1=1`、`gi_solo=0`、`ddgi_blend=0.85`） |

**结论**：原因未定位。DDGI 的时域回路（`blendAlpha=0.85` 叠加「前帧 HDR 反馈」）是唯一
还能解释「稳态值随构建/内存布局迁移」的机制，但**尚未证实**。

**因此**：凡**以 DDGI 为参照的比值**（§3.3 的量级比、§9.2-M/N/O 的 DDGI/SSGI 比、
§10.1 SSGI-CAL 的倍数与权重占比）**在重新测定前都不可引用**，文中这些数字已就地标注。
按最新读数，DDGI 与 SSGI 的量级只差约 **1.3–1.8 倍**，而非 22.9 倍。

**不受影响**：P5 的结论（相关系数、频段能量占比都是尺度无关量）；以及 SSGI 自身的 A/B
（每次都是同一环境下背靠背测量，比较的是相对变化）。

**下一步应做**：查 DDGI 的时域回路——它为何能让稳态值依赖构建环境。在查清之前，任何
「SSGI 比 DDGI 弱 N 倍」的结论都不成立。
...
| **帧图从「按通道」改为「按 Provider」执行** | 核心路径回归 | 逐类迁移 + 每步判据：**先只合并 specular 与 diffuse 两条循环**（形状相同、且正是 Lumen 需要共享的一对），AO 因存在旁路暂不动，`Compute`(DDGI) 与 `Custom`(IBL) 暂留；判据是**同一环境下背靠背的单源采样逐项一致**（该判据不受 §11.3.1 影响） |
