...
| DDGI | compute | **256 探针**（8×4×8）× `numSamples` 次单步采样。✅ **任务 12 起可按 `updateStride` 时间维分摊**（每 N 帧更新一轮探针，每帧样本量降为 1/N；默认 1 = 既有行为） |
...
| **Z** | 中 | **面板上的「每源耗时」是假信息**：`GIDebugData::avgRenderTimeMs` 只有声明与显示两处，**全仓没有一处给它赋值**（`grep` 只命中 `GlobalIllumination.h` 的定义行），因此 06.GILab 面板上「SSGI 耗时 / DDGI 耗时 / IBL 耗时 / SSR 耗时」**恒为 0.00 ms**。危害在于它长得像一个可用的性能读数：性能类任务（任务 12 的分摊收益、任务 13 的剔除收益、任务 17 的 march 质量/代价）都会自然地去读它，而它会一直回答 0 —— 与 §9.2-U（旧转储）、§9.2-Y（基线分两组）同属**测量可靠性**这一类。修法：接真实计时（GPU 时间戳查询最准；退一步用 per-pass 的 CPU 侧提交耗时并明确标注口径），并把「读数是否真的在变」纳入检查 | 排查/修法见 §10.1 任务 29 |
...
| **28** | **§9.2-Y · "空层栈"这一配置状态不可靠（兜底把 IBL 悄悄补回）** | 中 / 中 | 同一份逐字节等价的配置（漫反射四个权重全 0）在两次实验中落到两种有效层栈：一次保持空（基线 0.05720，三次可复现），一次被兜底补成 `{IBL}`（0.07554，且运行后示例程序把配置文件回写为 `gi_blend_diffuse_w0=1.000000`）。差别达 **32%**，足以污染任何"以空栈为基线"的做差实验。要做的是把这条路径查清并消除二义性：`GIRegistry::Degrade` 的兜底在什么时机、对哪个对象生效？为什么两次运行不同？以及**空层栈到底该不该被允许**（若不允许，就把它变成一个显式的、可复现的状态，而不是看运气）。判据：同一份配置连续 5 次运行必须给出同一个有效层栈与同一个读数 |
| **29** | **§9.2-Z · 给 GI 源装真实耗时读数**（面板现在恒显示 0.00 ms） | 中 / 低 | 性能类任务（12 的分摊、13 的剔除、17 的 march）都要"收益"这个数，而当前唯一看起来能给出它的 `GIDebugData::avgRenderTimeMs` **从未被赋值**。要做的：接真实计时——最准是 GPU 时间戳查询（每 pass 一对 timestamp，跨帧读回，注意飞行帧与读回延迟），退一步是 per-pass 的 CPU 提交耗时并**在 UI 上标明口径**（提交耗时 ≠ GPU 耗时）。判据：同一配置下开/关时间维分摊（`ddgi_update_stride`）时该读数**必须随之变化**，且变化方向与"每帧样本量 8192 → 2048"一致；读数在连续两次运行间的离散度有记录 |
