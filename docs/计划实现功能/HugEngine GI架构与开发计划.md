...
| 状态 | SSGI 原始纹理亮度 | 合成贡献 | 正贡献像素 | 纹理非零像素 | DDGI/SSGI |
|---|---|---|---|---|---|
...
> 遗留：三处几何缺陷修完后，SSGI 仍比 DDGI 低 **22.9 倍**，且累加项用的仍是命中点
> **反照率**而非入射辐射度（§9.2-P）。**这两件事不是同一类问题**：前者是几何/方向
> 错误（本次已修），后者是量纲缺失，必须补 `L_in` 并做实测标定才能解决——归 §10.1
> `SSGI-CAL`。在补齐之前，SSGI 无论怎么调 `intensity` 都不是 `E/π` 的估计。
...
| **P2** | **SSGI-CAL · SSGI 标度与量纲标定**（由 §9.2-L/P 与 §3.3 暴露） | 中 / 中 | 几何前置（§9.2 M/N/O）已完成；剩余是**量纲缺失**——shader 不含入射辐射度项，归一化只给 SSGI 约 **4%** 权重。**这是当前 GI 合成链上的主要缺口，也是 P5 退场后的接棒项**；需新增颜色输入与帧图依赖 |
...
- 动机：§9.2-L 修好后 SSGI 已真正生效，但实测（同一场景、单源层栈）**DDGI 比 SSGI 强
  22.9 倍**，且 `GI/SSGI.frag` 的累加式**只含命中点 albedo**：
  ...
  同一个量（§3.4），故现在 SSGI 只占输出值约 **4%**（1/(1+22.9)）——开着、每帧付一整幅
  pass 的成本，却几乎看不见。
