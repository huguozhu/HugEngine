...
| **AE** | 中 | **SSR 的 Hi-Z 层次 march 会漏掉反射**（任务 32 的平面镜解析对照发现）。同一个镜面、同一帧、同一套场景尺度参数下，线性 march（`ssr_use_hiz=0`，本 pass 的正式回退路径）把红盒与绿盒的反射**都**放在解析预测像素上（偏差 0.44 px / 0.35 px），而**默认**的 Hi-Z 路径只找到红盒（0.44 px，8633 像素），**绿盒一个像素都没有**。因此"两条 march 同量级 + 有效率非零"这类判据看不见它 —— Hi-Z 的镜面有效率反而是**最高**的（71%）：只有解析真值能判。**已定位的部分**：粗层的 min 深度可能属于射线路径**旁边的**几何，于是"该层被遮挡 ⇒ 降级"在斜视时会把整片反射绕过去（任务 32 已修掉其中一处：level 0 未过厚度判据时不再无条件 break，改用与线性 march 同一条早退判据）。**当前处置**：解析判据只钉线性回退路径，Hi-Z 那一路的读数由 `Tools/gi/ssr_mirror_check.ps1` 作为 `[KNOWN]` 报告项打出来，收尾留在任务 35 | 见任务 32 / 任务 35；`Tools/gi/ssr_mirror_check.{ps1,py}` |
| **AG** | 中 → ✅ **已修复**（任务 31 顺带） | **MRT 帧缓冲的附件数写死 7 个颜色**：`VulkanCommandList::BeginOffscreenPassMRT` 里附件数组是 `VkImageView attachments[kMaxColorAttachments]`（8）且循环写成 `attachmentCount < 7`，而 render pass 是按 PSO 的 `colorAttachmentCount` 建的（`kMaxColorAttachments` = 8，render pass 侧本来用 `kMaxColorAttachments + 1` = 9 个附件）。于是给 GBuffer 加第 8 个 MRT（任务 31 的光照图键）时，`vkCreateFramebuffer` 收到 8 个附件而 render pass 期望 9 个 ⇒ 校验层报 `attachmentCount 8 does not match 9`，紧接着驱动在 `vkCmdBeginRenderPass` 里崩溃（`0xC0000005`，栈顶 nvoglv64）。**这解释了为什么"GBuffer 七个 MRT 已满"看起来是硬上限**：真正的上限是 8 个颜色，卡住的是这段写死的 7。**修法**：附件数组与清除值数组都按"颜色上限 + 1（深度）"开，循环用 `kMaxColorAttachments`；pass 内其余逻辑不变 | `RHI/Vulkan/VulkanCommandList_RenderPass.cpp`；复现与判据 `Tools/gi/lightmap_key_check.ps1` |
...
| **31** | **Lightmap 真落地**（任务 18 的改判路径）—— **进行中**（前置条件 ① 部分落地，见 §10.2 任务 31） | 大 / 中 | **前置条件**：① 一个逐像素的**光照图键**；② 一条**烘焙路径**；③ 与 DDGI 的**分工**必须在 REDUNDANCY 诊断里说得清。**本轮已落地**：GBuffer 的第 8 个 MRT（`gb_lightmapkey` = uv0.xy + objectIndex，Deferred Lighting 已绑定 + 转储 + 检查工具 `lightmap_key_check`）、以及 RHI 里"MRT 附件数写死 7"的修复（不修它第 8 个附件会让驱动崩溃，见 §9.2-AG）。**两项实测结论（决定了剩余设计，也是本轮最有价值的产出）**：① **`objectIndex` 不是"一个物体"** —— 它是 **draw index**，GPU-driven 路径会把多个网格合批进一次绘制，实测 49 个页里只有 **34** 个页的世界 AABB 小于场景对角线的 30%（最差一页横跨 **85.7%**）⇒ **页号必须来自逐顶点数据**；② **uv0 不是光照图展开** —— 它是平铺纹理坐标（范围 [-1.42, 28.97]，只有 67% 的像素落在 [0,1]），128² 页下 **70.60%** 的 texel 被 ≥2 个不同世界位置命中（256² 下 49.37%）⇒ 必须有**程序化展开**。**剩余工作**（未完成，判据一条都还没验）：逐顶点的光照图 UV + 页号（顶点格式 + 生成 + 序列化）、按主导轴的 6 tile 箱式展开、烘焙（UV 空间光栅写世界位置/法线 + 光追逐纹素求辐照度）、源（能力位 + `DeferredLighting` 分支 + REDUNDANCY 分工说明）、判据（比 DDGI 锐 / 关掉实时 GI 仍成立 / 每帧零成本）。复现与检查：`Tools/gi/lightmap_key_check.{ps1,py}`；进度记录见 §10.2 任务 31 |
...
| **35** | **§9.2-AE · SSR 的 Hi-Z 路径漏反射**（任务 32 发现） | 中 / 低 | 平面镜解析对照显示：默认的 Hi-Z 路径能找到红盒反射（0.44 px）却**完全漏掉绿盒**，而线性回退路径两个都准（0.44 / 0.35 px）。做法：① 先把 `ssr_mirror_check.ps1` 里的 `[KNOWN]` 报告项升级成断言 —— **两条路径都必须落在预测像素 6 px 内**；② 逐层落盘 Hi-Z 的层级选择序列（哪一层、为什么前进/回退/降级），与线性 march 的命中步数对照，把"绕过去"的那一次判断揪出来；③ 修完复核任务 25 的收益（Hi-Z 的步数/耗时仍须显著优于线性）。**注意判据选用**：不要用"有效率"当判据（Hi-Z 的有效率反而更高：镜面上 71% vs 线性 4.60%），也不要只用"两条路径同量级" |
...
- **改前指纹**：`pipeline_mode=0` 下 `diffuse={RSM}` 的 HDR 与**空漫反射栈**逐位相同
  ...
  1. **示例没驱动阴影系统**：`ShadowSystem` 要调用方先 `SetRenderResources`（对象/阴影缓冲 +
     描述符集）再 `Update`（收集投影光源、拟合 CSM），02.Cube / 03.Sponza / AISamples 都这么做，
...
- **回归**：`rsm_indirect_check` 8/8（Deferred 侧读数不变：位置 53.36%、`S_rsm` 4.284e-05 ——
  ...
  静态断言在其中）。
- **残留风险（已记录）**：02.Cube / 03.Sponza / AISamples 也走 Forward 且都驱动阴影系统，
...
**31 · Lightmap 真落地（进行中）** —— 前置条件 ① 部分落地；两项实测结论改变了设计

- **已落地（本轮）**：
  1. **GBuffer 第 8 个 MRT `gb_lightmapkey`**（RGBA16F）= `(uv0.x, uv0.y, objectIndex, 0)`：
     `GBuffer.frag` 的 `SV_Target7`、`GBufferRenderer`（纹理/PSO/清除值/RenderGraph 导入/访问器）、
     `DeferredPipeline_FrameGraph` 的读写依赖、`LightingPass` 的绑定（点采样：键是**索引**，
     线性过滤会把页号插值成另一个物体）与 `DeferredLighting.frag` 的声明、示例的
     `gb_lightmapkey` 转储。
  2. **RHI 修复 §9.2-AG**：`BeginOffscreenPassMRT` 此前把颜色附件写死 7 个（数组 8、循环 `< 7`），
     而 render pass 按 PSO 的 `colorAttachmentCount` 建（上限 8）⇒ 第 8 个附件让
     `vkCreateFramebuffer` 收到 8 而 render pass 期望 9，校验层报警后驱动在
     `vkCmdBeginRenderPass` 崩溃（`0xC0000005`）。改完第 8 个 MRT 才能跑。**这也修正了任务 18
     记下的"GBuffer 七个 MRT 已满"**：真正的上限是 8 个颜色附件，卡住的是这段写死的 7。
- **两项实测结论**（`Tools/gi/lightmap_key_check.ps1`；这两条决定了后面怎么做，是本轮最主要的产出）：
  1. **`objectIndex` 不是"一个物体"，而是 draw index**。GPU-driven 路径把多个网格合批进一次
     绘制（MeshBatcher），于是一个 index 可能覆盖相距很远的几何。实测：49 个页里只有 **34** 个
     页的世界 AABB 小于场景对角线的 30%，最差一页（page 18）横跨 **85.7%**。
     ⇒ **光照图的页号必须来自逐顶点数据**，不能从绘制索引推。
  2. **uv0 不是光照图展开，而是平铺纹理坐标**。实测：uv 范围 **[-1.42, 28.97]**，只有 **67.25%**
     的像素落在 [0,1]；按候选烘焙分辨率量化后，128² 页下 **70.60%** 的 texel 被 ≥2 个不同世界
     位置命中（256² 下 **49.37%**）。⇒ 用 uv0 当键会把无关的表面点混进同一个 texel，
     **必须有程序化展开**（按主导轴的箱式投影，每物体 6 个 tile）。
- **剩余工作（未做，判据一条都还没验）**：
  - 逐顶点的**光照图 UV + 页号**：`StaticVertex` 加通道、顶点布局/着色器透传、**生成**（按物体
    程序化展开，物体级页号分配）与**序列化**（写回网格资产或载入时生成）；
  - **烘焙**：UV 空间的几何光栅（写世界位置 + 法线到页）→ 光追逐纹素求辐照度（可复用
    `DDGITracePass` 的 RT/SBT/参数设施，加"位置来自缓冲"的模式）；
  - **源**：能力位、`DeferredLighting` 的 `GISOURCE_LIGHTMAP` 分支（页 = 物体、页内 = 展开 UV）、
    与 DDGI 的 REDUNDANCY 分工说明（静态高分辨率烘焙 vs 动态/低频探针）；
  - **判据**：同一场景下 `{Lightmap}` 的间接光高频细节显著强于 `{DDGI}`（图像/高频能量对照）、
    关掉实时 GI 后画面仍成立、每帧 GI 成本为 0（lightmap 不需要逐帧 pass）。
- **为什么这一轮没能做完**：任务 18 估计的"加第 8 个 MRT 即可"是**低估**——通道只是必要条件，
  键的语义还缺两样（逐顶点展开 + 页号），而这两样各自都要动顶点格式与资产。任务保持**未完成**，
  不设"已结项"标记。
...
| `Tools/gi/lightmap_key_check.ps1` + `lightmap_key_check.py` | **光照图键检查**（任务 31 的前置条件 ①）：跑一帧 Deferred 并转储 `gb_lightmapkey`（第 8 个 GBuffer MRT = uv0.xy + objectIndex）。断言三条 —— 键的覆盖面**等于**几何覆盖面（天空为零键）、页号是精确整数、页号落在 `kGPUMaxObjects` 内。**报告两条实测结论**（它们是剩余工作的判据、现在还不成立）：① 页号**不是物体号**（49 页里只有 34 页的世界 AABB 小于场景对角线的 30%，最差 85.7%）——`objectIndex` 是 draw index，合批后一页可跨半个场景；② uv0 作键**不唯一**（128² 下 70.60% 的 texel 被 ≥2 个不同世界位置命中）。**顺带守住 §9.2-AG**：这条检查跑起来本身就要求 RHI 的 MRT 附件数不再是写死的 7（改前第 8 个附件会让驱动崩溃） |
...
| **C++/slang 双侧同步漏改** | 静默错值 | `static_assert(sizeof)` + 双侧常量同源；**结构体布局**另需 `offsetof` + `slangc -reflection-json` 复核 —— §9.2-AF：std140 下**非 float4 数组**的元素步长是 16，`float[3]` 在 Slang 里占 48 字节而 C++ 只占 12，数组之后的字段两端偏移错开、着色器静默读 0（共享结构体里的填充一律用 `float4`，`Pipeline/Material.h` 已把 `GIBlendParams` 的 `sizeof` 与三个 `offsetof` 钉死） |
