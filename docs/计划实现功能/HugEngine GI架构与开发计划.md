...
| **31** | **Lightmap 真落地**（任务 18 的改判路径）—— **进行中**（前置条件 ① 部分落地，见 §10.2 任务 31） | 大 / 中 | **前置条件**：① 一个逐像素的**光照图键**；② 一条**烘焙路径**；③ 与 DDGI 的**分工**必须在 REDUNDANCY 诊断里说得清。**本轮已落地**：GBuffer 的第 8 个 MRT（`gb_lightmapkey` = uv0.xy + objectIndex，Deferred Lighting 已绑定 + 转储 + 检查工具 `lightmap_key_check`）、以及 RHI 里"MRT 附件数写死 7"的修复（不修它第 8 个附件会让驱动崩溃，见 §9.2-AG）。**两项实测结论（决定了剩余设计，也是本轮最有价值的产出）**：① **页号本身可用** —— 我最初用"页的世界 AABB 是否小于场景对角线的 30%"判"一页是不是一个物体"，实测 49 页里只有 **34** 页通过，并**误判为"objectIndex 是 draw index、合批后一页跨半个场景"**；**该判据是错的（下一轮更正）**：`GPUScene::Collect` 按**组件（=每实例）**收集对象，一页只可能装一个物体的几何，而**大而空的壳体**（Sponza 外壳）按定义横跨整个场景、AABB 中心 30% 内几乎没有像素（最大的几页 inside-centre-30% = **0.000**、中位 mid-gap **0.000~0.08** ⇒ 分布不是双峰的批次形态；少数页的 2~3 个屏幕连通块来自**遮挡**）；② **uv0 不是光照图展开**（真正的阻塞项）—— 平铺纹理坐标（范围 [-1.42, 28.97]，只有 67% 的像素在 [0,1] 内），128² 页下 **70.60%** 的 texel 被 ≥2 个不同世界位置命中（256² 下 49.37%）⇒ 必须有**程序化展开**。**剩余工作**（未完成，判据一条都还没验）：按主导轴的 6 tile 箱式投影展开（每物体 AABB 可放进对象缓冲，无需改顶点格式）、烘焙（UV 空间光栅写世界位置/法线 + 光追逐纹素求辐照度）、源（能力位 + `DeferredLighting` 分支 + REDUNDANCY 分工说明）、判据（比 DDGI 锐 / 关掉实时 GI 仍成立 / 每帧零成本）。复现与检查：`Tools/gi/lightmap_key_check.{ps1,py}`；进度记录见 §10.2 任务 31 |
...
- **两项实测结论**（`Tools/gi/lightmap_key_check.ps1`；这两条决定了后面怎么做）：
  1. **页号（`objectIndex`）本身可用** —— 但这一条我**先判错过一次，值得记下来**。最初的检查用
     "页的世界 AABB 是否小于场景对角线的 30%"来判"一页是不是一个物体"：实测 49 页里只有 **34**
     页通过，于是我写成"`objectIndex` 是 draw index，GPU-driven 合批让一页跨半个场景"。**这个
     结论是错的**，下一轮复核时查清了两件事：
       · 代码事实：`GPUScene::Collect` 是**按组件（=每实例）**收集对象的，一页只可能装一个物体
         的几何；`MeshBatcher` 只服务 GPU 绘制路径，不改 `objectIndex` 的语义。
       · 测量事实：那些"跨得很大"的页是**大而空的壳体**（Sponza 的建筑外壳）—— 按定义就横跨
         整个场景，而它的 AABB 中心 30% 内几乎没有像素（最大的几页 inside-centre-30% =
         **0.000**，沿最长轴的中段空隙中位数 **0.000~0.08** ⇒ 世界位置分布**不是**双峰的合批
         形态）；少数页出现的 2~3 个屏幕连通块来自**遮挡**（一个物体被柱子切成几段屏幕区域）。
       ⇒ 结论：**AABB 紧致度是一个错误的判据**（空心物体必然不合格），页号可以直接用。
     【教训】用"包围盒大小"这类**代理量**去证明"语义上是不是同一个物体"很容易错；能查到代码
     事实（收集粒度）时应当先查代码，而不是拿一个方便的几何量当判据。
  2. **uv0 不是光照图展开，而是平铺纹理坐标**（这是真正的阻塞项）。实测：uv 范围
     **[-1.42, 28.97]**，只有 **67.25%** 的像素落在 [0,1]；按候选烘焙分辨率量化后，128² 页下
     **70.60%** 的 texel 被 ≥2 个不同世界位置命中（256² 下 **49.37%**）。⇒ 用 uv0 当键会把无关的
     表面点混进同一个 texel，**必须有程序化展开**（按主导轴的箱式投影，每物体 6 个 tile）。
- **剩余工作（未做，判据一条都还没验）**：
  - 按主导轴的 **6 tile 箱式投影展开**：每物体的 AABB 加进对象缓冲（`GPUObjectData`）即可，
    **不需要改顶点格式** —— GBuffer 里用"世界位置 + 世界法线 + 该物体 AABB"就能算出
    `(页=objectIndex, tile=主导轴, tile 内 uv)` 的键，烘焙端用同一套公式；
...
| `Tools/gi/lightmap_key_check.ps1` + `lightmap_key_check.py` | **光照图键检查**（任务 31 的前置条件 ①）：跑一帧 Deferred 并转储 `gb_lightmapkey`（第 8 个 GBuffer MRT = uv0.xy + objectIndex）。断言三条 —— 键的覆盖面**等于**几何覆盖面（天空为零键）、页号是精确整数、页号落在 `kGPUMaxObjects` 内。**报告两条结论**：① 页号可用（`GPUScene::Collect` 按组件收集 ⇒ 一页只装一个物体；大的空心壳体横跨场景是正常的 —— 这里**记下了一次判据错误**：最初用"页 AABB 是否小于场景对角线的 30%"当判据，得到"49 页只有 34 页合格"并误判为合批，复核后确认那是空心物体 + 遮挡，判据本身不对）；② uv0 作键**不唯一**（128² 下 70.60% 的 texel 被 ≥2 个不同世界位置命中），需要程序化展开 —— 这是真正的剩余工作。**顺带守住 §9.2-AG**：这条检查跑起来本身就要求 RHI 的 MRT 附件数不再是写死的 7（改前第 8 个附件会让驱动崩溃） |
