...
可复现的实测数据（来自 `Tests/TestNaniteMaterialMapRepro.cpp` 的插桩输出）：
...
| 机理 | 去重后的三角形段装着**全部 6 个 LOD 级**的唯一内容，共 524657 条 ≈ 原始 262267 的 **2 倍** ⇒ 非 LOD0 的簇偏移大多落在原始区间之外 |
...
它们的 `triangleOffset` 虽落在原始区间内，但**次序已被去重打乱**。LOD0 是近距离可见的那一级
...
| 8 | **材质映射 `unmapped=4103`**（§14.33 ⑤） | `materials_sample` 读数行 | 无（读数已打印，但**没有断言/告警**把它变成"可见的问题"） |
...
| 11 | **资产 `vertexOffset` 非单调**（§14.32 ②，任务 24 的守卫） | 尚无（任务 24 才加 `stream=off reason=nonmonotonic`） | 待任务 24 |
| 12 | **页缺失 / 页池满**（§14.32 ⑨） | 尚无（任务 24 才加 `stream` 读数行） | 待任务 24 |
...
| 15 | **硬光栅 341× 过绘 / mesh 线程利用率 ≈50%**（§14.31 ⑨，留给任务 23） | 任务 23 的 `perf` 行（若落地） | 任务 23 落地后复核 |
| 16 | **关闭档泄漏 / pass 顺序漂移**（判据 ⑥、8e） | `acceptance_sweep` 判据 ⑥（含负向验证）+ 8e 冻结指纹 | 无 |
| 17 | **验收脚本自身的抖动**（§14.11：判据 ⑦/⑧ 在未改动代码上各失败过一次） | 只在人肉对比历史日志时可见 | **无工具**：没有"这次 FAIL 是不是抖动"的自动判定 |
