...
| 偶发崩溃 | 🔄 判据满足、根因未证（40/40 soak 零崩溃） | **Wave 0.1（§二）** |
| 每帧校验违规（四类） | ✅ 判据达成（0/0/0/0） | **Wave 0.7（§二）** |
| 白炉测试（能量守恒判据） | ✅ 设施与判据建成；读数 1.7998 待收敛到 1.0 | **Wave 0.2（§二）** |
...
| 开关 | 用途 |
|---|---|
| `HE_TRACE_FB=1` | framebuffer 创建/销毁追踪（`delay=0` = 同帧销毁，是"对象生命周期越界"类问题的直接指标） |
| `HE_TRACE_PASSES=1` | 打印每个 pass 的开始，把 pass 名与校验层报错在时间上对齐 |
...
- [x] **（0.1 门槛）** 连续 10 次冷启动零崩溃 —— ✅ 已满足（修复后累计约 100 次启动零崩溃，含
      Wave 0.7 修复后的 40/40 soak）。⚠️ 但**根因未获证明**：最可疑根因（framebuffer 被同帧销毁）
      已修复且机理与崩溃点同源，但原崩溃率约 1/55，故仍保留为**观察项**（§1.3.3）。
- [x] **（0.2）白炉测试设施与判据** —— ✅ 已建成（RHI 读回 + 探针 + `furnaceMode` + 绝对值判据）；
      判据：`HE_FURNACE=1` 时中心像素绝对亮度应为 **1.0**；**当前 1.7998**（+80%，即缺口 (A) 的
      归一化之外加法项），待 0.4/0.5 与 Wave 1 修复后收敛（§1.4(A)、进度快照）。
...
Wave 0.7（四类每帧校验违规）    ✅ 判据达成：真实计数 0/0/0/0（前置：FB 同帧销毁修复、布局追踪、
                                  render pass 边界维护、交换链同步原语多份化）
Wave 0.8（崩溃处理器）          ✅ 已完成并自检通过
   └─→ Wave 0.1（偶发崩溃按死）  🔄 判据满足（40/40 soak 零崩溃），根因保留为观察项
          └─→ Wave 0.2（白炉判据）✅ 设施与判据建成：读数 1.7998 → 目标 1.0
                 └─→ Wave 0（IBL/RSM 归位 = 把归一化之外的项收进层栈：0.4 / 0.5）
                        └─→ M5.3（DDGI SH 修正，影响低频源能量）   ← 可与 Wave 0 并行/紧随
                        └─→ Wave 1（UBO 按源数组化 + shader 按源分派）  ← ⏳ 下一步
...
| **偶发崩溃（55 次 1 崩）** | 白炉等"长跑 + 读回数值"的验收被污染，可能误判为 GI 数值错误 | 🔄 崩溃点已定位到 `VulkanTexture::GetImageView()`（野指针，§1.3.2①）；最可疑根因（FB 被同帧销毁）已修 → **40/40 soak 零崩溃**、累计约 100 次启动零崩溃；根因保留为观察项，再发即由崩溃处理器出调用栈 |
| ~~周期性 Vulkan 校验违规（确定性必现）~~ | ~~命令缓冲引用已销毁对象 + 布局推导错误~~ | ✅ **已解决**：三处根因（命令列表析构擅自清空延迟销毁队列 / render pass 边界布局转变未同步追踪器 / 交换链每帧复用同一信号量）均已修复，真实计数 **0/0/0/0**（§1.3.4） |
| **白炉读数偏高（1.7998 应 1.0）** | 能量不守恒：IBL 环境项在归一化之外累加（缺口 A） | ✅ 判据已建成并量化误差；修复归入 Wave 0.4/0.5（把归一化之外的项收进层栈）与 Wave 1，收敛目标 = 1.0 |
...
| 合成改造后"画面变了" | 回归难判断 | 单源配置逐像素截图对比；白炉数值判据（现已可量化） |
| C++/slang 双侧同步漏改 | 静默错值（F1 老痛点） | `static_assert(sizeof)` + 双侧常量同源 + 改一处清单化 |
| ~~白炉读回设施工程量被低估~~ | ~~阻塞 Wave 0 验收~~ | ✅ 已完成（RHI `CopyTextureToBuffer` + 探针 + 面板/日志读数） |
| P4 抽象过度/爆改 | 大范围回归 | AO 通道单通道试点；适配器包装而非重写 |
| 频率分离的 `LowPass` 选型不当 | 噪声/振铃/跳变 | 先做离线小实验（降采样 vs SH 低阶）；保留 Normalized 作对照 |
| **校验层重复消息去重会掩盖计数** | 误把"报告数"当真实次数（本轮已两次误判） | 关闭 `duplicate_message_limit` 取真实计数，或用独立仪器（`HE_TRACE_FB` 的 `delay=0`）交叉验证 |
