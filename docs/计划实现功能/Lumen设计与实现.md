...
| 38 白炉真值覆盖 | ◐ 逐轮已做（未固化成脚本） | 每轮的验收都跑"开 Lumen 白炉：" `prov6_final` min = mean = max = **1.0000**；步骤 23 另有探针 l0 = √π 的解析判据。**待做**：把"每个新信号都要在白炉下自证标度"写成一条可复跑的扫描（本轮起草了 `build/verify/acceptance_sweep.ps1` 把 ①②③④⑤ 串成一条命令，但脚本自身的判定逻辑还不可靠 —— 两次运行给出不同结论，故不作为凭据，留待下一轮修） |
| 39 背靠背单源采样对照 | ◐ 逐轮已做（未固化成脚本） | 每轮都跑同配置两次并逐转储比对，且**已明确排除既有的抖动族**（`prov0_ao_*` SSAO、`hdr`、`radiance`）——判据是"抖动族之外 ≤2 个 f16 ULP"（步骤 27/28/35/36 均有实测记录）。**待做**：同上，固化进扫描脚本 |
