...
**现在可信的验证手段只有两类**（都不依赖那条读回路径）：① 逐 mesh 场自检（1024 探针、近表面
精确、与 CPU 参考自洽）；② **射线验证**（sphere tracing 的命中/穿漏统计，读的是场纹理）。
下一步把"场坐标 → 场值"的取样改成**沿射线逐点写进射线结果缓冲**（复用已工作的射线路径），
再与 CPU 精确距离对照 —— 之后才轮到洪泛精度（大网格 138~279 单位）与 3 条穿漏。
