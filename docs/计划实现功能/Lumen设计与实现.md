...
- **实测**：图像与基线差 **≤2 个 f16 ULP**（6819 像素）、白炉 1.0000、单测 237/5792 全绿；
  **校验层行数未变**（仍 46）—— 因为剩下的 `expects GENERAL` 来自 clipmap 层 / 覆盖率 / 调试 /
  atlas / 辐照度这几张，它们的创建点（`CreateGlobalGPUObjects` / `BuildCards` /
  `CreateDebugGPUObjects` / `CreateIrradianceTexture`）**没有命令列表参数**，要把转换挪进各自
  创建路径就得给它们加 `cmd`（或改成"创建 + 转换"两段式）。这一步留给下一轮，
  连同"把 12 ms 的 `RunFeedback` 回读改成确定性实现"一起做 —— 有了正确的布局前提，
  上一轮那个"改了读法就变黑"的现象才有一个可检验的解释。