...
> 也**不改变** §12 B 组的触发条件。本文件不含任何代码改动。
>
> **决策已下（2026-09-19）**：① ReSTIR GI/GRIS 的落点 = **P6 / Lumen 侧**（本文的 M0~M3 归那边执行，PT 侧只保留"薄适配"这一条接口要求）；② **PT 不转实时主路径** ⇒ B7 保持未做。本文件自此转为该决策的**依据与执行留档**（若将来重新评估，先看 §4.1 的成本表与 §8.6 的组合结论）。
>
...
| 落点与触发（**已决，2026-09-19**） | 落点 = **P6 / Lumen 侧**（实现一份内核，PT 侧只做薄适配）；**PT 不转实时主路径** ⇒ B7 保持未做。前置条件见 §8.5（11.3 统一降噪框架 + P6 数组化/PROVIDER-EXEC） |
