...
void EditorApp::MainLoop() {
    while (!m_Engine->GetWindow()->ShouldClose()) {
        ...
        // 当 ImGui 文本输入框激活时不触发，避免与文本编辑快捷键冲突
        ...
        // 鏇存柊瑙嗗彛鐩告満瀹介珮姣旓紙浠?SwapChain 灏哄鑾峰彇锛屼笉渚濊禆 ImGui 涓婁笅鏂囷級
        ...
        // 涓婚潰鏉跨獥鍙ｏ紙濉厖鑿滃崟鏍忎笅鏂瑰叏閮ㄧ┖闂达紝鏃犳爣棰樻爮/鏃犺竟妗嗭級
        ...
        // 3D 鍦烘櫙鐢?ViewportPanel::Render 娓叉煋鍒?BackBuffer
        // Phase 3-2: 鏀逛负娓叉煋鍒?off-screen texture 鈫?ImGui::Image
        ImGui::EndChild();
...
void EditorApp::Shutdown() {
    m_Device->WaitIdle();
    m_ImGui->Shutdown();
    // 鍦?Engine 閿�姣佸墠閲婃斁鎵�鏈夊瓙绯荤粺锛堥伩鍏嶆瀽鏋勬椂 Logger 宸插け鏁堬級
    m_Pipeline.reset();
    m_SceneGraph.reset();
    m_World.reset();
    m_CmdList.reset();
