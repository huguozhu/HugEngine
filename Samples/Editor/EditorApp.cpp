...
void EditorApp::MainLoop() {
    while (!m_Engine->GetWindow()->ShouldClose()) {
        ...
        // Ctrl+Z / Ctrl+Y Undo/Redo ¼üÅÌ¿ì½İ¼ü
        // µ± ImGui ÎÄ±¾ÊäÈë¿ò¼¤»îÊ±²»´¥·¢£¬±ÜÃâÓëÎÄ±¾±à¼­¿ì½İ¼ü³åÍ»
        ...
        // ³¡¾°äÖÈ¾£¨ÓÉ ViewportPanel Çı¶¯£©
        // æ›´æ–°è§†å£ç›¸æœºå®½é«˜æ¯”ï¼ˆä» SwapChain å°ºå¯¸è·å–ï¼Œä¸ä¾èµ– ImGui ä¸Šä¸‹æ–‡ï¼‰
...
void EditorApp::Shutdown() {
    m_Device->WaitIdle();
    m_ImGui->Shutdown();
    // åœ¨ Engine é”€æ¯å‰é‡Šæ”¾æ‰€æœ‰å­ç³»ç»Ÿï¼ˆé¿å…ææ„æ—¶ Logger å·²å¤±æ•ˆï¼‰
    m_Pipeline.reset();
    m_SceneGraph.reset();
    m_World.reset();
    m_CmdList.reset();
