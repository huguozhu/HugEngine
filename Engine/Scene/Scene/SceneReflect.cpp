...
HE_BEGIN_REGISTER(he::AnimationComponent)
    HE_REGISTER_PROPERTY(he::AnimationComponent, i32, currentClip)
        HE_ATTR_CATEGORY("Animation") HE_ATTR_AI_VISIBLE() HE_ATTR_AI_WRITABLE() HE_ATTR_AI_DESCRIPTION("当前播放的动画片段索引")
    HE_END_PROPERTY()
    HE_REGISTER_PROPERTY(he::AnimationComponent, float, time)
        HE_ATTR_CATEGORY("Animation") HE_ATTR_AI_VISIBLE() HE_ATTR_AI_WRITABLE() HE_ATTR_AI_DESCRIPTION("当前动画播放时间（秒）")
    HE_END_PROPERTY()
    HE_REGISTER_PROPERTY(he::AnimationComponent, float, speed)
        HE_ATTR_CATEGORY("Animation") HE_ATTR_AI_VISIBLE() HE_ATTR_AI_WRITABLE() HE_ATTR_AI_DESCRIPTION("播放速度倍率")
    HE_END_PROPERTY()
    HE_REGISTER_PROPERTY(he::AnimationComponent, bool, playing)
        HE_ATTR_CATEGORY("Animation") HE_ATTR_AI_VISIBLE() HE_ATTR_AI_WRITABLE() HE_ATTR_AI_DESCRIPTION("是否正在播放动画")
    HE_END_PROPERTY()
HE_END_REGISTER()
...
HE_BEGIN_REGISTER(he::DecalComponent)
    ...
    HE_END_PROPERTY()
    HE_REGISTER_PROPERTY(he::DecalComponent, float, projectionDepth)
        HE_ATTR_CATEGORY("Decal") HE_ATTR_AI_VISIBLE() HE_ATTR_AI_WRITABLE() HE_ATTR_AI_DESCRIPTION("投影体积厚度（世界单位，Deferred 投影贴花用：需包住要贴的表面）")
    HE_END_PROPERTY()
