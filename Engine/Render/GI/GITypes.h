...
enum class GISourceId : u8 {
    ...
    Lightmap      = 2,   // 烘焙光照（**未实现**：见 ToPipelineCap 里的说明；原任务 31 已取消）
    DDGI          = 3,   // 动态漫反射探针网格
    // 中频（近处细节）
    SSGI          = 4,   // 屏幕空间间接漫反射
    SSR           = 5,   // 屏幕空间反射
...
inline u32 ToPipelineCap(GISourceId id) {
    ...
    // 而两样都不在现有架构里 ——
    //   1. **逐像素的光照图键**：lightmap 必须按每像素的键（展开 UV + 页号）查表，而 GBuffer 的
    //      七个 MRT 槽位（A/B/C/D/E/F/G）原本已全部占满（第 8 个槽位与键通道后来在已取消的
    //      任务 31 里做过，见 §10.2；RHI 的 8 附件修复保留，但源本身仍未实现、仍不给位）；
    //   2. **一条烘焙路径**（离线或载入时多次弹射）。
    // 而"不做新通道、用世界坐标查表"的替代方案（世界空间辐照度体）与 DDGI 是**同一个估计量**，
    // 会被本仓库自己的 REDUNDANCY 诊断判为冗余源（§2.2 的「重复估计」）—— 那等于白付一份全量
    // 成本。故：**先不给位**；承接它的任务 31 已取消（见文档 §10.2）。
