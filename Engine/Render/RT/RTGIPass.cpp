...
void RTGIPass::Execute(rhi::IRHICommandList* cmd,
    ...
    pc.flags        = (m_QuarterRes ? 1u : 0u)
                    ...
                    // bit2=白炉（任务 33）：命中与未命中两条路径都返回"全白环境"的理想值，
                    // 于是白炉读数恒等于 1，成为一条能看见绝对量级的判据（§9.2-AC）。
                    | (ctx.furnace ? 4u : 0u);
