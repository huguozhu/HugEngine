...
struct NaniteSettings {
    ...
    // ============================================================
    // 【§14.8 任务 26】屏幕可视化（debug view）—— 验收明文点名的四项
    //
    // 【这个开关控制什么（三件事一起看）】
    //   ① 开启（非 0）时模块**自己**建一张 64×32 的 `R32_UINT` 小目标（8 KB）与一个 8 KB 的
    //      读回缓冲，并在 dump 帧多打印**恰好一行** `[Nanite] debug_view …` 读数；
    //   ② 关闭（0，**默认**）时：**一个 pass 都不注册、一个 GPU 资源都不建、一行日志都不打**
    //      （§14.2 不变式 1 的硬纪律；既有档位的日志与转储必须逐字不变）；
    //   ③ **不碰任何 GBuffer / 既有渲染目标**：可视化写的是模块自建的小目标，读回并打印统计
    //      ⇒ 它验证的是"可视化真的产出了非空画面"，而不是"改了可见画面"。
    //
    // 【档位与语义（一像素 = 一个屏幕 tile；详见 `NaniteTypes.h` 的 `kNaniteDebugView*`）】
    //     0 = 关（默认）；1 = 可见簇数；2 = 软硬光栅占比；3 = LOD 层级；4 = BVH 深度。
    //   目标宽 64 = 两个 32 列面板：面板 A 是各模式的主量，面板 B 是辅助量（模式 3/4 的"簇计数"、
    //   模式 2 的"硬簇计数"）。每一种模式的确切编码写在 `NaniteTypes.h` 与
    //   `Nanite_DebugView.comp.slang` 的注释里（一个像素代表什么、亮度编码什么量）。
    //
    // 【为什么默认 0（不是保守，是硬纪律）】新增 GPU 资源与新增读数行都必须挂在独立档位上：
    //   判据 ⑥/⑧e 的 pass 集指纹与 ⑧b 的转储差集都建立在"默认档什么都不变"之上。
    //   配置层 = cfg 键 `nanite_debug_view`（默认 0，钳到 [0, kNaniteDebugViewMaxMode]），
    //   样例负责解析/序列化 + 面板勾选框。
    u32 debugView = 0u;
};
