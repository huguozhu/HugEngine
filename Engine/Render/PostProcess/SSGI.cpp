...
bool SSGI::Initialize(rhi::IRHIDevice* device, u32 width, u32 height) {
...
void SSGI::Shutdown(){m_PSO.reset();if(m_Device&&m_DescLayout!=rhi::kInvalidLayout)m_Device->DestroyDescriptorSetLayout(m_DescLayout);m_Output.reset();m_Sampler.reset();m_PointSampler.reset();m_Device=nullptr;m_Ready=false;}

void SSGI::OnResize(u32 w,u32 h){m_Width=w;m_Height=h;CreateOutputTex(w,h);}

void SSGI::CreateOutputTex(u32 w,u32 h){
...
void SSGI::SetInputs(rhi::IRHITexture* depth, rhi::IRHITexture* normal, rhi::IRHITexture* albedo){
...
void SSGI::Render(rhi::IRHICommandList* cmd){
    if(!m_Ready||!enabled||!m_Depth||!m_Normal||!m_Albedo)return;
    ...
    pc.p=float4(radius,intensity,float(sampleCount),0);
